...
public interface CashflowCashflowAmount extends Validator<Cashflow> {
	...
	class Default implements CashflowCashflowAmount {
		...
		private ComparisonResult executeDataRule(Cashflow cashflow) {
			try {
				if (exists(MapperS.of(cashflow).<ResolvablePriceQuantity>map("getPriceQuantity", _cashflow -> _cashflow.getPriceQuantity())).getOrDefault(false)) {
					return greaterThanEquals(MapperS.of(cashflow).<ResolvablePriceQuantity>map("getPriceQuantity", _cashflow -> _cashflow.getPriceQuantity()).<ReferenceWithMetaNonNegativeQuantitySchedule>map("getQuantitySchedule", resolvablePriceQuantity -> resolvablePriceQuantity.getQuantitySchedule()).<NonNegativeQuantitySchedule>map("getValue", _f->_f.getValue()).<BigDecimal>map("getValue", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getValue()), MapperS.of(new BigDecimal("0.0")), CardinalityOperator.All);
				}
