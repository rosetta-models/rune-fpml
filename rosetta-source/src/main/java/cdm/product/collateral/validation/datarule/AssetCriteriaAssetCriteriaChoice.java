...
public interface AssetCriteriaAssetCriteriaChoice extends Validator<AssetCriteria> {
	...
	String DEFINITION = "optional choice collateralAssetType, collateralTaxonomy, assetIdentifier";
	...
	class Default implements AssetCriteriaAssetCriteriaChoice {
		...
		private ComparisonResult executeDataRule(AssetCriteria assetCriteria) {
			try {
				return choice(MapperS.of(assetCriteria), Arrays.asList("collateralAssetType", "collateralTaxonomy", "assetIdentifier"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
