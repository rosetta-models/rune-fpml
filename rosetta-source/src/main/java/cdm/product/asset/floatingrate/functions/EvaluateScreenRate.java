	...
	*/
	...
	public static class EvaluateScreenRateDefault extends EvaluateScreenRate {
		...
		protected MapperS<BigDecimal> observedRate(FloatingRate rateDef, ResetDates resetDates, CalculationPeriodBase calculationPeriod) {
			return MapperS.of(indexValueObservation.evaluate(fixingDate(rateDef, resetDates, calculationPeriod).get(), MapperS.of(rateDef).<ReferenceWithMetaFloatingRateIndex>map("getRateOption", floatingRate -> floatingRate.getRateOption()).<FloatingRateIndex>map("getValue", _f->_f.getValue()).get()));
		}
	}
}
