...
public interface BondReferenceBondUnderlier extends Validator<BondReference> {
	...
	String DEFINITION = "bond -> securityType = SecurityTypeEnum -> Debt";
	...
	class Default implements BondReferenceBondUnderlier {
		...
		private ComparisonResult executeDataRule(BondReference bondReference) {
			try {
				return areEqual(MapperS.of(bondReference).<Security>map("getBond", _bondReference -> _bondReference.getBond()).<SecurityTypeEnum>map("getSecurityType", security -> security.getSecurityType()), MapperS.of(SecurityTypeEnum.DEBT), CardinalityOperator.All);
			}
			catch (Exception ex) {
