...
public interface ReferenceObligationMustBeDebtSecurity extends Validator<ReferenceObligation> {
	...
	String DEFINITION = "if security exists then security -> securityType = SecurityTypeEnum -> Debt";
		...
		private ComparisonResult executeDataRule(ReferenceObligation referenceObligation) {
			try {
				if (exists(MapperS.of(referenceObligation).<Security>map("getSecurity", _referenceObligation -> _referenceObligation.getSecurity())).getOrDefault(false)) {
					return areEqual(MapperS.of(referenceObligation).<Security>map("getSecurity", _referenceObligation -> _referenceObligation.getSecurity()).<SecurityTypeEnum>map("getSecurityType", security -> security.getSecurityType()), MapperS.of(SecurityTypeEnum.DEBT), CardinalityOperator.All);
				}
