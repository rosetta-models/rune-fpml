...
public interface PerformancePayoutEquitySpecificAttributes extends Validator<PerformancePayout> {
	...
	class Default implements PerformancePayoutEquitySpecificAttributes {
		...
		private ComparisonResult executeDataRule(PerformancePayout performancePayout) {
			try {
				if (areEqual(MapperS.of(qualify_UnderlierObservable_Equity.evaluate(MapperS.of(performancePayout).<Underlier>map("getUnderlier", _performancePayout -> _performancePayout.getUnderlier()).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("getValue", _f->_f.getValue()).get())), MapperS.of(false), CardinalityOperator.All).getOrDefault(false)) {
