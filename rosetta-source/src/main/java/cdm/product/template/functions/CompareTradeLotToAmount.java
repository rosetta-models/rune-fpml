	...
	*/
	...
	public static class CompareTradeLotToAmountDefault extends CompareTradeLotToAmount {
		...
		protected Boolean assignOutput(Boolean result, TradeLot tradeLot, CompareOp op, BigDecimal amount) {
			final MapperC<NonNegativeQuantitySchedule> thenArg = MapperS.of(tradeLot).<PriceQuantity>mapC("getPriceQuantity", _tradeLot -> _tradeLot.getPriceQuantity()).<FieldWithMetaNonNegativeQuantitySchedule>mapC("getQuantity", priceQuantity -> priceQuantity.getQuantity()).<NonNegativeQuantitySchedule>map("getValue", _f->_f.getValue())
				.filterItemNullSafe(item -> exists(item.<BigDecimal>map("getValue", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getValue())).get());
			result = areEqual(thenArg
				.mapItem(item -> MapperS.of(compareNumbers.evaluate(item.<BigDecimal>map("getValue", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getValue()).get(), op, amount))), MapperS.of(true), CardinalityOperator.All).asMapper().get();
