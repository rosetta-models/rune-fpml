	...
	*/
	...
	public static class FxMarkToMarketDefault extends FxMarkToMarket {
		...
		protected MapperS<? extends SettlementPayout> settlementPayout(Trade trade) {
			return MapperS.of(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).<EconomicTerms>map("getEconomicTerms", nonTransferableProduct -> nonTransferableProduct.getEconomicTerms()).<Payout>map("getPayout", economicTerms -> economicTerms.getPayout()).<SettlementPayout>mapC("getSettlementPayout", payout -> payout.getSettlementPayout()).get());
		}
		
		@Override
		protected MapperS<String> quotedCurrency(Trade trade) {
			return MapperS.of(distinct(MapperS.of(trade).<TradeLot>mapC("getTradeLot", _trade -> _trade.getTradeLot()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity()).<FieldWithMetaPriceSchedule>mapC("getPrice", priceQuantity -> priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()).<UnitType>map("getUnit", priceSchedule -> priceSchedule.getUnit()).<FieldWithMetaString>map("getCurrency", unitType -> unitType.getCurrency()).<String>map("getValue", _f->_f.getValue())).get());
		}
		...
		protected MapperC<? extends NonNegativeQuantitySchedule> quantities(Trade trade) {
			return MapperS.of(MapperS.of(trade).<TradeLot>mapC("getTradeLot", _trade -> _trade.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity()).<FieldWithMetaNonNegativeQuantitySchedule>mapC("getQuantity", priceQuantity -> priceQuantity.getQuantity()).<NonNegativeQuantitySchedule>map("getValue", _f->_f.getValue());
		}
		
		@Override
		protected MapperS<BigDecimal> quotedQuantity(Trade trade) {
			return MapperS.of(MapperC.of(filterQuantityByCurrency.evaluate(quantities(trade).getMulti(), quotedCurrency(trade).get())).get()).<BigDecimal>map("getValue", quantitySchedule -> quantitySchedule.getValue());
		}
		
		@Override
		protected MapperS<BigDecimal> baseQuantity(Trade trade) {
			return MapperS.of(MapperC.of(filterQuantityByCurrency.evaluate(quantities(trade).getMulti(), baseCurrency(trade).get())).get()).<BigDecimal>map("getValue", quantitySchedule -> quantitySchedule.getValue());
		}
