	...
	*/
	...
	public static class Qualify_SecuritySettlementDefault extends Qualify_SecuritySettlement {
		...
		protected Boolean assignOutput(Boolean is_event, BusinessEvent businessEvent) {
			is_event = exists(transfers(businessEvent).<Asset>map("getAsset", transfer -> transfer.getAsset()).<Instrument>map("getInstrument", asset -> asset.getInstrument()).<Security>map("getSecurity", instrument -> instrument.getSecurity())).and(onlyExists(transfers(businessEvent).<NonNegativeQuantity>map("getQuantity", transfer -> transfer.getQuantity()).<UnitType>map("getUnit", nonNegativeQuantity -> nonNegativeQuantity.getUnit()), Arrays.asList("capacityUnit", "weatherUnit", "financialUnit", "currency"), Arrays.asList("currency"))).and(areEqual(MapperS.of(MapperC.of(filterCashTransfers.evaluate(transfers(businessEvent).getMulti())).get()).<PartyReferencePayerReceiver>map("getPayerReceiver", transfer -> transfer.getPayerReceiver()).<ReferenceWithMetaParty>map("getPayerPartyReference", partyReferencePayerReceiver -> partyReferencePayerReceiver.getPayerPartyReference()).<Party>map("getValue", _f->_f.getValue()), MapperS.of(MapperC.of(filterSecurityTransfers.evaluate(transfers(businessEvent).getMulti())).get()).<PartyReferencePayerReceiver>map("getPayerReceiver", transfer -> transfer.getPayerReceiver()).<ReferenceWithMetaParty>map("getReceiverPartyReference", partyReferencePayerReceiver -> partyReferencePayerReceiver.getReceiverPartyReference()).<Party>map("getValue", _f->_f.getValue()), CardinalityOperator.All)).get();
