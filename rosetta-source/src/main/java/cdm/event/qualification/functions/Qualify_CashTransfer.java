	...
	*/
	...
	public static class Qualify_CashTransferDefault extends Qualify_CashTransfer {
		...
		protected Boolean assignOutput(Boolean is_event, BusinessEvent businessEvent) {
			is_event = onlyExists(MapperS.of(businessEvent).<Instruction>mapC("getInstruction", _businessEvent -> _businessEvent.getInstruction()).<PrimitiveInstruction>map("getPrimitiveInstruction", instruction -> instruction.getPrimitiveInstruction()), Arrays.asList("contractFormation", "execution", "exercise", "partyChange", "quantityChange", "reset", "split", "termsChange", "transfer", "indexTransition", "stockSplit", "observation", "valuation"), Arrays.asList("transfer")).and(areEqual(MapperMaths.<Integer, Integer, Integer>add(MapperS.of(beforeTransfers(businessEvent).resultCount()), MapperS.of(transferInstructions(businessEvent).resultCount())), MapperS.of(afterTransfers(businessEvent).resultCount()), CardinalityOperator.All)).and(exists(transferInstructions(businessEvent).<TransferState>mapC("getTransferState", transferInstruction -> transferInstruction.getTransferState()).<Transfer>map("getTransfer", transferState -> transferState.getTransfer()).<NonNegativeQuantity>map("getQuantity", transfer -> transfer.getQuantity()).<UnitType>map("getUnit", nonNegativeQuantity -> nonNegativeQuantity.getUnit()).<FieldWithMetaString>map("getCurrency", unitType -> unitType.getCurrency()).<String>map("getValue", _f->_f.getValue()))).get();
