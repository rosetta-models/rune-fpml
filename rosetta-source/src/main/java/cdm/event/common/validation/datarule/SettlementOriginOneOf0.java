...
public interface SettlementOriginOneOf0 extends Validator<SettlementOrigin> {
	...
	class Default implements SettlementOriginOneOf0 {
		...
		private ComparisonResult executeDataRule(SettlementOrigin settlementOrigin) {
			try {
				return choice(MapperS.of(settlementOrigin), Arrays.asList("commodityPayout", "creditDefaultPayout", "settlementPayout", "interestRatePayout", "optionPayout", "assetPayout", "settlementTerms", "performancePayout", "fixedPricePayout"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
