...
public interface TradeSettlementPayout extends Validator<Trade> {
	...
	String DEFINITION = "if product -> economicTerms -> payout -> settlementPayout only exists then partyRole -> role contains PartyRoleEnum -> Buyer and partyRole -> role contains PartyRoleEnum -> Seller and tradeLot -> priceQuantity -> price exists";
	...
	class Default implements TradeSettlementPayout {
		...
		private ComparisonResult executeDataRule(Trade trade) {
			try {
				if (onlyExists(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).<EconomicTerms>map("getEconomicTerms", nonTransferableProduct -> nonTransferableProduct.getEconomicTerms()).<Payout>map("getPayout", economicTerms -> economicTerms.getPayout()), Arrays.asList("interestRatePayout", "creditDefaultPayout", "optionPayout", "commodityPayout", "settlementPayout", "fixedPricePayout", "cashflow", "performancePayout", "assetPayout"), Arrays.asList("settlementPayout")).getOrDefault(false)) {
