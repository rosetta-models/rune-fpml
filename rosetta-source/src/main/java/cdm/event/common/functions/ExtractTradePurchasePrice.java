	...
	*/
	...
	public static class ExtractTradePurchasePriceDefault extends ExtractTradePurchasePrice {
		...
		protected List<BigDecimal> assignOutput(List<BigDecimal> value, TradableProduct tradableProduct) {
			final MapperListOfLists<NonNegativeQuantitySchedule> thenArg0 = MapperS.of(MapperS.of(tradableProduct).<TradeLot>mapC("getTradeLot", _tradableProduct -> _tradableProduct.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity())
			...
			final MapperC<NonNegativeQuantitySchedule> thenArg1 = thenArg0
				.flattenList();
			value = thenArg1
				.mapItem(item -> item.<BigDecimal>map("getValue", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getValue())).getMulti();
