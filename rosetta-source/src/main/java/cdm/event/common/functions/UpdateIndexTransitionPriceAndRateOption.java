	...
	public static class UpdateIndexTransitionPriceAndRateOptionDefault extends UpdateIndexTransitionPriceAndRateOption {
		...
		protected PriceQuantity.PriceQuantityBuilder assignOutput(PriceQuantity.PriceQuantityBuilder updatedPriceQuantity, PriceQuantity priceQuantity, PriceQuantity instruction) {
			...
			if (exists(MapperS.of(instruction)).getOrDefault(false)) {
				ifThenElseResult0 = MapperMaths.<BigDecimal, BigDecimal, BigDecimal>add(MapperS.of(MapperS.of(priceQuantity).<FieldWithMetaPriceSchedule>mapC("getPrice", _priceQuantity -> _priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()).get()).<BigDecimal>map("getValue", priceSchedule -> priceSchedule.getValue()), MapperS.of(MapperS.of(instruction).<FieldWithMetaPriceSchedule>mapC("getPrice", _priceQuantity -> _priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()).get()).<BigDecimal>map("getValue", priceSchedule -> priceSchedule.getValue())).get();
			} else {
				ifThenElseResult0 = MapperS.of(MapperS.of(priceQuantity).<FieldWithMetaPriceSchedule>mapC("getPrice", _priceQuantity -> _priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()).get()).<BigDecimal>map("getValue", priceSchedule -> priceSchedule.getValue()).get();
			}
			updatedPriceQuantity
				.getOrCreatePrice(0).getOrCreateValue()
				.setValue(ifThenElseResult0);
			
			final FloatingRateIndex ifThenElseResult1;
			if (exists(MapperS.of(instruction)).getOrDefault(false)) {
				ifThenElseResult1 = MapperS.of(instruction).<FieldWithMetaObservable>map("getObservable", _priceQuantity -> _priceQuantity.getObservable()).<Observable>map("getValue", _f->_f.getValue()).<Index>map("getIndex", observable -> observable.getIndex()).<FieldWithMetaFloatingRateIndex>map("getFloatingRateIndex", index -> index.getFloatingRateIndex()).<FloatingRateIndex>map("getValue", _f->_f.getValue()).get();
			} else {
			...
			updatedPriceQuantity
				...
				.getOrCreateIndex()
				.setFloatingRateIndexValue(ifThenElseResult1);
