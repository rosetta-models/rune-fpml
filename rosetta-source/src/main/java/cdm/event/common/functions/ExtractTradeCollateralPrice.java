	...
	*/
	...
	public static class ExtractTradeCollateralPriceDefault extends ExtractTradeCollateralPrice {
		...
		protected List<BigDecimal> assignOutput(List<BigDecimal> value, TradableProduct tradableProduct) {
			final MapperListOfLists<PriceSchedule> thenArg0 = MapperS.of(MapperS.of(tradableProduct).<TradeLot>mapC("getTradeLot", _tradableProduct -> _tradableProduct.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity())
			...
			value = thenArg2
				.mapItem(item -> item.<BigDecimal>map("getValue", priceSchedule -> priceSchedule.getValue())).getMulti();
