	...
	*/
	...
	public static class Qualify_OnDemandRateChangeDefault extends Qualify_OnDemandRateChange {
		...
		protected MapperS<? extends Trade> beforeTrade(BusinessEvent businessEvent) {
			return MapperS.of(MapperS.of(businessEvent).<Instruction>mapC("getInstruction", _businessEvent -> _businessEvent.getInstruction()).get()).<ReferenceWithMetaTradeState>map("getBefore", instruction -> instruction.getBefore()).<TradeState>map("getValue", _f->_f.getValue()).<Trade>map("getTrade", tradeState -> tradeState.getTrade());
		}
		...
		protected MapperC<BigDecimal> beforePriceQuantityRateOnly(BusinessEvent businessEvent) {
			final MapperListOfLists<PriceSchedule> thenArg0 = MapperS.of(beforeTrade(businessEvent).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity())
		...
		protected MapperC<BigDecimal> openPriceQuantityRateOnly(BusinessEvent businessEvent) {
			final MapperListOfLists<PriceSchedule> thenArg0 = MapperS.of(openTrade(businessEvent).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity())
				.mapItemToList(item -> item.<FieldWithMetaPriceSchedule>mapC("getPrice", priceQuantity -> priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()));
			final MapperC<PriceSchedule> thenArg1 = thenArg0
				.flattenList();
			final MapperC<PriceSchedule> thenArg2 = thenArg1
		...
		protected MapperC<? extends PriceQuantity> beforePriceQuantityNoRate(BusinessEvent businessEvent) {
			return MapperS.of(beforeTrade(businessEvent).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity())
				.mapItem(item -> MapperS.of(PriceQuantity.builder()
					...
					.setObservableValue(item.<FieldWithMetaObservable>map("getObservable", priceQuantity -> priceQuantity.getObservable()).<Observable>map("getValue", _f->_f.getValue()).get())
					.build()));
		}
