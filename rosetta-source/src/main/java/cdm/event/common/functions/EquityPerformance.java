	...
	*/
	...
	public static class EquityPerformanceDefault extends EquityPerformance {
		...
		protected MapperS<? extends PerformancePayout> performancePayout(Trade trade, Price observation, Date date) {
			return MapperS.of(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).<EconomicTerms>map("getEconomicTerms", nonTransferableProduct -> nonTransferableProduct.getEconomicTerms()).<Payout>map("getPayout", economicTerms -> economicTerms.getPayout()).<PerformancePayout>mapC("getPerformancePayout", payout -> payout.getPerformancePayout()).get());
		}
		
		@Override
		protected MapperS<? extends PriceSchedule> periodStartPrice(Trade trade, Price observation, Date date) {
			return MapperS.of(resolvePerformancePeriodStartPrice.evaluate(performancePayout(trade, observation, date).get(), MapperS.of(MapperS.of(trade).<TradeLot>mapC("getTradeLot", _trade -> _trade.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity()).<FieldWithMetaPriceSchedule>mapC("getPrice", priceQuantity -> priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()).getMulti(), MapperS.of(trade).<TradeLot>mapC("getTradeLot", _trade -> _trade.getTradeLot()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity()).<FieldWithMetaObservable>map("getObservable", priceQuantity -> priceQuantity.getObservable()).<Observable>map("getValue", _f->_f.getValue()).get(), date));
		}
		...
		protected MapperS<BigDecimal> numberOfSecurities(Trade trade, Price observation, Date date) {
			return MapperMaths.<BigDecimal, BigDecimal, BigDecimal>divide(performancePayout(trade, observation, date).<ResolvablePriceQuantity>map("getPriceQuantity", _performancePayout -> _performancePayout.getPriceQuantity()).<ReferenceWithMetaNonNegativeQuantitySchedule>map("getQuantitySchedule", resolvablePriceQuantity -> resolvablePriceQuantity.getQuantitySchedule()).<NonNegativeQuantitySchedule>map("getValue", _f->_f.getValue()).<BigDecimal>map("getValue", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getValue()), periodStartPrice(trade, observation, date).<BigDecimal>map("getValue", priceSchedule -> priceSchedule.getValue()));
		}
