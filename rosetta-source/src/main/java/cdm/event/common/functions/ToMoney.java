	...
	*/
	...
	public static class ToMoneyDefault extends ToMoney {
		...
		protected Money.MoneyBuilder assignOutput(Money.MoneyBuilder money, Quantity quantity) {
			...
			money
				.getOrCreateUnit()
				.setCurrencyValue(MapperS.of(quantity).<UnitType>map("getUnit", _quantity -> _quantity.getUnit()).<FieldWithMetaString>map("getCurrency", unitType -> unitType.getCurrency()).<String>map("getValue", _f->_f.getValue()).get());
