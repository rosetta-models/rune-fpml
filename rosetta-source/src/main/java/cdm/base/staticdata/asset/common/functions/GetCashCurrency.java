	...
	*/
	...
	public static class GetCashCurrencyDefault extends GetCashCurrency {
		...
		protected CurrencyCodeEnum assignOutput(CurrencyCodeEnum currencyEnum, Cash cash) {
			currencyEnum = cashId(cash).<FieldWithMetaString>map("getIdentifier", assetIdentifier -> assetIdentifier.getIdentifier()).<String>map("getValue", _f->_f.getValue())
				.first().checkedMap("to-enum", CurrencyCodeEnum::fromDisplayName, IllegalArgumentException.class).get();
