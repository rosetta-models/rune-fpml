...
public interface MoneyCurrencyUnitExists extends Validator<Money> {
	...
	class Default implements MoneyCurrencyUnitExists {
		...
		private ComparisonResult executeDataRule(Money money) {
			try {
				return exists(MapperS.of(money).<UnitType>map("getUnit", _money -> _money.getUnit()).<FieldWithMetaString>map("getCurrency", unitType -> unitType.getCurrency()).<String>map("getValue", _f->_f.getValue()));
			}
			catch (Exception ex) {
