...
public interface IndexChoice extends Validator<Index> {
	...
	class Default implements IndexChoice {
		...
		private ComparisonResult executeDataRule(Index index) {
			try {
				return choice(MapperS.of(index), Arrays.asList("CreditIndex", "EquityIndex", "FloatingRateIndex", "ForeignExchangeRateIndex", "OtherIndex"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
