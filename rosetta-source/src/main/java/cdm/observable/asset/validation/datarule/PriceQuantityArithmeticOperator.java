...
public interface PriceQuantityArithmeticOperator extends Validator<PriceQuantity> {
	...
	String DEFINITION = "if observable -> Index -> FloatingRateIndex exists and price exists then price -> arithmeticOperator exists";
	...
	class Default implements PriceQuantityArithmeticOperator {
		...
		private ComparisonResult executeDataRule(PriceQuantity priceQuantity) {
			try {
				if (exists(MapperS.of(priceQuantity).<FieldWithMetaObservable>map("getObservable", _priceQuantity -> _priceQuantity.getObservable()).<Observable>map("getValue", _f->_f.getValue()).<Index>map("getIndex", observable -> observable.getIndex()).<FieldWithMetaFloatingRateIndex>map("getFloatingRateIndex", index -> index.getFloatingRateIndex()).<FloatingRateIndex>map("getValue", _f->_f.getValue())).and(exists(MapperS.of(priceQuantity).<FieldWithMetaPriceSchedule>mapC("getPrice", _priceQuantity -> _priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()))).getOrDefault(false)) {
					return exists(MapperS.of(priceQuantity).<FieldWithMetaPriceSchedule>mapC("getPrice", _priceQuantity -> _priceQuantity.getPrice()).<PriceSchedule>map("getValue", _f->_f.getValue()).<ArithmeticOperationEnum>map("getArithmeticOperator", priceSchedule -> priceSchedule.getArithmeticOperator()));
				}
