	...
	public static class StandardizedScheduleVarianceSwapNotionalAmountDefault extends StandardizedScheduleVarianceSwapNotionalAmount {
		...
		protected MapperS<BigDecimal> varianceAmount(PerformancePayout performancePayout) {
			...
			if (exists(thenArg.<Measure>map("getMultiplier", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getMultiplier())).getOrDefault(false)) {
				...
				ifThenElseResult0 = thenArg.<BigDecimal>map("getValue", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getValue());
			}
