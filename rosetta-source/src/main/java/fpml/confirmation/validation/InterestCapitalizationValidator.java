...
public class InterestCapitalizationValidator implements Validator<InterestCapitalization> {
	...
	private List<ComparisonResult> getComparisonResults(InterestCapitalization o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("interestCapitalizationSequence", (InterestCapitalizationSequence) o.getInterestCapitalizationSequence() != null ? 1 : 0, 0, 1)
			);
	}

	@Override
	public ValidationResult<InterestCapitalization> validate(RosettaPath path, InterestCapitalization o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InterestCapitalization", ValidationType.CARDINALITY, "InterestCapitalization", path, "", error);
		}
		return success("InterestCapitalization", ValidationType.CARDINALITY, "InterestCapitalization", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestCapitalization", ValidationType.CARDINALITY, "InterestCapitalization", path, "", res.getError());
				}
				return success("InterestCapitalization", ValidationType.CARDINALITY, "InterestCapitalization", path, "");
			})
			.collect(toList());
