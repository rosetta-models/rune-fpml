...
public class SettlementPostponementValidator implements Validator<SettlementPostponement> {
	...
	public ValidationResult<SettlementPostponement> validate(RosettaPath path, SettlementPostponement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SettlementPostponement", ValidationType.CARDINALITY, "SettlementPostponement", path, "", error);
		}
		return success("SettlementPostponement", ValidationType.CARDINALITY, "SettlementPostponement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPostponement", ValidationType.CARDINALITY, "SettlementPostponement", path, "", res.getError());
				}
				return success("SettlementPostponement", ValidationType.CARDINALITY, "SettlementPostponement", path, "");
			})
			.collect(toList());
