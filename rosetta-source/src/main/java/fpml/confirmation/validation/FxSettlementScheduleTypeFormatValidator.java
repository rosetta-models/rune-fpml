...
public class FxSettlementScheduleTypeFormatValidator implements Validator<FxSettlementSchedule> {
	...
	public ValidationResult<FxSettlementSchedule> validate(RosettaPath path, FxSettlementSchedule o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxSettlementSchedule", ValidationType.TYPE_FORMAT, "FxSettlementSchedule", path, "", error);
		}
		return success("FxSettlementSchedule", ValidationType.TYPE_FORMAT, "FxSettlementSchedule", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSettlementSchedule", ValidationType.TYPE_FORMAT, "FxSettlementSchedule", path, "", res.getError());
				}
				return success("FxSettlementSchedule", ValidationType.TYPE_FORMAT, "FxSettlementSchedule", path, "");
			})
			.collect(toList());
