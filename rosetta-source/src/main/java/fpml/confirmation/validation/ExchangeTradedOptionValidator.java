...
public class ExchangeTradedOptionValidator implements Validator<ExchangeTradedOption> {
	...
	public ValidationResult<ExchangeTradedOption> validate(RosettaPath path, ExchangeTradedOption o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExchangeTradedOption", ValidationType.CARDINALITY, "ExchangeTradedOption", path, "", error);
		}
		return success("ExchangeTradedOption", ValidationType.CARDINALITY, "ExchangeTradedOption", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTradedOption", ValidationType.CARDINALITY, "ExchangeTradedOption", path, "", res.getError());
				}
				return success("ExchangeTradedOption", ValidationType.CARDINALITY, "ExchangeTradedOption", path, "");
			})
			.collect(toList());
