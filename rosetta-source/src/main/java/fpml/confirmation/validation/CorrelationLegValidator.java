...
public class CorrelationLegValidator implements Validator<CorrelationLeg> {
	...
	public ValidationResult<CorrelationLeg> validate(RosettaPath path, CorrelationLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CorrelationLeg", ValidationType.CARDINALITY, "CorrelationLeg", path, "", error);
		}
		return success("CorrelationLeg", ValidationType.CARDINALITY, "CorrelationLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrelationLeg", ValidationType.CARDINALITY, "CorrelationLeg", path, "", res.getError());
				}
				return success("CorrelationLeg", ValidationType.CARDINALITY, "CorrelationLeg", path, "");
			})
			.collect(toList());
