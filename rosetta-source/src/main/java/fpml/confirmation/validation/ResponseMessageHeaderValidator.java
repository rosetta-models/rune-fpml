...
public class ResponseMessageHeaderValidator implements Validator<ResponseMessageHeader> {
	...
	private List<ComparisonResult> getComparisonResults(ResponseMessageHeader o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("inReplyTo", (MessageId) o.getInReplyTo() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<ResponseMessageHeader> validate(RosettaPath path, ResponseMessageHeader o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ResponseMessageHeader", ValidationType.CARDINALITY, "ResponseMessageHeader", path, "", error);
		}
		return success("ResponseMessageHeader", ValidationType.CARDINALITY, "ResponseMessageHeader", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResponseMessageHeader", ValidationType.CARDINALITY, "ResponseMessageHeader", path, "", res.getError());
				}
				return success("ResponseMessageHeader", ValidationType.CARDINALITY, "ResponseMessageHeader", path, "");
			})
			.collect(toList());
