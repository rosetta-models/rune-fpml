...
public class AbstractLcEventValidator implements Validator<AbstractLcEvent> {
	...
	public ValidationResult<AbstractLcEvent> validate(RosettaPath path, AbstractLcEvent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AbstractLcEvent", ValidationType.CARDINALITY, "AbstractLcEvent", path, "", error);
		}
		return success("AbstractLcEvent", ValidationType.CARDINALITY, "AbstractLcEvent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLcEvent", ValidationType.CARDINALITY, "AbstractLcEvent", path, "", res.getError());
				}
				return success("AbstractLcEvent", ValidationType.CARDINALITY, "AbstractLcEvent", path, "");
			})
			.collect(toList());
