...
public class FxFlexibleForwardExecutionPeriodTypeFormatValidator implements Validator<FxFlexibleForwardExecutionPeriod> {
	...
	public ValidationResult<FxFlexibleForwardExecutionPeriod> validate(RosettaPath path, FxFlexibleForwardExecutionPeriod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxFlexibleForwardExecutionPeriod", ValidationType.TYPE_FORMAT, "FxFlexibleForwardExecutionPeriod", path, "", error);
		}
		return success("FxFlexibleForwardExecutionPeriod", ValidationType.TYPE_FORMAT, "FxFlexibleForwardExecutionPeriod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFlexibleForwardExecutionPeriod", ValidationType.TYPE_FORMAT, "FxFlexibleForwardExecutionPeriod", path, "", res.getError());
				}
				return success("FxFlexibleForwardExecutionPeriod", ValidationType.TYPE_FORMAT, "FxFlexibleForwardExecutionPeriod", path, "");
			})
			.collect(toList());
