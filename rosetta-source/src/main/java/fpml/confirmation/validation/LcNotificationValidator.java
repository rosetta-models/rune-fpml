...
public class LcNotificationValidator implements Validator<LcNotification> {
	...
	private List<ComparisonResult> getComparisonResults(LcNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("letterOfCreditDetailsModel", (LetterOfCreditDetailsModel) o.getLetterOfCreditDetailsModel() != null ? 1 : 0, 1, 1), 
				checkCardinality("party", (List<? extends Party>) o.getParty() == null ? 0 : ((List<? extends Party>) o.getParty()).size(), 1, 0)
			);
	}

	@Override
	public ValidationResult<LcNotification> validate(RosettaPath path, LcNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcNotification", ValidationType.CARDINALITY, "LcNotification", path, "", error);
		}
		return success("LcNotification", ValidationType.CARDINALITY, "LcNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcNotification", ValidationType.CARDINALITY, "LcNotification", path, "", res.getError());
				}
				return success("LcNotification", ValidationType.CARDINALITY, "LcNotification", path, "");
			})
			.collect(toList());
