...
public class BasketChangeTypeFormatValidator implements Validator<BasketChange> {
	...
	public ValidationResult<BasketChange> validate(RosettaPath path, BasketChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BasketChange", ValidationType.TYPE_FORMAT, "BasketChange", path, "", error);
		}
		return success("BasketChange", ValidationType.TYPE_FORMAT, "BasketChange", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasketChange", ValidationType.TYPE_FORMAT, "BasketChange", path, "", res.getError());
				}
				return success("BasketChange", ValidationType.TYPE_FORMAT, "BasketChange", path, "");
			})
			.collect(toList());
