...
public class VerificationMethodTypeFormatValidator implements Validator<VerificationMethod> {
	...
	public ValidationResult<VerificationMethod> validate(RosettaPath path, VerificationMethod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("VerificationMethod", ValidationType.TYPE_FORMAT, "VerificationMethod", path, "", error);
		}
		return success("VerificationMethod", ValidationType.TYPE_FORMAT, "VerificationMethod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VerificationMethod", ValidationType.TYPE_FORMAT, "VerificationMethod", path, "", res.getError());
				}
				return success("VerificationMethod", ValidationType.TYPE_FORMAT, "VerificationMethod", path, "");
			})
			.collect(toList());
