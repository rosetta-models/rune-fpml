...
public class AcknowledgementTypeFormatValidator implements Validator<Acknowledgement> {
	...
	public ValidationResult<Acknowledgement> validate(RosettaPath path, Acknowledgement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Acknowledgement", ValidationType.TYPE_FORMAT, "Acknowledgement", path, "", error);
		}
		return success("Acknowledgement", ValidationType.TYPE_FORMAT, "Acknowledgement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Acknowledgement", ValidationType.TYPE_FORMAT, "Acknowledgement", path, "", res.getError());
				}
				return success("Acknowledgement", ValidationType.TYPE_FORMAT, "Acknowledgement", path, "");
			})
			.collect(toList());
