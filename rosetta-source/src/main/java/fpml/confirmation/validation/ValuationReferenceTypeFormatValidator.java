...
public class ValuationReferenceTypeFormatValidator implements Validator<ValuationReference> {
	...
	public ValidationResult<ValuationReference> validate(RosettaPath path, ValuationReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ValuationReference", ValidationType.TYPE_FORMAT, "ValuationReference", path, "", error);
		}
		return success("ValuationReference", ValidationType.TYPE_FORMAT, "ValuationReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationReference", ValidationType.TYPE_FORMAT, "ValuationReference", path, "", res.getError());
				}
				return success("ValuationReference", ValidationType.TYPE_FORMAT, "ValuationReference", path, "");
			})
			.collect(toList());
