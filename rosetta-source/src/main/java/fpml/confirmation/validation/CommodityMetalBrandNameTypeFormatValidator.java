...
public class CommodityMetalBrandNameTypeFormatValidator implements Validator<CommodityMetalBrandName> {
	...
	public ValidationResult<CommodityMetalBrandName> validate(RosettaPath path, CommodityMetalBrandName o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityMetalBrandName", ValidationType.TYPE_FORMAT, "CommodityMetalBrandName", path, "", error);
		}
		return success("CommodityMetalBrandName", ValidationType.TYPE_FORMAT, "CommodityMetalBrandName", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityMetalBrandName", ValidationType.TYPE_FORMAT, "CommodityMetalBrandName", path, "", res.getError());
				}
				return success("CommodityMetalBrandName", ValidationType.TYPE_FORMAT, "CommodityMetalBrandName", path, "");
			})
			.collect(toList());
