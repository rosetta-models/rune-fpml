...
public class LoanBulkServicingNotificationTypeFormatValidator implements Validator<LoanBulkServicingNotification> {
	...
	public ValidationResult<LoanBulkServicingNotification> validate(RosettaPath path, LoanBulkServicingNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanBulkServicingNotification", ValidationType.TYPE_FORMAT, "LoanBulkServicingNotification", path, "", error);
		}
		return success("LoanBulkServicingNotification", ValidationType.TYPE_FORMAT, "LoanBulkServicingNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanBulkServicingNotification", ValidationType.TYPE_FORMAT, "LoanBulkServicingNotification", path, "", res.getError());
				}
				return success("LoanBulkServicingNotification", ValidationType.TYPE_FORMAT, "LoanBulkServicingNotification", path, "");
			})
			.collect(toList());
