...
public class NotionalAmountTypeFormatValidator implements Validator<NotionalAmount> {
	...
	public ValidationResult<NotionalAmount> validate(RosettaPath path, NotionalAmount o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("NotionalAmount", ValidationType.TYPE_FORMAT, "NotionalAmount", path, "", error);
		}
		return success("NotionalAmount", ValidationType.TYPE_FORMAT, "NotionalAmount", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotionalAmount", ValidationType.TYPE_FORMAT, "NotionalAmount", path, "", res.getError());
				}
				return success("NotionalAmount", ValidationType.TYPE_FORMAT, "NotionalAmount", path, "");
			})
			.collect(toList());
