...
public class FacilityEventGroupValidator implements Validator<FacilityEventGroup> {
	...
	public ValidationResult<FacilityEventGroup> validate(RosettaPath path, FacilityEventGroup o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityEventGroup", ValidationType.CARDINALITY, "FacilityEventGroup", path, "", error);
		}
		return success("FacilityEventGroup", ValidationType.CARDINALITY, "FacilityEventGroup", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityEventGroup", ValidationType.CARDINALITY, "FacilityEventGroup", path, "", res.getError());
				}
				return success("FacilityEventGroup", ValidationType.CARDINALITY, "FacilityEventGroup", path, "");
			})
			.collect(toList());
