...
public class PricingInputTypeTypeFormatValidator implements Validator<PricingInputType> {
	...
	public ValidationResult<PricingInputType> validate(RosettaPath path, PricingInputType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PricingInputType", ValidationType.TYPE_FORMAT, "PricingInputType", path, "", error);
		}
		return success("PricingInputType", ValidationType.TYPE_FORMAT, "PricingInputType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingInputType", ValidationType.TYPE_FORMAT, "PricingInputType", path, "", res.getError());
				}
				return success("PricingInputType", ValidationType.TYPE_FORMAT, "PricingInputType", path, "");
			})
			.collect(toList());
