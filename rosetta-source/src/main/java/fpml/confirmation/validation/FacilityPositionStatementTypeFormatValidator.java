...
public class FacilityPositionStatementTypeFormatValidator implements Validator<FacilityPositionStatement> {
	...
	public ValidationResult<FacilityPositionStatement> validate(RosettaPath path, FacilityPositionStatement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityPositionStatement", ValidationType.TYPE_FORMAT, "FacilityPositionStatement", path, "", error);
		}
		return success("FacilityPositionStatement", ValidationType.TYPE_FORMAT, "FacilityPositionStatement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityPositionStatement", ValidationType.TYPE_FORMAT, "FacilityPositionStatement", path, "", res.getError());
				}
				return success("FacilityPositionStatement", ValidationType.TYPE_FORMAT, "FacilityPositionStatement", path, "");
			})
			.collect(toList());
