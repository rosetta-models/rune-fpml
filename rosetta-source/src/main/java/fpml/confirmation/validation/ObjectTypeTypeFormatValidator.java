...
public class ObjectTypeTypeFormatValidator implements Validator<ObjectType> {
	...
	public ValidationResult<ObjectType> validate(RosettaPath path, ObjectType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ObjectType", ValidationType.TYPE_FORMAT, "ObjectType", path, "", error);
		}
		return success("ObjectType", ValidationType.TYPE_FORMAT, "ObjectType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObjectType", ValidationType.TYPE_FORMAT, "ObjectType", path, "", res.getError());
				}
				return success("ObjectType", ValidationType.TYPE_FORMAT, "ObjectType", path, "");
			})
			.collect(toList());
