...
public class AccruingFeeExpiryValidator implements Validator<AccruingFeeExpiry> {
	...
	public ValidationResult<AccruingFeeExpiry> validate(RosettaPath path, AccruingFeeExpiry o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AccruingFeeExpiry", ValidationType.CARDINALITY, "AccruingFeeExpiry", path, "", error);
		}
		return success("AccruingFeeExpiry", ValidationType.CARDINALITY, "AccruingFeeExpiry", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingFeeExpiry", ValidationType.CARDINALITY, "AccruingFeeExpiry", path, "", res.getError());
				}
				return success("AccruingFeeExpiry", ValidationType.CARDINALITY, "AccruingFeeExpiry", path, "");
			})
			.collect(toList());
