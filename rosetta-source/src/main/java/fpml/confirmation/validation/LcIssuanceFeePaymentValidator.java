...
public class LcIssuanceFeePaymentValidator implements Validator<LcIssuanceFeePayment> {
	...
	public ValidationResult<LcIssuanceFeePayment> validate(RosettaPath path, LcIssuanceFeePayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcIssuanceFeePayment", ValidationType.CARDINALITY, "LcIssuanceFeePayment", path, "", error);
		}
		return success("LcIssuanceFeePayment", ValidationType.CARDINALITY, "LcIssuanceFeePayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcIssuanceFeePayment", ValidationType.CARDINALITY, "LcIssuanceFeePayment", path, "", res.getError());
				}
				return success("LcIssuanceFeePayment", ValidationType.CARDINALITY, "LcIssuanceFeePayment", path, "");
			})
			.collect(toList());
