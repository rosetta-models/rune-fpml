...
public class LoanTradingLetterOfCreditAccrualValidator implements Validator<LoanTradingLetterOfCreditAccrual> {
	...
	public ValidationResult<LoanTradingLetterOfCreditAccrual> validate(RosettaPath path, LoanTradingLetterOfCreditAccrual o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanTradingLetterOfCreditAccrual", ValidationType.CARDINALITY, "LoanTradingLetterOfCreditAccrual", path, "", error);
		}
		return success("LoanTradingLetterOfCreditAccrual", ValidationType.CARDINALITY, "LoanTradingLetterOfCreditAccrual", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradingLetterOfCreditAccrual", ValidationType.CARDINALITY, "LoanTradingLetterOfCreditAccrual", path, "", res.getError());
				}
				return success("LoanTradingLetterOfCreditAccrual", ValidationType.CARDINALITY, "LoanTradingLetterOfCreditAccrual", path, "");
			})
			.collect(toList());
