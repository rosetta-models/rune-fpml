...
public class OilDeliveryTypeFormatValidator implements Validator<OilDelivery> {
	...
	public ValidationResult<OilDelivery> validate(RosettaPath path, OilDelivery o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("OilDelivery", ValidationType.TYPE_FORMAT, "OilDelivery", path, "", error);
		}
		return success("OilDelivery", ValidationType.TYPE_FORMAT, "OilDelivery", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OilDelivery", ValidationType.TYPE_FORMAT, "OilDelivery", path, "", res.getError());
				}
				return success("OilDelivery", ValidationType.TYPE_FORMAT, "OilDelivery", path, "");
			})
			.collect(toList());
