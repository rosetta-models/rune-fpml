...
public class CreditLimitBaseTypeFormatValidator implements Validator<CreditLimitBase> {
	...
	public ValidationResult<CreditLimitBase> validate(RosettaPath path, CreditLimitBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CreditLimitBase", ValidationType.TYPE_FORMAT, "CreditLimitBase", path, "", error);
		}
		return success("CreditLimitBase", ValidationType.TYPE_FORMAT, "CreditLimitBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditLimitBase", ValidationType.TYPE_FORMAT, "CreditLimitBase", path, "", res.getError());
				}
				return success("CreditLimitBase", ValidationType.TYPE_FORMAT, "CreditLimitBase", path, "");
			})
			.collect(toList());
