...
public class FxAverageRateObservationValidator implements Validator<FxAverageRateObservation> {
	...
	private List<ComparisonResult> getComparisonResults(FxAverageRateObservation o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("averageRateWeightingFactor", (BigDecimal) o.getAverageRateWeightingFactor() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<FxAverageRateObservation> validate(RosettaPath path, FxAverageRateObservation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxAverageRateObservation", ValidationType.CARDINALITY, "FxAverageRateObservation", path, "", error);
		}
		return success("FxAverageRateObservation", ValidationType.CARDINALITY, "FxAverageRateObservation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAverageRateObservation", ValidationType.CARDINALITY, "FxAverageRateObservation", path, "", res.getError());
				}
				return success("FxAverageRateObservation", ValidationType.CARDINALITY, "FxAverageRateObservation", path, "");
			})
			.collect(toList());
