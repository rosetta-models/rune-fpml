...
public class GasDeliveryPointTypeFormatValidator implements Validator<GasDeliveryPoint> {
	...
	public ValidationResult<GasDeliveryPoint> validate(RosettaPath path, GasDeliveryPoint o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("GasDeliveryPoint", ValidationType.TYPE_FORMAT, "GasDeliveryPoint", path, "", error);
		}
		return success("GasDeliveryPoint", ValidationType.TYPE_FORMAT, "GasDeliveryPoint", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GasDeliveryPoint", ValidationType.TYPE_FORMAT, "GasDeliveryPoint", path, "", res.getError());
				}
				return success("GasDeliveryPoint", ValidationType.TYPE_FORMAT, "GasDeliveryPoint", path, "");
			})
			.collect(toList());
