...
public class VarianceOptionTransactionSupplementValidator implements Validator<VarianceOptionTransactionSupplement> {
	...
	public ValidationResult<VarianceOptionTransactionSupplement> validate(RosettaPath path, VarianceOptionTransactionSupplement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("VarianceOptionTransactionSupplement", ValidationType.CARDINALITY, "VarianceOptionTransactionSupplement", path, "", error);
		}
		return success("VarianceOptionTransactionSupplement", ValidationType.CARDINALITY, "VarianceOptionTransactionSupplement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VarianceOptionTransactionSupplement", ValidationType.CARDINALITY, "VarianceOptionTransactionSupplement", path, "", res.getError());
				}
				return success("VarianceOptionTransactionSupplement", ValidationType.CARDINALITY, "VarianceOptionTransactionSupplement", path, "");
			})
			.collect(toList());
