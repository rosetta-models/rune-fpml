...
public class AbstractFacilityNotificationTypeFormatValidator implements Validator<AbstractFacilityNotification> {
	...
	public ValidationResult<AbstractFacilityNotification> validate(RosettaPath path, AbstractFacilityNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AbstractFacilityNotification", ValidationType.TYPE_FORMAT, "AbstractFacilityNotification", path, "", error);
		}
		return success("AbstractFacilityNotification", ValidationType.TYPE_FORMAT, "AbstractFacilityNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityNotification", ValidationType.TYPE_FORMAT, "AbstractFacilityNotification", path, "", res.getError());
				}
				return success("AbstractFacilityNotification", ValidationType.TYPE_FORMAT, "AbstractFacilityNotification", path, "");
			})
			.collect(toList());
