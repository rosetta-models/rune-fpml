...
public class MasterAgreementVersionTypeFormatValidator implements Validator<MasterAgreementVersion> {
	...
	public ValidationResult<MasterAgreementVersion> validate(RosettaPath path, MasterAgreementVersion o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MasterAgreementVersion", ValidationType.TYPE_FORMAT, "MasterAgreementVersion", path, "", error);
		}
		return success("MasterAgreementVersion", ValidationType.TYPE_FORMAT, "MasterAgreementVersion", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementVersion", ValidationType.TYPE_FORMAT, "MasterAgreementVersion", path, "", res.getError());
				}
				return success("MasterAgreementVersion", ValidationType.TYPE_FORMAT, "MasterAgreementVersion", path, "");
			})
			.collect(toList());
