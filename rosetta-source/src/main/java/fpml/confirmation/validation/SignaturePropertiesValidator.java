...
public class SignaturePropertiesValidator implements Validator<SignatureProperties> {
	...
	public ValidationResult<SignatureProperties> validate(RosettaPath path, SignatureProperties o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SignatureProperties", ValidationType.CARDINALITY, "SignatureProperties", path, "", error);
		}
		return success("SignatureProperties", ValidationType.CARDINALITY, "SignatureProperties", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureProperties", ValidationType.CARDINALITY, "SignatureProperties", path, "", res.getError());
				}
				return success("SignatureProperties", ValidationType.CARDINALITY, "SignatureProperties", path, "");
			})
			.collect(toList());
