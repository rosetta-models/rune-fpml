...
public class CommodityBasketModelValidator implements Validator<CommodityBasketModel> {
	...
	public ValidationResult<CommodityBasketModel> validate(RosettaPath path, CommodityBasketModel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityBasketModel", ValidationType.CARDINALITY, "CommodityBasketModel", path, "", error);
		}
		return success("CommodityBasketModel", ValidationType.CARDINALITY, "CommodityBasketModel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBasketModel", ValidationType.CARDINALITY, "CommodityBasketModel", path, "", res.getError());
				}
				return success("CommodityBasketModel", ValidationType.CARDINALITY, "CommodityBasketModel", path, "");
			})
			.collect(toList());
