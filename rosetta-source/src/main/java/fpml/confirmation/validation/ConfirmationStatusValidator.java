...
public class ConfirmationStatusValidator implements Validator<ConfirmationStatus> {
	...
	private List<ComparisonResult> getComparisonResults(ConfirmationStatus o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("confirmationStatusSequence", (ConfirmationStatusSequence) o.getConfirmationStatusSequence() != null ? 1 : 0, 0, 1), 
	...
	@Override
	public ValidationResult<ConfirmationStatus> validate(RosettaPath path, ConfirmationStatus o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ConfirmationStatus", ValidationType.CARDINALITY, "ConfirmationStatus", path, "", error);
		}
		return success("ConfirmationStatus", ValidationType.CARDINALITY, "ConfirmationStatus", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationStatus", ValidationType.CARDINALITY, "ConfirmationStatus", path, "", res.getError());
				}
				return success("ConfirmationStatus", ValidationType.CARDINALITY, "ConfirmationStatus", path, "");
			})
			.collect(toList());
