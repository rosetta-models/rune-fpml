...
public class FxAveragingProcessValidator implements Validator<FxAveragingProcess> {
	...
	public ValidationResult<FxAveragingProcess> validate(RosettaPath path, FxAveragingProcess o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxAveragingProcess", ValidationType.CARDINALITY, "FxAveragingProcess", path, "", error);
		}
		return success("FxAveragingProcess", ValidationType.CARDINALITY, "FxAveragingProcess", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAveragingProcess", ValidationType.CARDINALITY, "FxAveragingProcess", path, "", res.getError());
				}
				return success("FxAveragingProcess", ValidationType.CARDINALITY, "FxAveragingProcess", path, "");
			})
			.collect(toList());
