...
public class MessageHeaderTypeFormatValidator implements Validator<MessageHeader> {
	...
	public ValidationResult<MessageHeader> validate(RosettaPath path, MessageHeader o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MessageHeader", ValidationType.TYPE_FORMAT, "MessageHeader", path, "", error);
		}
		return success("MessageHeader", ValidationType.TYPE_FORMAT, "MessageHeader", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MessageHeader", ValidationType.TYPE_FORMAT, "MessageHeader", path, "", res.getError());
				}
				return success("MessageHeader", ValidationType.TYPE_FORMAT, "MessageHeader", path, "");
			})
			.collect(toList());
