...
public class ExchangeRestrictionsValidator implements Validator<ExchangeRestrictions> {
	...
	public ValidationResult<ExchangeRestrictions> validate(RosettaPath path, ExchangeRestrictions o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExchangeRestrictions", ValidationType.CARDINALITY, "ExchangeRestrictions", path, "", error);
		}
		return success("ExchangeRestrictions", ValidationType.CARDINALITY, "ExchangeRestrictions", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeRestrictions", ValidationType.CARDINALITY, "ExchangeRestrictions", path, "", res.getError());
				}
				return success("ExchangeRestrictions", ValidationType.CARDINALITY, "ExchangeRestrictions", path, "");
			})
			.collect(toList());
