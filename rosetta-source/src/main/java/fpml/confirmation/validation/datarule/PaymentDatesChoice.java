...
public interface PaymentDatesChoice extends Validator<PaymentDates> {
	...
	String DEFINITION = "required choice calculationPeriodDatesReference, resetDatesReference , valuationDatesReference";
	...
	class Default implements PaymentDatesChoice {
		...
		private ComparisonResult executeDataRule(PaymentDates paymentDates) {
			try {
				return choice(MapperS.of(paymentDates), Arrays.asList("calculationPeriodDatesReference", "resetDatesReference", "valuationDatesReference"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
