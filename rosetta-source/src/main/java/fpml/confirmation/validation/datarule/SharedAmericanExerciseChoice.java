...
public interface SharedAmericanExerciseChoice extends Validator<SharedAmericanExercise> {
	...
	ValidationResult<SharedAmericanExercise> validate(RosettaPath path, SharedAmericanExercise sharedAmericanExercise);
	
	class Default implements SharedAmericanExerciseChoice {
	
		@Override
		public ValidationResult<SharedAmericanExercise> validate(RosettaPath path, SharedAmericanExercise sharedAmericanExercise) {
			ComparisonResult result = executeDataRule(sharedAmericanExercise);
			if (result.get()) {
				return ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "SharedAmericanExercise", path, DEFINITION);
			}
			...
			return ValidationResult.failure(NAME, ValidationType.DATA_RULE, "SharedAmericanExercise", path, DEFINITION, failureMessage);
		}
