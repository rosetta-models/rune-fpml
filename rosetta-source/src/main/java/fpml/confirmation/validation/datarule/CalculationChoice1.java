...
public interface CalculationChoice1 extends Validator<Calculation> {
	...
	String DEFINITION = "required choice calculationSequence, rateCalculation";
	...
	class Default implements CalculationChoice1 {
		...
		private ComparisonResult executeDataRule(Calculation calculation) {
			try {
				return choice(MapperS.of(calculation), Arrays.asList("calculationSequence", "rateCalculation"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
