...
public interface FxCashSettlementSimpleChoice extends Validator<FxCashSettlementSimple> {
	...
	ValidationResult<FxCashSettlementSimple> validate(RosettaPath path, FxCashSettlementSimple fxCashSettlementSimple);
	
	class Default implements FxCashSettlementSimpleChoice {
	
		@Override
		public ValidationResult<FxCashSettlementSimple> validate(RosettaPath path, FxCashSettlementSimple fxCashSettlementSimple) {
			ComparisonResult result = executeDataRule(fxCashSettlementSimple);
			if (result.get()) {
				return ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "FxCashSettlementSimple", path, DEFINITION);
			}
			...
			return ValidationResult.failure(NAME, ValidationType.DATA_RULE, "FxCashSettlementSimple", path, DEFINITION, failureMessage);
		}
