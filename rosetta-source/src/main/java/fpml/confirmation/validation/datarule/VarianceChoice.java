...
public interface VarianceChoice extends Validator<Variance> {
	...
	ValidationResult<Variance> validate(RosettaPath path, Variance variance);
	
	class Default implements VarianceChoice {
	
		@Override
		public ValidationResult<Variance> validate(RosettaPath path, Variance variance) {
			ComparisonResult result = executeDataRule(variance);
			if (result.get()) {
				return ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "Variance", path, DEFINITION);
			}
			...
			return ValidationResult.failure(NAME, ValidationType.DATA_RULE, "Variance", path, DEFINITION, failureMessage);
		}
