...
public interface FxPerformanceSwapChoice extends Validator<FxPerformanceSwap> {
	...
	ValidationResult<FxPerformanceSwap> validate(RosettaPath path, FxPerformanceSwap fxPerformanceSwap);
	
	class Default implements FxPerformanceSwapChoice {
	
		@Override
		public ValidationResult<FxPerformanceSwap> validate(RosettaPath path, FxPerformanceSwap fxPerformanceSwap) {
			ComparisonResult result = executeDataRule(fxPerformanceSwap);
			if (result.get()) {
				return ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "FxPerformanceSwap", path, DEFINITION);
			}
			...
			return ValidationResult.failure(NAME, ValidationType.DATA_RULE, "FxPerformanceSwap", path, DEFINITION, failureMessage);
		}
