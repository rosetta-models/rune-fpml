 ...
 */
@RosettaDataRule("FxFixingDateChoice")
@ImplementedBy(FxFixingDateChoice.Default.class)
public interface FxFixingDateChoice extends Validator<FxFixingDate> {
	
	String NAME = "FxFixingDateChoice";
	String DEFINITION = "required choice dateRelativeToPaymentDates, dateRelativeToCalculationPeriodDates";
	
	ValidationResult<FxFixingDate> validate(RosettaPath path, FxFixingDate fxFixingDate);
	
	class Default implements FxFixingDateChoice {
		...
		private ComparisonResult executeDataRule(FxFixingDate fxFixingDate) {
			try {
				return choice(MapperS.of(fxFixingDate), Arrays.asList("dateRelativeToPaymentDates", "dateRelativeToCalculationPeriodDates"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements FxFixingDateChoice {
