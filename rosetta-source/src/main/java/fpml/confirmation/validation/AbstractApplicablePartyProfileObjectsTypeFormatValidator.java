...
public class AbstractApplicablePartyProfileObjectsTypeFormatValidator implements Validator<AbstractApplicablePartyProfileObjects> {
	...
	public ValidationResult<AbstractApplicablePartyProfileObjects> validate(RosettaPath path, AbstractApplicablePartyProfileObjects o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AbstractApplicablePartyProfileObjects", ValidationType.TYPE_FORMAT, "AbstractApplicablePartyProfileObjects", path, "", error);
		}
		return success("AbstractApplicablePartyProfileObjects", ValidationType.TYPE_FORMAT, "AbstractApplicablePartyProfileObjects", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractApplicablePartyProfileObjects", ValidationType.TYPE_FORMAT, "AbstractApplicablePartyProfileObjects", path, "", res.getError());
				}
				return success("AbstractApplicablePartyProfileObjects", ValidationType.TYPE_FORMAT, "AbstractApplicablePartyProfileObjects", path, "");
			})
			.collect(toList());
