...
public class FxAmericanExerciseValidator implements Validator<FxAmericanExercise> {
	...
	public ValidationResult<FxAmericanExercise> validate(RosettaPath path, FxAmericanExercise o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxAmericanExercise", ValidationType.CARDINALITY, "FxAmericanExercise", path, "", error);
		}
		return success("FxAmericanExercise", ValidationType.CARDINALITY, "FxAmericanExercise", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAmericanExercise", ValidationType.CARDINALITY, "FxAmericanExercise", path, "", res.getError());
				}
				return success("FxAmericanExercise", ValidationType.CARDINALITY, "FxAmericanExercise", path, "");
			})
			.collect(toList());
