...
public class CommodityBusinessCalendarTypeFormatValidator implements Validator<CommodityBusinessCalendar> {
	...
	public ValidationResult<CommodityBusinessCalendar> validate(RosettaPath path, CommodityBusinessCalendar o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityBusinessCalendar", ValidationType.TYPE_FORMAT, "CommodityBusinessCalendar", path, "", error);
		}
		return success("CommodityBusinessCalendar", ValidationType.TYPE_FORMAT, "CommodityBusinessCalendar", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBusinessCalendar", ValidationType.TYPE_FORMAT, "CommodityBusinessCalendar", path, "", res.getError());
				}
				return success("CommodityBusinessCalendar", ValidationType.TYPE_FORMAT, "CommodityBusinessCalendar", path, "");
			})
			.collect(toList());
