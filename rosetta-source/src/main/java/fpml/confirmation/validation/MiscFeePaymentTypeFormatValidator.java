...
public class MiscFeePaymentTypeFormatValidator implements Validator<MiscFeePayment> {
	...
	public ValidationResult<MiscFeePayment> validate(RosettaPath path, MiscFeePayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MiscFeePayment", ValidationType.TYPE_FORMAT, "MiscFeePayment", path, "", error);
		}
		return success("MiscFeePayment", ValidationType.TYPE_FORMAT, "MiscFeePayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MiscFeePayment", ValidationType.TYPE_FORMAT, "MiscFeePayment", path, "", res.getError());
				}
				return success("MiscFeePayment", ValidationType.TYPE_FORMAT, "MiscFeePayment", path, "");
			})
			.collect(toList());
