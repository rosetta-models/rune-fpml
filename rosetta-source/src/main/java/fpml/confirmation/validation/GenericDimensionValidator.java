...
public class GenericDimensionValidator implements Validator<GenericDimension> {
	...
	public ValidationResult<GenericDimension> validate(RosettaPath path, GenericDimension o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("GenericDimension", ValidationType.CARDINALITY, "GenericDimension", path, "", error);
		}
		return success("GenericDimension", ValidationType.CARDINALITY, "GenericDimension", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericDimension", ValidationType.CARDINALITY, "GenericDimension", path, "", res.getError());
				}
				return success("GenericDimension", ValidationType.CARDINALITY, "GenericDimension", path, "");
			})
			.collect(toList());
