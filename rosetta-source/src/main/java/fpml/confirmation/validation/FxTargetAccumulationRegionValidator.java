...
public class FxTargetAccumulationRegionValidator implements Validator<FxTargetAccumulationRegion> {
	...
	public ValidationResult<FxTargetAccumulationRegion> validate(RosettaPath path, FxTargetAccumulationRegion o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxTargetAccumulationRegion", ValidationType.CARDINALITY, "FxTargetAccumulationRegion", path, "", error);
		}
		return success("FxTargetAccumulationRegion", ValidationType.CARDINALITY, "FxTargetAccumulationRegion", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetAccumulationRegion", ValidationType.CARDINALITY, "FxTargetAccumulationRegion", path, "", res.getError());
				}
				return success("FxTargetAccumulationRegion", ValidationType.CARDINALITY, "FxTargetAccumulationRegion", path, "");
			})
			.collect(toList());
