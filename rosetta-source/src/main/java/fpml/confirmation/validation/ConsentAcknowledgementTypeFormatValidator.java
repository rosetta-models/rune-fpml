...
public class ConsentAcknowledgementTypeFormatValidator implements Validator<ConsentAcknowledgement> {
	...
	@Override
	public ValidationResult<ConsentAcknowledgement> validate(RosettaPath path, ConsentAcknowledgement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ConsentAcknowledgement", ValidationType.TYPE_FORMAT, "ConsentAcknowledgement", path, "", error);
		}
		return success("ConsentAcknowledgement", ValidationType.TYPE_FORMAT, "ConsentAcknowledgement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentAcknowledgement", ValidationType.TYPE_FORMAT, "ConsentAcknowledgement", path, "", res.getError());
				}
				return success("ConsentAcknowledgement", ValidationType.TYPE_FORMAT, "ConsentAcknowledgement", path, "");
			})
			.collect(toList());
