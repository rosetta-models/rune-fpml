...
public class ObjectTypeSequenceValidator implements Validator<ObjectTypeSequence> {
	...
	public ValidationResult<ObjectTypeSequence> validate(RosettaPath path, ObjectTypeSequence o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ObjectTypeSequence", ValidationType.CARDINALITY, "ObjectTypeSequence", path, "", error);
		}
		return success("ObjectTypeSequence", ValidationType.CARDINALITY, "ObjectTypeSequence", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObjectTypeSequence", ValidationType.CARDINALITY, "ObjectTypeSequence", path, "", res.getError());
				}
				return success("ObjectTypeSequence", ValidationType.CARDINALITY, "ObjectTypeSequence", path, "");
			})
			.collect(toList());
