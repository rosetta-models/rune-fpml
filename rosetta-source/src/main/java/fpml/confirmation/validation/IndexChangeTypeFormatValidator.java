...
public class IndexChangeTypeFormatValidator implements Validator<IndexChange> {
	...
	public ValidationResult<IndexChange> validate(RosettaPath path, IndexChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("IndexChange", ValidationType.TYPE_FORMAT, "IndexChange", path, "", error);
		}
		return success("IndexChange", ValidationType.TYPE_FORMAT, "IndexChange", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexChange", ValidationType.TYPE_FORMAT, "IndexChange", path, "", res.getError());
				}
				return success("IndexChange", ValidationType.TYPE_FORMAT, "IndexChange", path, "");
			})
			.collect(toList());
