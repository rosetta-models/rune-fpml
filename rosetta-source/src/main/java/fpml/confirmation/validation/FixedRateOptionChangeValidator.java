...
public class FixedRateOptionChangeValidator implements Validator<FixedRateOptionChange> {
	...
	private List<ComparisonResult> getComparisonResults(FixedRateOptionChange o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("fixedRateOption", (FixedRateOption) o.getFixedRateOption() != null ? 1 : 0, 1, 1)
			);
	}
	...
	public ValidationResult<FixedRateOptionChange> validate(RosettaPath path, FixedRateOptionChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FixedRateOptionChange", ValidationType.CARDINALITY, "FixedRateOptionChange", path, "", error);
		}
		return success("FixedRateOptionChange", ValidationType.CARDINALITY, "FixedRateOptionChange", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedRateOptionChange", ValidationType.CARDINALITY, "FixedRateOptionChange", path, "", res.getError());
				}
				return success("FixedRateOptionChange", ValidationType.CARDINALITY, "FixedRateOptionChange", path, "");
			})
			.collect(toList());
