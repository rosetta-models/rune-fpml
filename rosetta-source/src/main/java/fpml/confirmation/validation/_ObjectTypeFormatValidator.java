...
public class _ObjectTypeFormatValidator implements Validator<_Object> {
	...
	public ValidationResult<_Object> validate(RosettaPath path, _Object o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("_Object", ValidationType.TYPE_FORMAT, "_Object", path, "", error);
		}
		return success("_Object", ValidationType.TYPE_FORMAT, "_Object", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("_Object", ValidationType.TYPE_FORMAT, "_Object", path, "", res.getError());
				}
				return success("_Object", ValidationType.TYPE_FORMAT, "_Object", path, "");
			})
			.collect(toList());
