...
public class FxTriggerBaseTypeFormatValidator implements Validator<FxTriggerBase> {
	...
	public ValidationResult<FxTriggerBase> validate(RosettaPath path, FxTriggerBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxTriggerBase", ValidationType.TYPE_FORMAT, "FxTriggerBase", path, "", error);
		}
		return success("FxTriggerBase", ValidationType.TYPE_FORMAT, "FxTriggerBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTriggerBase", ValidationType.TYPE_FORMAT, "FxTriggerBase", path, "", res.getError());
				}
				return success("FxTriggerBase", ValidationType.TYPE_FORMAT, "FxTriggerBase", path, "");
			})
			.collect(toList());
