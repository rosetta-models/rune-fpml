...
public class RequestExecutionRetractedValidator implements Validator<RequestExecutionRetracted> {
	...
	public ValidationResult<RequestExecutionRetracted> validate(RosettaPath path, RequestExecutionRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestExecutionRetracted", ValidationType.CARDINALITY, "RequestExecutionRetracted", path, "", error);
		}
		return success("RequestExecutionRetracted", ValidationType.CARDINALITY, "RequestExecutionRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestExecutionRetracted", ValidationType.CARDINALITY, "RequestExecutionRetracted", path, "", res.getError());
				}
				return success("RequestExecutionRetracted", ValidationType.CARDINALITY, "RequestExecutionRetracted", path, "");
			})
			.collect(toList());
