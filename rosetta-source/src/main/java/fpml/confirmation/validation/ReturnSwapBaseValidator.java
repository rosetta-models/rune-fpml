...
public class ReturnSwapBaseValidator implements Validator<ReturnSwapBase> {
	...
	public ValidationResult<ReturnSwapBase> validate(RosettaPath path, ReturnSwapBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ReturnSwapBase", ValidationType.CARDINALITY, "ReturnSwapBase", path, "", error);
		}
		return success("ReturnSwapBase", ValidationType.CARDINALITY, "ReturnSwapBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnSwapBase", ValidationType.CARDINALITY, "ReturnSwapBase", path, "", res.getError());
				}
				return success("ReturnSwapBase", ValidationType.CARDINALITY, "ReturnSwapBase", path, "");
			})
			.collect(toList());
