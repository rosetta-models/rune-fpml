...
public class BankruptcyEventTypeFormatValidator implements Validator<BankruptcyEvent> {
	...
	public ValidationResult<BankruptcyEvent> validate(RosettaPath path, BankruptcyEvent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BankruptcyEvent", ValidationType.TYPE_FORMAT, "BankruptcyEvent", path, "", error);
		}
		return success("BankruptcyEvent", ValidationType.TYPE_FORMAT, "BankruptcyEvent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BankruptcyEvent", ValidationType.TYPE_FORMAT, "BankruptcyEvent", path, "", res.getError());
				}
				return success("BankruptcyEvent", ValidationType.TYPE_FORMAT, "BankruptcyEvent", path, "");
			})
			.collect(toList());
