...
public class NonNegativeMoneyValidator implements Validator<NonNegativeMoney> {
	...
	@Override
	public ValidationResult<NonNegativeMoney> validate(RosettaPath path, NonNegativeMoney o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("NonNegativeMoney", ValidationType.CARDINALITY, "NonNegativeMoney", path, "", error);
		}
		return success("NonNegativeMoney", ValidationType.CARDINALITY, "NonNegativeMoney", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonNegativeMoney", ValidationType.CARDINALITY, "NonNegativeMoney", path, "", res.getError());
				}
				return success("NonNegativeMoney", ValidationType.CARDINALITY, "NonNegativeMoney", path, "");
			})
			.collect(toList());
