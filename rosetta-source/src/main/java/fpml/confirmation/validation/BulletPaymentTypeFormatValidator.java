...
public class BulletPaymentTypeFormatValidator implements Validator<BulletPayment> {
	...
	public ValidationResult<BulletPayment> validate(RosettaPath path, BulletPayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BulletPayment", ValidationType.TYPE_FORMAT, "BulletPayment", path, "", error);
		}
		return success("BulletPayment", ValidationType.TYPE_FORMAT, "BulletPayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BulletPayment", ValidationType.TYPE_FORMAT, "BulletPayment", path, "", res.getError());
				}
				return success("BulletPayment", ValidationType.TYPE_FORMAT, "BulletPayment", path, "");
			})
			.collect(toList());
