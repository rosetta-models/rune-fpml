...
public class RSAKeyValueTypeTypeFormatValidator implements Validator<RSAKeyValueType> {
	...
	public ValidationResult<RSAKeyValueType> validate(RosettaPath path, RSAKeyValueType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RSAKeyValueType", ValidationType.TYPE_FORMAT, "RSAKeyValueType", path, "", error);
		}
		return success("RSAKeyValueType", ValidationType.TYPE_FORMAT, "RSAKeyValueType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RSAKeyValueType", ValidationType.TYPE_FORMAT, "RSAKeyValueType", path, "", res.getError());
				}
				return success("RSAKeyValueType", ValidationType.TYPE_FORMAT, "RSAKeyValueType", path, "");
			})
			.collect(toList());
