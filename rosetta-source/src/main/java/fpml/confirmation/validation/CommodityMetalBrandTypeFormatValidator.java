...
public class CommodityMetalBrandTypeFormatValidator implements Validator<CommodityMetalBrand> {
	...
	public ValidationResult<CommodityMetalBrand> validate(RosettaPath path, CommodityMetalBrand o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityMetalBrand", ValidationType.TYPE_FORMAT, "CommodityMetalBrand", path, "", error);
		}
		return success("CommodityMetalBrand", ValidationType.TYPE_FORMAT, "CommodityMetalBrand", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityMetalBrand", ValidationType.TYPE_FORMAT, "CommodityMetalBrand", path, "", res.getError());
				}
				return success("CommodityMetalBrand", ValidationType.TYPE_FORMAT, "CommodityMetalBrand", path, "");
			})
			.collect(toList());
