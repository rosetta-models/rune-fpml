...
public class ExchangeTradedFundValidator implements Validator<ExchangeTradedFund> {
	...
	public ValidationResult<ExchangeTradedFund> validate(RosettaPath path, ExchangeTradedFund o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExchangeTradedFund", ValidationType.CARDINALITY, "ExchangeTradedFund", path, "", error);
		}
		return success("ExchangeTradedFund", ValidationType.CARDINALITY, "ExchangeTradedFund", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTradedFund", ValidationType.CARDINALITY, "ExchangeTradedFund", path, "", res.getError());
				}
				return success("ExchangeTradedFund", ValidationType.CARDINALITY, "ExchangeTradedFund", path, "");
			})
			.collect(toList());
