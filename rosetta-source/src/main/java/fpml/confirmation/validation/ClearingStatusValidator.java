...
public class ClearingStatusValidator implements Validator<ClearingStatus> {
	...
	@Override
	public ValidationResult<ClearingStatus> validate(RosettaPath path, ClearingStatus o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ClearingStatus", ValidationType.CARDINALITY, "ClearingStatus", path, "", error);
		}
		return success("ClearingStatus", ValidationType.CARDINALITY, "ClearingStatus", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingStatus", ValidationType.CARDINALITY, "ClearingStatus", path, "", res.getError());
				}
				return success("ClearingStatus", ValidationType.CARDINALITY, "ClearingStatus", path, "");
			})
			.collect(toList());
