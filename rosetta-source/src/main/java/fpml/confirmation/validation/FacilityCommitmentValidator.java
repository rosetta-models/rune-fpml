...
public class FacilityCommitmentValidator implements Validator<FacilityCommitment> {
	...
	private List<ComparisonResult> getComparisonResults(FacilityCommitment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("facilityCommitmentSequence", (FacilityCommitmentSequence) o.getFacilityCommitmentSequence() != null ? 1 : 0, 0, 1)
			);
	}
	...
	public ValidationResult<FacilityCommitment> validate(RosettaPath path, FacilityCommitment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityCommitment", ValidationType.CARDINALITY, "FacilityCommitment", path, "", error);
		}
		return success("FacilityCommitment", ValidationType.CARDINALITY, "FacilityCommitment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityCommitment", ValidationType.CARDINALITY, "FacilityCommitment", path, "", res.getError());
				}
				return success("FacilityCommitment", ValidationType.CARDINALITY, "FacilityCommitment", path, "");
			})
			.collect(toList());
