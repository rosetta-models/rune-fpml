...
public class VolatilitySwapTypeFormatValidator implements Validator<VolatilitySwap> {
	...
	public ValidationResult<VolatilitySwap> validate(RosettaPath path, VolatilitySwap o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("VolatilitySwap", ValidationType.TYPE_FORMAT, "VolatilitySwap", path, "", error);
		}
		return success("VolatilitySwap", ValidationType.TYPE_FORMAT, "VolatilitySwap", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilitySwap", ValidationType.TYPE_FORMAT, "VolatilitySwap", path, "", res.getError());
				}
				return success("VolatilitySwap", ValidationType.TYPE_FORMAT, "VolatilitySwap", path, "");
			})
			.collect(toList());
