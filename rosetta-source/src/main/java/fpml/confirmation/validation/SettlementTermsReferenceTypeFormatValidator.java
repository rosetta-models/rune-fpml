...
public class SettlementTermsReferenceTypeFormatValidator implements Validator<SettlementTermsReference> {
	...
	public ValidationResult<SettlementTermsReference> validate(RosettaPath path, SettlementTermsReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SettlementTermsReference", ValidationType.TYPE_FORMAT, "SettlementTermsReference", path, "", error);
		}
		return success("SettlementTermsReference", ValidationType.TYPE_FORMAT, "SettlementTermsReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementTermsReference", ValidationType.TYPE_FORMAT, "SettlementTermsReference", path, "", res.getError());
				}
				return success("SettlementTermsReference", ValidationType.TYPE_FORMAT, "SettlementTermsReference", path, "");
			})
			.collect(toList());
