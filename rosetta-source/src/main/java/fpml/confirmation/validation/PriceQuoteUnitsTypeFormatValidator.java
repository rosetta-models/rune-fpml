...
public class PriceQuoteUnitsTypeFormatValidator implements Validator<PriceQuoteUnits> {
	...
	public ValidationResult<PriceQuoteUnits> validate(RosettaPath path, PriceQuoteUnits o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PriceQuoteUnits", ValidationType.TYPE_FORMAT, "PriceQuoteUnits", path, "", error);
		}
		return success("PriceQuoteUnits", ValidationType.TYPE_FORMAT, "PriceQuoteUnits", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceQuoteUnits", ValidationType.TYPE_FORMAT, "PriceQuoteUnits", path, "", res.getError());
				}
				return success("PriceQuoteUnits", ValidationType.TYPE_FORMAT, "PriceQuoteUnits", path, "");
			})
			.collect(toList());
