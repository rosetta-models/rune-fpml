...
public class CoalPhysicalLegValidator implements Validator<CoalPhysicalLeg> {
	...
	private List<ComparisonResult> getComparisonResults(CoalPhysicalLeg o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("deliveryConditions", (CoalDelivery) o.getDeliveryConditions() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<CoalPhysicalLeg> validate(RosettaPath path, CoalPhysicalLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CoalPhysicalLeg", ValidationType.CARDINALITY, "CoalPhysicalLeg", path, "", error);
		}
		return success("CoalPhysicalLeg", ValidationType.CARDINALITY, "CoalPhysicalLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalPhysicalLeg", ValidationType.CARDINALITY, "CoalPhysicalLeg", path, "", res.getError());
				}
				return success("CoalPhysicalLeg", ValidationType.CARDINALITY, "CoalPhysicalLeg", path, "");
			})
			.collect(toList());
