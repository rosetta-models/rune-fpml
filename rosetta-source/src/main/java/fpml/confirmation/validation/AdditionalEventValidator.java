...
public class AdditionalEventValidator implements Validator<AdditionalEvent> {
	...
	public ValidationResult<AdditionalEvent> validate(RosettaPath path, AdditionalEvent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AdditionalEvent", ValidationType.CARDINALITY, "AdditionalEvent", path, "", error);
		}
		return success("AdditionalEvent", ValidationType.CARDINALITY, "AdditionalEvent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdditionalEvent", ValidationType.CARDINALITY, "AdditionalEvent", path, "", res.getError());
				}
				return success("AdditionalEvent", ValidationType.CARDINALITY, "AdditionalEvent", path, "");
			})
			.collect(toList());
