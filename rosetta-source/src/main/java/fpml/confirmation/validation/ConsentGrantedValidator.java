...
public class ConsentGrantedValidator implements Validator<ConsentGranted> {
	...
	@Override
	public ValidationResult<ConsentGranted> validate(RosettaPath path, ConsentGranted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ConsentGranted", ValidationType.CARDINALITY, "ConsentGranted", path, "", error);
		}
		return success("ConsentGranted", ValidationType.CARDINALITY, "ConsentGranted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentGranted", ValidationType.CARDINALITY, "ConsentGranted", path, "", res.getError());
				}
				return success("ConsentGranted", ValidationType.CARDINALITY, "ConsentGranted", path, "");
			})
			.collect(toList());
