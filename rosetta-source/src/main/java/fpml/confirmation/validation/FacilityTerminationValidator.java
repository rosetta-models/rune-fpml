...
public class FacilityTerminationValidator implements Validator<FacilityTermination> {
	...
	public ValidationResult<FacilityTermination> validate(RosettaPath path, FacilityTermination o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityTermination", ValidationType.CARDINALITY, "FacilityTermination", path, "", error);
		}
		return success("FacilityTermination", ValidationType.CARDINALITY, "FacilityTermination", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityTermination", ValidationType.CARDINALITY, "FacilityTermination", path, "", res.getError());
				}
				return success("FacilityTermination", ValidationType.CARDINALITY, "FacilityTermination", path, "");
			})
			.collect(toList());
