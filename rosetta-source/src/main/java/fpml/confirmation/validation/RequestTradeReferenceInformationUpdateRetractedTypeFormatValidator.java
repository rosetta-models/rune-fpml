...
public class RequestTradeReferenceInformationUpdateRetractedTypeFormatValidator implements Validator<RequestTradeReferenceInformationUpdateRetracted> {
	...
	public ValidationResult<RequestTradeReferenceInformationUpdateRetracted> validate(RosettaPath path, RequestTradeReferenceInformationUpdateRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestTradeReferenceInformationUpdateRetracted", ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdateRetracted", path, "", error);
		}
		return success("RequestTradeReferenceInformationUpdateRetracted", ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdateRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestTradeReferenceInformationUpdateRetracted", ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdateRetracted", path, "", res.getError());
				}
				return success("RequestTradeReferenceInformationUpdateRetracted", ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdateRetracted", path, "");
			})
			.collect(toList());
