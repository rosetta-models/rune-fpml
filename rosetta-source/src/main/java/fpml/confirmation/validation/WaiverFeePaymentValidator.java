...
public class WaiverFeePaymentValidator implements Validator<WaiverFeePayment> {
	...
	public ValidationResult<WaiverFeePayment> validate(RosettaPath path, WaiverFeePayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("WaiverFeePayment", ValidationType.CARDINALITY, "WaiverFeePayment", path, "", error);
		}
		return success("WaiverFeePayment", ValidationType.CARDINALITY, "WaiverFeePayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WaiverFeePayment", ValidationType.CARDINALITY, "WaiverFeePayment", path, "", res.getError());
				}
				return success("WaiverFeePayment", ValidationType.CARDINALITY, "WaiverFeePayment", path, "");
			})
			.collect(toList());
