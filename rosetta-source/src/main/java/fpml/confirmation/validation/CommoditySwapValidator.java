...
public class CommoditySwapValidator implements Validator<CommoditySwap> {
	...
	public ValidationResult<CommoditySwap> validate(RosettaPath path, CommoditySwap o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommoditySwap", ValidationType.CARDINALITY, "CommoditySwap", path, "", error);
		}
		return success("CommoditySwap", ValidationType.CARDINALITY, "CommoditySwap", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySwap", ValidationType.CARDINALITY, "CommoditySwap", path, "", res.getError());
				}
				return success("CommoditySwap", ValidationType.CARDINALITY, "CommoditySwap", path, "");
			})
			.collect(toList());
