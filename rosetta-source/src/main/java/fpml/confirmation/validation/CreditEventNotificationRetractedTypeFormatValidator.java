...
public class CreditEventNotificationRetractedTypeFormatValidator implements Validator<CreditEventNotificationRetracted> {
	...
	public ValidationResult<CreditEventNotificationRetracted> validate(RosettaPath path, CreditEventNotificationRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CreditEventNotificationRetracted", ValidationType.TYPE_FORMAT, "CreditEventNotificationRetracted", path, "", error);
		}
		return success("CreditEventNotificationRetracted", ValidationType.TYPE_FORMAT, "CreditEventNotificationRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventNotificationRetracted", ValidationType.TYPE_FORMAT, "CreditEventNotificationRetracted", path, "", res.getError());
				}
				return success("CreditEventNotificationRetracted", ValidationType.TYPE_FORMAT, "CreditEventNotificationRetracted", path, "");
			})
			.collect(toList());
