...
public class TransformsTypeFormatValidator implements Validator<Transforms> {
	...
	public ValidationResult<Transforms> validate(RosettaPath path, Transforms o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Transforms", ValidationType.TYPE_FORMAT, "Transforms", path, "", error);
		}
		return success("Transforms", ValidationType.TYPE_FORMAT, "Transforms", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Transforms", ValidationType.TYPE_FORMAT, "Transforms", path, "", res.getError());
				}
				return success("Transforms", ValidationType.TYPE_FORMAT, "Transforms", path, "");
			})
			.collect(toList());
