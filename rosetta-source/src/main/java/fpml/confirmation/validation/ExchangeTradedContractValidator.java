...
public class ExchangeTradedContractValidator implements Validator<ExchangeTradedContract> {
	...
	public ValidationResult<ExchangeTradedContract> validate(RosettaPath path, ExchangeTradedContract o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExchangeTradedContract", ValidationType.CARDINALITY, "ExchangeTradedContract", path, "", error);
		}
		return success("ExchangeTradedContract", ValidationType.CARDINALITY, "ExchangeTradedContract", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTradedContract", ValidationType.CARDINALITY, "ExchangeTradedContract", path, "", res.getError());
				}
				return success("ExchangeTradedContract", ValidationType.CARDINALITY, "ExchangeTradedContract", path, "");
			})
			.collect(toList());
