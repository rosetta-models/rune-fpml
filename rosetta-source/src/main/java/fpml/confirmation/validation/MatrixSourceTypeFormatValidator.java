...
public class MatrixSourceTypeFormatValidator implements Validator<MatrixSource> {
	...
	public ValidationResult<MatrixSource> validate(RosettaPath path, MatrixSource o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MatrixSource", ValidationType.TYPE_FORMAT, "MatrixSource", path, "", error);
		}
		return success("MatrixSource", ValidationType.TYPE_FORMAT, "MatrixSource", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MatrixSource", ValidationType.TYPE_FORMAT, "MatrixSource", path, "", res.getError());
				}
				return success("MatrixSource", ValidationType.TYPE_FORMAT, "MatrixSource", path, "");
			})
			.collect(toList());
