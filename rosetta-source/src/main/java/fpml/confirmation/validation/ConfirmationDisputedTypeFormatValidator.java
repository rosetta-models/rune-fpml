...
public class ConfirmationDisputedTypeFormatValidator implements Validator<ConfirmationDisputed> {
	...
	public ValidationResult<ConfirmationDisputed> validate(RosettaPath path, ConfirmationDisputed o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ConfirmationDisputed", ValidationType.TYPE_FORMAT, "ConfirmationDisputed", path, "", error);
		}
		return success("ConfirmationDisputed", ValidationType.TYPE_FORMAT, "ConfirmationDisputed", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationDisputed", ValidationType.TYPE_FORMAT, "ConfirmationDisputed", path, "", res.getError());
				}
				return success("ConfirmationDisputed", ValidationType.TYPE_FORMAT, "ConfirmationDisputed", path, "");
			})
			.collect(toList());
