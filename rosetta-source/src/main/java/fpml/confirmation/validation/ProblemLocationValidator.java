...
public class ProblemLocationValidator implements Validator<ProblemLocation> {
	...
	public ValidationResult<ProblemLocation> validate(RosettaPath path, ProblemLocation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ProblemLocation", ValidationType.CARDINALITY, "ProblemLocation", path, "", error);
		}
		return success("ProblemLocation", ValidationType.CARDINALITY, "ProblemLocation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ProblemLocation", ValidationType.CARDINALITY, "ProblemLocation", path, "", res.getError());
				}
				return success("ProblemLocation", ValidationType.CARDINALITY, "ProblemLocation", path, "");
			})
			.collect(toList());
