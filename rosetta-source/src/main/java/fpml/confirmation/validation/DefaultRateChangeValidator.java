...
public class DefaultRateChangeValidator implements Validator<DefaultRateChange> {
	...
	public ValidationResult<DefaultRateChange> validate(RosettaPath path, DefaultRateChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DefaultRateChange", ValidationType.CARDINALITY, "DefaultRateChange", path, "", error);
		}
		return success("DefaultRateChange", ValidationType.CARDINALITY, "DefaultRateChange", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DefaultRateChange", ValidationType.CARDINALITY, "DefaultRateChange", path, "", res.getError());
				}
				return success("DefaultRateChange", ValidationType.CARDINALITY, "DefaultRateChange", path, "");
			})
			.collect(toList());
