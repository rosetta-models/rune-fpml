...
public class PhysicalForwardLegValidator implements Validator<PhysicalForwardLeg> {
	...
	public ValidationResult<PhysicalForwardLeg> validate(RosettaPath path, PhysicalForwardLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PhysicalForwardLeg", ValidationType.CARDINALITY, "PhysicalForwardLeg", path, "", error);
		}
		return success("PhysicalForwardLeg", ValidationType.CARDINALITY, "PhysicalForwardLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PhysicalForwardLeg", ValidationType.CARDINALITY, "PhysicalForwardLeg", path, "", res.getError());
				}
				return success("PhysicalForwardLeg", ValidationType.CARDINALITY, "PhysicalForwardLeg", path, "");
			})
			.collect(toList());
