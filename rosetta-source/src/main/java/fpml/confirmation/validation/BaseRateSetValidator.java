...
public class BaseRateSetValidator implements Validator<BaseRateSet> {
	...
	public ValidationResult<BaseRateSet> validate(RosettaPath path, BaseRateSet o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BaseRateSet", ValidationType.CARDINALITY, "BaseRateSet", path, "", error);
		}
		return success("BaseRateSet", ValidationType.CARDINALITY, "BaseRateSet", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BaseRateSet", ValidationType.CARDINALITY, "BaseRateSet", path, "", res.getError());
				}
				return success("BaseRateSet", ValidationType.CARDINALITY, "BaseRateSet", path, "");
			})
			.collect(toList());
