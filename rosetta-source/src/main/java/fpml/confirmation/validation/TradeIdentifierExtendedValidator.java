...
public class TradeIdentifierExtendedValidator implements Validator<TradeIdentifierExtended> {
	...
	private List<ComparisonResult> getComparisonResults(TradeIdentifierExtended o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("associatedPartyReference", (PartyReference) o.getAssociatedPartyReference() != null ? 1 : 0, 1, 1)
			);
	}
	...
	public ValidationResult<TradeIdentifierExtended> validate(RosettaPath path, TradeIdentifierExtended o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TradeIdentifierExtended", ValidationType.CARDINALITY, "TradeIdentifierExtended", path, "", error);
		}
		return success("TradeIdentifierExtended", ValidationType.CARDINALITY, "TradeIdentifierExtended", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeIdentifierExtended", ValidationType.CARDINALITY, "TradeIdentifierExtended", path, "", res.getError());
				}
				return success("TradeIdentifierExtended", ValidationType.CARDINALITY, "TradeIdentifierExtended", path, "");
			})
			.collect(toList());
