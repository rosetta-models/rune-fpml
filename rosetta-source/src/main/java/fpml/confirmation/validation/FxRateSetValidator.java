...
public class FxRateSetValidator implements Validator<FxRateSet> {
	...
	public ValidationResult<FxRateSet> validate(RosettaPath path, FxRateSet o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxRateSet", ValidationType.CARDINALITY, "FxRateSet", path, "", error);
		}
		return success("FxRateSet", ValidationType.CARDINALITY, "FxRateSet", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxRateSet", ValidationType.CARDINALITY, "FxRateSet", path, "", res.getError());
				}
				return success("FxRateSet", ValidationType.CARDINALITY, "FxRateSet", path, "");
			})
			.collect(toList());
