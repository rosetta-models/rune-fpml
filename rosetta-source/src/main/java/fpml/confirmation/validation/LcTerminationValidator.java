...
public class LcTerminationValidator implements Validator<LcTermination> {
	...
	public ValidationResult<LcTermination> validate(RosettaPath path, LcTermination o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcTermination", ValidationType.CARDINALITY, "LcTermination", path, "", error);
		}
		return success("LcTermination", ValidationType.CARDINALITY, "LcTermination", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcTermination", ValidationType.CARDINALITY, "LcTermination", path, "", res.getError());
				}
				return success("LcTermination", ValidationType.CARDINALITY, "LcTermination", path, "");
			})
			.collect(toList());
