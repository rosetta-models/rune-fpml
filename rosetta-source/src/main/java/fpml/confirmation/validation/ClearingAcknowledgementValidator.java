...
public class ClearingAcknowledgementValidator implements Validator<ClearingAcknowledgement> {
	...
	public ValidationResult<ClearingAcknowledgement> validate(RosettaPath path, ClearingAcknowledgement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ClearingAcknowledgement", ValidationType.CARDINALITY, "ClearingAcknowledgement", path, "", error);
		}
		return success("ClearingAcknowledgement", ValidationType.CARDINALITY, "ClearingAcknowledgement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingAcknowledgement", ValidationType.CARDINALITY, "ClearingAcknowledgement", path, "", res.getError());
				}
				return success("ClearingAcknowledgement", ValidationType.CARDINALITY, "ClearingAcknowledgement", path, "");
			})
			.collect(toList());
