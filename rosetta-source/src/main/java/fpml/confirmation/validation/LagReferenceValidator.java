...
public class LagReferenceValidator implements Validator<LagReference> {
	...
	public ValidationResult<LagReference> validate(RosettaPath path, LagReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LagReference", ValidationType.CARDINALITY, "LagReference", path, "", error);
		}
		return success("LagReference", ValidationType.CARDINALITY, "LagReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LagReference", ValidationType.CARDINALITY, "LagReference", path, "", res.getError());
				}
				return success("LagReference", ValidationType.CARDINALITY, "LagReference", path, "");
			})
			.collect(toList());
