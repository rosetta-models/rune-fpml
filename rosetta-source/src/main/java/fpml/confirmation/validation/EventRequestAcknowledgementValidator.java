...
public class EventRequestAcknowledgementValidator implements Validator<EventRequestAcknowledgement> {
	...
	public ValidationResult<EventRequestAcknowledgement> validate(RosettaPath path, EventRequestAcknowledgement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EventRequestAcknowledgement", ValidationType.CARDINALITY, "EventRequestAcknowledgement", path, "", error);
		}
		return success("EventRequestAcknowledgement", ValidationType.CARDINALITY, "EventRequestAcknowledgement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventRequestAcknowledgement", ValidationType.CARDINALITY, "EventRequestAcknowledgement", path, "", res.getError());
				}
				return success("EventRequestAcknowledgement", ValidationType.CARDINALITY, "EventRequestAcknowledgement", path, "");
			})
			.collect(toList());
