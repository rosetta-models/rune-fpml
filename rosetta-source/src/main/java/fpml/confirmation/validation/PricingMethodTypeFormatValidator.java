...
public class PricingMethodTypeFormatValidator implements Validator<PricingMethod> {
	...
	public ValidationResult<PricingMethod> validate(RosettaPath path, PricingMethod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PricingMethod", ValidationType.TYPE_FORMAT, "PricingMethod", path, "", error);
		}
		return success("PricingMethod", ValidationType.TYPE_FORMAT, "PricingMethod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingMethod", ValidationType.TYPE_FORMAT, "PricingMethod", path, "", res.getError());
				}
				return success("PricingMethod", ValidationType.TYPE_FORMAT, "PricingMethod", path, "");
			})
			.collect(toList());
