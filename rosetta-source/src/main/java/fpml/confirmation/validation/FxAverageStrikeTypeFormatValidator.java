...
public class FxAverageStrikeTypeFormatValidator implements Validator<FxAverageStrike> {
	...
	public ValidationResult<FxAverageStrike> validate(RosettaPath path, FxAverageStrike o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxAverageStrike", ValidationType.TYPE_FORMAT, "FxAverageStrike", path, "", error);
		}
		return success("FxAverageStrike", ValidationType.TYPE_FORMAT, "FxAverageStrike", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAverageStrike", ValidationType.TYPE_FORMAT, "FxAverageStrike", path, "", res.getError());
				}
				return success("FxAverageStrike", ValidationType.TYPE_FORMAT, "FxAverageStrike", path, "");
			})
			.collect(toList());
