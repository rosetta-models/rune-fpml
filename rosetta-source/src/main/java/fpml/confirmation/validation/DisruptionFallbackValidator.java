...
public class DisruptionFallbackValidator implements Validator<DisruptionFallback> {
	...
	public ValidationResult<DisruptionFallback> validate(RosettaPath path, DisruptionFallback o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DisruptionFallback", ValidationType.CARDINALITY, "DisruptionFallback", path, "", error);
		}
		return success("DisruptionFallback", ValidationType.CARDINALITY, "DisruptionFallback", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DisruptionFallback", ValidationType.CARDINALITY, "DisruptionFallback", path, "", res.getError());
				}
				return success("DisruptionFallback", ValidationType.CARDINALITY, "DisruptionFallback", path, "");
			})
			.collect(toList());
