...
public class NonRecurringFeePaymentNotificationValidator implements Validator<NonRecurringFeePaymentNotification> {
	...
	private List<ComparisonResult> getComparisonResults(NonRecurringFeePaymentNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("facilityFeePaymentGroup", (NonRecurringFeePayment) o.getFacilityFeePaymentGroup() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<NonRecurringFeePaymentNotification> validate(RosettaPath path, NonRecurringFeePaymentNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("NonRecurringFeePaymentNotification", ValidationType.CARDINALITY, "NonRecurringFeePaymentNotification", path, "", error);
		}
		return success("NonRecurringFeePaymentNotification", ValidationType.CARDINALITY, "NonRecurringFeePaymentNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonRecurringFeePaymentNotification", ValidationType.CARDINALITY, "NonRecurringFeePaymentNotification", path, "", res.getError());
				}
				return success("NonRecurringFeePaymentNotification", ValidationType.CARDINALITY, "NonRecurringFeePaymentNotification", path, "");
			})
			.collect(toList());
