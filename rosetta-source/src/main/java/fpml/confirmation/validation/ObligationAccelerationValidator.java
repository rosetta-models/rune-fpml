...
public class ObligationAccelerationValidator implements Validator<ObligationAcceleration> {
	...
	public ValidationResult<ObligationAcceleration> validate(RosettaPath path, ObligationAcceleration o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ObligationAcceleration", ValidationType.CARDINALITY, "ObligationAcceleration", path, "", error);
		}
		return success("ObligationAcceleration", ValidationType.CARDINALITY, "ObligationAcceleration", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObligationAcceleration", ValidationType.CARDINALITY, "ObligationAcceleration", path, "", res.getError());
				}
				return success("ObligationAcceleration", ValidationType.CARDINALITY, "ObligationAcceleration", path, "");
			})
			.collect(toList());
