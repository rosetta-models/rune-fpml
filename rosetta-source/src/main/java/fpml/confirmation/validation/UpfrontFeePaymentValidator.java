...
public class UpfrontFeePaymentValidator implements Validator<UpfrontFeePayment> {
	...
	public ValidationResult<UpfrontFeePayment> validate(RosettaPath path, UpfrontFeePayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("UpfrontFeePayment", ValidationType.CARDINALITY, "UpfrontFeePayment", path, "", error);
		}
		return success("UpfrontFeePayment", ValidationType.CARDINALITY, "UpfrontFeePayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UpfrontFeePayment", ValidationType.CARDINALITY, "UpfrontFeePayment", path, "", res.getError());
				}
				return success("UpfrontFeePayment", ValidationType.CARDINALITY, "UpfrontFeePayment", path, "");
			})
			.collect(toList());
