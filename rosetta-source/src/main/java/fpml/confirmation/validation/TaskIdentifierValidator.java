...
public class TaskIdentifierValidator implements Validator<TaskIdentifier> {
	...
	public ValidationResult<TaskIdentifier> validate(RosettaPath path, TaskIdentifier o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TaskIdentifier", ValidationType.CARDINALITY, "TaskIdentifier", path, "", error);
		}
		return success("TaskIdentifier", ValidationType.CARDINALITY, "TaskIdentifier", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TaskIdentifier", ValidationType.CARDINALITY, "TaskIdentifier", path, "", res.getError());
				}
				return success("TaskIdentifier", ValidationType.CARDINALITY, "TaskIdentifier", path, "");
			})
			.collect(toList());
