...
public class CommoditySpreadScheduleTypeFormatValidator implements Validator<CommoditySpreadSchedule> {
	...
	public ValidationResult<CommoditySpreadSchedule> validate(RosettaPath path, CommoditySpreadSchedule o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommoditySpreadSchedule", ValidationType.TYPE_FORMAT, "CommoditySpreadSchedule", path, "", error);
		}
		return success("CommoditySpreadSchedule", ValidationType.TYPE_FORMAT, "CommoditySpreadSchedule", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySpreadSchedule", ValidationType.TYPE_FORMAT, "CommoditySpreadSchedule", path, "", res.getError());
				}
				return success("CommoditySpreadSchedule", ValidationType.TYPE_FORMAT, "CommoditySpreadSchedule", path, "");
			})
			.collect(toList());
