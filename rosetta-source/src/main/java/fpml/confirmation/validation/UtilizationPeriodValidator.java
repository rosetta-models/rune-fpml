...
public class UtilizationPeriodValidator implements Validator<UtilizationPeriod> {
	...
	public ValidationResult<UtilizationPeriod> validate(RosettaPath path, UtilizationPeriod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("UtilizationPeriod", ValidationType.CARDINALITY, "UtilizationPeriod", path, "", error);
		}
		return success("UtilizationPeriod", ValidationType.CARDINALITY, "UtilizationPeriod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UtilizationPeriod", ValidationType.CARDINALITY, "UtilizationPeriod", path, "", res.getError());
				}
				return success("UtilizationPeriod", ValidationType.CARDINALITY, "UtilizationPeriod", path, "");
			})
			.collect(toList());
