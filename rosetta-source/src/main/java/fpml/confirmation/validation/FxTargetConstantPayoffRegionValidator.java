...
public class FxTargetConstantPayoffRegionValidator implements Validator<FxTargetConstantPayoffRegion> {
	...
	private List<ComparisonResult> getComparisonResults(FxTargetConstantPayoffRegion o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("fxTargetRegionBoundModel", (FxTargetRegionBoundModel) o.getFxTargetRegionBoundModel() != null ? 1 : 0, 1, 1), 
	...
	@Override
	public ValidationResult<FxTargetConstantPayoffRegion> validate(RosettaPath path, FxTargetConstantPayoffRegion o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxTargetConstantPayoffRegion", ValidationType.CARDINALITY, "FxTargetConstantPayoffRegion", path, "", error);
		}
		return success("FxTargetConstantPayoffRegion", ValidationType.CARDINALITY, "FxTargetConstantPayoffRegion", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetConstantPayoffRegion", ValidationType.CARDINALITY, "FxTargetConstantPayoffRegion", path, "", res.getError());
				}
				return success("FxTargetConstantPayoffRegion", ValidationType.CARDINALITY, "FxTargetConstantPayoffRegion", path, "");
			})
			.collect(toList());
