...
public class LoanChoiceValidator implements Validator<LoanChoice> {
	...
	public ValidationResult<LoanChoice> validate(RosettaPath path, LoanChoice o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanChoice", ValidationType.CARDINALITY, "LoanChoice", path, "", error);
		}
		return success("LoanChoice", ValidationType.CARDINALITY, "LoanChoice", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanChoice", ValidationType.CARDINALITY, "LoanChoice", path, "", res.getError());
				}
				return success("LoanChoice", ValidationType.CARDINALITY, "LoanChoice", path, "");
			})
			.collect(toList());
