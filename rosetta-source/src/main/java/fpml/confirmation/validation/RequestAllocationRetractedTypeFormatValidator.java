...
public class RequestAllocationRetractedTypeFormatValidator implements Validator<RequestAllocationRetracted> {
	...
	public ValidationResult<RequestAllocationRetracted> validate(RosettaPath path, RequestAllocationRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestAllocationRetracted", ValidationType.TYPE_FORMAT, "RequestAllocationRetracted", path, "", error);
		}
		return success("RequestAllocationRetracted", ValidationType.TYPE_FORMAT, "RequestAllocationRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestAllocationRetracted", ValidationType.TYPE_FORMAT, "RequestAllocationRetracted", path, "", res.getError());
				}
				return success("RequestAllocationRetracted", ValidationType.TYPE_FORMAT, "RequestAllocationRetracted", path, "");
			})
			.collect(toList());
