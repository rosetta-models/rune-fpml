...
public class ClearingExceptionValidator implements Validator<ClearingException> {
	...
	@Override
	public ValidationResult<ClearingException> validate(RosettaPath path, ClearingException o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ClearingException", ValidationType.CARDINALITY, "ClearingException", path, "", error);
		}
		return success("ClearingException", ValidationType.CARDINALITY, "ClearingException", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingException", ValidationType.CARDINALITY, "ClearingException", path, "", res.getError());
				}
				return success("ClearingException", ValidationType.CARDINALITY, "ClearingException", path, "");
			})
			.collect(toList());
