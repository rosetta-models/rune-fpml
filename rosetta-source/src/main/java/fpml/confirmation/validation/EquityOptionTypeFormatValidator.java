...
public class EquityOptionTypeFormatValidator implements Validator<EquityOption> {
	...
	@Override
	public ValidationResult<EquityOption> validate(RosettaPath path, EquityOption o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EquityOption", ValidationType.TYPE_FORMAT, "EquityOption", path, "", error);
		}
		return success("EquityOption", ValidationType.TYPE_FORMAT, "EquityOption", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityOption", ValidationType.TYPE_FORMAT, "EquityOption", path, "", res.getError());
				}
				return success("EquityOption", ValidationType.TYPE_FORMAT, "EquityOption", path, "");
			})
			.collect(toList());
