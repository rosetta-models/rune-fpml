...
public class PhysicalSettlementTypeFormatValidator implements Validator<PhysicalSettlement> {
	...
	public ValidationResult<PhysicalSettlement> validate(RosettaPath path, PhysicalSettlement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PhysicalSettlement", ValidationType.TYPE_FORMAT, "PhysicalSettlement", path, "", error);
		}
		return success("PhysicalSettlement", ValidationType.TYPE_FORMAT, "PhysicalSettlement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PhysicalSettlement", ValidationType.TYPE_FORMAT, "PhysicalSettlement", path, "", res.getError());
				}
				return success("PhysicalSettlement", ValidationType.TYPE_FORMAT, "PhysicalSettlement", path, "");
			})
			.collect(toList());
