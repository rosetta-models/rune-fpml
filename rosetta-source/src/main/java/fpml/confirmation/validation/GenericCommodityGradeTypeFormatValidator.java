...
public class GenericCommodityGradeTypeFormatValidator implements Validator<GenericCommodityGrade> {
	...
	public ValidationResult<GenericCommodityGrade> validate(RosettaPath path, GenericCommodityGrade o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("GenericCommodityGrade", ValidationType.TYPE_FORMAT, "GenericCommodityGrade", path, "", error);
		}
		return success("GenericCommodityGrade", ValidationType.TYPE_FORMAT, "GenericCommodityGrade", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericCommodityGrade", ValidationType.TYPE_FORMAT, "GenericCommodityGrade", path, "", res.getError());
				}
				return success("GenericCommodityGrade", ValidationType.TYPE_FORMAT, "GenericCommodityGrade", path, "");
			})
			.collect(toList());
