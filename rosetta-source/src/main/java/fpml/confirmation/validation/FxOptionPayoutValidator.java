...
public class FxOptionPayoutValidator implements Validator<FxOptionPayout> {
	...
	private List<ComparisonResult> getComparisonResults(FxOptionPayout o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("payoutStyle", (PayoutEnum) o.getPayoutStyle() != null ? 1 : 0, 1, 1), 
				checkCardinality("settlementInformation", (SettlementInformation) o.getSettlementInformation() != null ? 1 : 0, 0, 1)
			);
	}

	@Override
	public ValidationResult<FxOptionPayout> validate(RosettaPath path, FxOptionPayout o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxOptionPayout", ValidationType.CARDINALITY, "FxOptionPayout", path, "", error);
		}
		return success("FxOptionPayout", ValidationType.CARDINALITY, "FxOptionPayout", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOptionPayout", ValidationType.CARDINALITY, "FxOptionPayout", path, "", res.getError());
				}
				return success("FxOptionPayout", ValidationType.CARDINALITY, "FxOptionPayout", path, "");
			})
			.collect(toList());
