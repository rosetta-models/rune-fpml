...
public class LoanAcknowledgementTypeFormatValidator implements Validator<LoanAcknowledgement> {
	...
	public ValidationResult<LoanAcknowledgement> validate(RosettaPath path, LoanAcknowledgement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanAcknowledgement", ValidationType.TYPE_FORMAT, "LoanAcknowledgement", path, "", error);
		}
		return success("LoanAcknowledgement", ValidationType.TYPE_FORMAT, "LoanAcknowledgement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAcknowledgement", ValidationType.TYPE_FORMAT, "LoanAcknowledgement", path, "", res.getError());
				}
				return success("LoanAcknowledgement", ValidationType.TYPE_FORMAT, "LoanAcknowledgement", path, "");
			})
			.collect(toList());
