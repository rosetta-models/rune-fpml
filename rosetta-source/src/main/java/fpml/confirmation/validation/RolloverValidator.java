...
public class RolloverValidator implements Validator<Rollover> {
	...
	private List<ComparisonResult> getComparisonResults(Rollover o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("maturingContracts", (String) o.getMaturingContracts() != null ? 1 : 0, 1, 1), 
				checkCardinality("currentContracts", (String) o.getCurrentContracts() != null ? 1 : 0, 1, 1), 
				checkCardinality("rolloverChoice", (List<? extends RolloverChoice>) o.getRolloverChoice() == null ? 0 : ((List<? extends RolloverChoice>) o.getRolloverChoice()).size(), 1, 0)
			);
	}
	...
	public ValidationResult<Rollover> validate(RosettaPath path, Rollover o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Rollover", ValidationType.CARDINALITY, "Rollover", path, "", error);
		}
		return success("Rollover", ValidationType.CARDINALITY, "Rollover", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Rollover", ValidationType.CARDINALITY, "Rollover", path, "", res.getError());
				}
				return success("Rollover", ValidationType.CARDINALITY, "Rollover", path, "");
			})
			.collect(toList());
