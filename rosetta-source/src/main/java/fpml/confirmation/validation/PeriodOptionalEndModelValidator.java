...
public class PeriodOptionalEndModelValidator implements Validator<PeriodOptionalEndModel> {
	...
	public ValidationResult<PeriodOptionalEndModel> validate(RosettaPath path, PeriodOptionalEndModel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PeriodOptionalEndModel", ValidationType.CARDINALITY, "PeriodOptionalEndModel", path, "", error);
		}
		return success("PeriodOptionalEndModel", ValidationType.CARDINALITY, "PeriodOptionalEndModel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PeriodOptionalEndModel", ValidationType.CARDINALITY, "PeriodOptionalEndModel", path, "", res.getError());
				}
				return success("PeriodOptionalEndModel", ValidationType.CARDINALITY, "PeriodOptionalEndModel", path, "");
			})
			.collect(toList());
