...
public class MaterialValidator implements Validator<Material> {
	...
	@Override
	public ValidationResult<Material> validate(RosettaPath path, Material o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Material", ValidationType.CARDINALITY, "Material", path, "", error);
		}
		return success("Material", ValidationType.CARDINALITY, "Material", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Material", ValidationType.CARDINALITY, "Material", path, "", res.getError());
				}
				return success("Material", ValidationType.CARDINALITY, "Material", path, "");
			})
			.collect(toList());
