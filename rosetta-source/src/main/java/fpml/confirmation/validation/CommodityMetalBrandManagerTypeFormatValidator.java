...
public class CommodityMetalBrandManagerTypeFormatValidator implements Validator<CommodityMetalBrandManager> {
	...
	public ValidationResult<CommodityMetalBrandManager> validate(RosettaPath path, CommodityMetalBrandManager o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityMetalBrandManager", ValidationType.TYPE_FORMAT, "CommodityMetalBrandManager", path, "", error);
		}
		return success("CommodityMetalBrandManager", ValidationType.TYPE_FORMAT, "CommodityMetalBrandManager", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityMetalBrandManager", ValidationType.TYPE_FORMAT, "CommodityMetalBrandManager", path, "", res.getError());
				}
				return success("CommodityMetalBrandManager", ValidationType.TYPE_FORMAT, "CommodityMetalBrandManager", path, "");
			})
			.collect(toList());
