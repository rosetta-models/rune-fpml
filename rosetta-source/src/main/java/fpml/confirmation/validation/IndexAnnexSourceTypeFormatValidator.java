...
public class IndexAnnexSourceTypeFormatValidator implements Validator<IndexAnnexSource> {
	...
	public ValidationResult<IndexAnnexSource> validate(RosettaPath path, IndexAnnexSource o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("IndexAnnexSource", ValidationType.TYPE_FORMAT, "IndexAnnexSource", path, "", error);
		}
		return success("IndexAnnexSource", ValidationType.TYPE_FORMAT, "IndexAnnexSource", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexAnnexSource", ValidationType.TYPE_FORMAT, "IndexAnnexSource", path, "", res.getError());
				}
				return success("IndexAnnexSource", ValidationType.TYPE_FORMAT, "IndexAnnexSource", path, "");
			})
			.collect(toList());
