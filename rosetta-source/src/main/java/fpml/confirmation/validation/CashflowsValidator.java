...
public class CashflowsValidator implements Validator<Cashflows> {
	...
	public ValidationResult<Cashflows> validate(RosettaPath path, Cashflows o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Cashflows", ValidationType.CARDINALITY, "Cashflows", path, "", error);
		}
		return success("Cashflows", ValidationType.CARDINALITY, "Cashflows", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Cashflows", ValidationType.CARDINALITY, "Cashflows", path, "", res.getError());
				}
				return success("Cashflows", ValidationType.CARDINALITY, "Cashflows", path, "");
			})
			.collect(toList());
