...
public class StandardProductTypeFormatValidator implements Validator<StandardProduct> {
	...
	public ValidationResult<StandardProduct> validate(RosettaPath path, StandardProduct o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("StandardProduct", ValidationType.TYPE_FORMAT, "StandardProduct", path, "", error);
		}
		return success("StandardProduct", ValidationType.TYPE_FORMAT, "StandardProduct", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StandardProduct", ValidationType.TYPE_FORMAT, "StandardProduct", path, "", res.getError());
				}
				return success("StandardProduct", ValidationType.TYPE_FORMAT, "StandardProduct", path, "");
			})
			.collect(toList());
