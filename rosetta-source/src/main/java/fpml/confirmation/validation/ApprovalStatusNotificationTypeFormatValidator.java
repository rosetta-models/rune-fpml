...
public class ApprovalStatusNotificationTypeFormatValidator implements Validator<ApprovalStatusNotification> {
	...
	public ValidationResult<ApprovalStatusNotification> validate(RosettaPath path, ApprovalStatusNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ApprovalStatusNotification", ValidationType.TYPE_FORMAT, "ApprovalStatusNotification", path, "", error);
		}
		return success("ApprovalStatusNotification", ValidationType.TYPE_FORMAT, "ApprovalStatusNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ApprovalStatusNotification", ValidationType.TYPE_FORMAT, "ApprovalStatusNotification", path, "", res.getError());
				}
				return success("ApprovalStatusNotification", ValidationType.TYPE_FORMAT, "ApprovalStatusNotification", path, "");
			})
			.collect(toList());
