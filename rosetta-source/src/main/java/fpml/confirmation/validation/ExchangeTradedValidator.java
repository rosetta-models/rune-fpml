...
public class ExchangeTradedValidator implements Validator<ExchangeTraded> {
	...
	public ValidationResult<ExchangeTraded> validate(RosettaPath path, ExchangeTraded o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExchangeTraded", ValidationType.CARDINALITY, "ExchangeTraded", path, "", error);
		}
		return success("ExchangeTraded", ValidationType.CARDINALITY, "ExchangeTraded", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTraded", ValidationType.CARDINALITY, "ExchangeTraded", path, "", res.getError());
				}
				return success("ExchangeTraded", ValidationType.CARDINALITY, "ExchangeTraded", path, "");
			})
			.collect(toList());
