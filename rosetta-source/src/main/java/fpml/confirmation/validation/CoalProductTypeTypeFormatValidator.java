...
public class CoalProductTypeTypeFormatValidator implements Validator<CoalProductType> {
	...
	public ValidationResult<CoalProductType> validate(RosettaPath path, CoalProductType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CoalProductType", ValidationType.TYPE_FORMAT, "CoalProductType", path, "", error);
		}
		return success("CoalProductType", ValidationType.TYPE_FORMAT, "CoalProductType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalProductType", ValidationType.TYPE_FORMAT, "CoalProductType", path, "", res.getError());
				}
				return success("CoalProductType", ValidationType.TYPE_FORMAT, "CoalProductType", path, "");
			})
			.collect(toList());
