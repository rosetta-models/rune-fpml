...
public class ExerciseValidator implements Validator<Exercise> {
	...
	public ValidationResult<Exercise> validate(RosettaPath path, Exercise o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Exercise", ValidationType.CARDINALITY, "Exercise", path, "", error);
		}
		return success("Exercise", ValidationType.CARDINALITY, "Exercise", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Exercise", ValidationType.CARDINALITY, "Exercise", path, "", res.getError());
				}
				return success("Exercise", ValidationType.CARDINALITY, "Exercise", path, "");
			})
			.collect(toList());
