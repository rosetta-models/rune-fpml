...
public class AdjustableRelativeOrPeriodicDates2TypeFormatValidator implements Validator<AdjustableRelativeOrPeriodicDates2> {
	...
	public ValidationResult<AdjustableRelativeOrPeriodicDates2> validate(RosettaPath path, AdjustableRelativeOrPeriodicDates2 o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AdjustableRelativeOrPeriodicDates2", ValidationType.TYPE_FORMAT, "AdjustableRelativeOrPeriodicDates2", path, "", error);
		}
		return success("AdjustableRelativeOrPeriodicDates2", ValidationType.TYPE_FORMAT, "AdjustableRelativeOrPeriodicDates2", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableRelativeOrPeriodicDates2", ValidationType.TYPE_FORMAT, "AdjustableRelativeOrPeriodicDates2", path, "", res.getError());
				}
				return success("AdjustableRelativeOrPeriodicDates2", ValidationType.TYPE_FORMAT, "AdjustableRelativeOrPeriodicDates2", path, "");
			})
			.collect(toList());
