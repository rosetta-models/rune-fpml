...
public class QuotationTypeFormatValidator implements Validator<Quotation> {
	...
	public ValidationResult<Quotation> validate(RosettaPath path, Quotation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Quotation", ValidationType.TYPE_FORMAT, "Quotation", path, "", error);
		}
		return success("Quotation", ValidationType.TYPE_FORMAT, "Quotation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Quotation", ValidationType.TYPE_FORMAT, "Quotation", path, "", res.getError());
				}
				return success("Quotation", ValidationType.TYPE_FORMAT, "Quotation", path, "");
			})
			.collect(toList());
