...
public class BasicQuotationValidator implements Validator<BasicQuotation> {
	...
	public ValidationResult<BasicQuotation> validate(RosettaPath path, BasicQuotation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BasicQuotation", ValidationType.CARDINALITY, "BasicQuotation", path, "", error);
		}
		return success("BasicQuotation", ValidationType.CARDINALITY, "BasicQuotation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasicQuotation", ValidationType.CARDINALITY, "BasicQuotation", path, "", res.getError());
				}
				return success("BasicQuotation", ValidationType.CARDINALITY, "BasicQuotation", path, "");
			})
			.collect(toList());
