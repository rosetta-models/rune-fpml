...
public class RequestEventStatusValidator implements Validator<RequestEventStatus> {
	...
	@Override
	public ValidationResult<RequestEventStatus> validate(RosettaPath path, RequestEventStatus o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestEventStatus", ValidationType.CARDINALITY, "RequestEventStatus", path, "", error);
		}
		return success("RequestEventStatus", ValidationType.CARDINALITY, "RequestEventStatus", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestEventStatus", ValidationType.CARDINALITY, "RequestEventStatus", path, "", res.getError());
				}
				return success("RequestEventStatus", ValidationType.CARDINALITY, "RequestEventStatus", path, "");
			})
			.collect(toList());
