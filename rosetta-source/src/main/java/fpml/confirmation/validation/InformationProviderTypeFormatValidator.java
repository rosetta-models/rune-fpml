...
public class InformationProviderTypeFormatValidator implements Validator<InformationProvider> {
	...
	public ValidationResult<InformationProvider> validate(RosettaPath path, InformationProvider o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InformationProvider", ValidationType.TYPE_FORMAT, "InformationProvider", path, "", error);
		}
		return success("InformationProvider", ValidationType.TYPE_FORMAT, "InformationProvider", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InformationProvider", ValidationType.TYPE_FORMAT, "InformationProvider", path, "", res.getError());
				}
				return success("InformationProvider", ValidationType.TYPE_FORMAT, "InformationProvider", path, "");
			})
			.collect(toList());
