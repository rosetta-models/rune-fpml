...
public class AssociationToAssetIdentifierValidator implements Validator<AssociationToAssetIdentifier> {
	...
	private List<ComparisonResult> getComparisonResults(AssociationToAssetIdentifier o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("associationtId", (AssociationToAssetId) o.getAssociationtId() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<AssociationToAssetIdentifier> validate(RosettaPath path, AssociationToAssetIdentifier o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AssociationToAssetIdentifier", ValidationType.CARDINALITY, "AssociationToAssetIdentifier", path, "", error);
		}
		return success("AssociationToAssetIdentifier", ValidationType.CARDINALITY, "AssociationToAssetIdentifier", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssociationToAssetIdentifier", ValidationType.CARDINALITY, "AssociationToAssetIdentifier", path, "", res.getError());
				}
				return success("AssociationToAssetIdentifier", ValidationType.CARDINALITY, "AssociationToAssetIdentifier", path, "");
			})
			.collect(toList());
