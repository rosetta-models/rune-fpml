...
public class CommodityBasketUnderlyingByPercentageValidator implements Validator<CommodityBasketUnderlyingByPercentage> {
	...
	public ValidationResult<CommodityBasketUnderlyingByPercentage> validate(RosettaPath path, CommodityBasketUnderlyingByPercentage o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityBasketUnderlyingByPercentage", ValidationType.CARDINALITY, "CommodityBasketUnderlyingByPercentage", path, "", error);
		}
		return success("CommodityBasketUnderlyingByPercentage", ValidationType.CARDINALITY, "CommodityBasketUnderlyingByPercentage", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBasketUnderlyingByPercentage", ValidationType.CARDINALITY, "CommodityBasketUnderlyingByPercentage", path, "", res.getError());
				}
				return success("CommodityBasketUnderlyingByPercentage", ValidationType.CARDINALITY, "CommodityBasketUnderlyingByPercentage", path, "");
			})
			.collect(toList());
