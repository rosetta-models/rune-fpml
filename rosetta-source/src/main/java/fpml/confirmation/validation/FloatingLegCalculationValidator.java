...
public class FloatingLegCalculationValidator implements Validator<FloatingLegCalculation> {
	...
	public ValidationResult<FloatingLegCalculation> validate(RosettaPath path, FloatingLegCalculation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FloatingLegCalculation", ValidationType.CARDINALITY, "FloatingLegCalculation", path, "", error);
		}
		return success("FloatingLegCalculation", ValidationType.CARDINALITY, "FloatingLegCalculation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingLegCalculation", ValidationType.CARDINALITY, "FloatingLegCalculation", path, "", res.getError());
				}
				return success("FloatingLegCalculation", ValidationType.CARDINALITY, "FloatingLegCalculation", path, "");
			})
			.collect(toList());
