...
public class ClearingRefusedTypeFormatValidator implements Validator<ClearingRefused> {
	...
	public ValidationResult<ClearingRefused> validate(RosettaPath path, ClearingRefused o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ClearingRefused", ValidationType.TYPE_FORMAT, "ClearingRefused", path, "", error);
		}
		return success("ClearingRefused", ValidationType.TYPE_FORMAT, "ClearingRefused", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingRefused", ValidationType.TYPE_FORMAT, "ClearingRefused", path, "", res.getError());
				}
				return success("ClearingRefused", ValidationType.TYPE_FORMAT, "ClearingRefused", path, "");
			})
			.collect(toList());
