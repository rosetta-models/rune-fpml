...
public class FxPivotReferenceTypeFormatValidator implements Validator<FxPivotReference> {
	...
	public ValidationResult<FxPivotReference> validate(RosettaPath path, FxPivotReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxPivotReference", ValidationType.TYPE_FORMAT, "FxPivotReference", path, "", error);
		}
		return success("FxPivotReference", ValidationType.TYPE_FORMAT, "FxPivotReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxPivotReference", ValidationType.TYPE_FORMAT, "FxPivotReference", path, "", res.getError());
				}
				return success("FxPivotReference", ValidationType.TYPE_FORMAT, "FxPivotReference", path, "");
			})
			.collect(toList());
