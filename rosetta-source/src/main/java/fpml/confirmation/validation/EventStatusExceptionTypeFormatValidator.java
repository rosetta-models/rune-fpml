...
public class EventStatusExceptionTypeFormatValidator implements Validator<EventStatusException> {
	...
	public ValidationResult<EventStatusException> validate(RosettaPath path, EventStatusException o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EventStatusException", ValidationType.TYPE_FORMAT, "EventStatusException", path, "", error);
		}
		return success("EventStatusException", ValidationType.TYPE_FORMAT, "EventStatusException", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventStatusException", ValidationType.TYPE_FORMAT, "EventStatusException", path, "", res.getError());
				}
				return success("EventStatusException", ValidationType.TYPE_FORMAT, "EventStatusException", path, "");
			})
			.collect(toList());
