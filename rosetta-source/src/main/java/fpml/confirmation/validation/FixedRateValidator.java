...
public class FixedRateValidator implements Validator<FixedRate> {
	...
	public ValidationResult<FixedRate> validate(RosettaPath path, FixedRate o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FixedRate", ValidationType.CARDINALITY, "FixedRate", path, "", error);
		}
		return success("FixedRate", ValidationType.CARDINALITY, "FixedRate", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedRate", ValidationType.CARDINALITY, "FixedRate", path, "", res.getError());
				}
				return success("FixedRate", ValidationType.CARDINALITY, "FixedRate", path, "");
			})
			.collect(toList());
