...
public class LoanTransferNotificationTypeFormatValidator implements Validator<LoanTransferNotification> {
	...
	public ValidationResult<LoanTransferNotification> validate(RosettaPath path, LoanTransferNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanTransferNotification", ValidationType.TYPE_FORMAT, "LoanTransferNotification", path, "", error);
		}
		return success("LoanTransferNotification", ValidationType.TYPE_FORMAT, "LoanTransferNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTransferNotification", ValidationType.TYPE_FORMAT, "LoanTransferNotification", path, "", res.getError());
				}
				return success("LoanTransferNotification", ValidationType.TYPE_FORMAT, "LoanTransferNotification", path, "");
			})
			.collect(toList());
