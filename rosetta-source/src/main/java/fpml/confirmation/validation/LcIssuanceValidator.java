...
public class LcIssuanceValidator implements Validator<LcIssuance> {
	...
	public ValidationResult<LcIssuance> validate(RosettaPath path, LcIssuance o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcIssuance", ValidationType.CARDINALITY, "LcIssuance", path, "", error);
		}
		return success("LcIssuance", ValidationType.CARDINALITY, "LcIssuance", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcIssuance", ValidationType.CARDINALITY, "LcIssuance", path, "", res.getError());
				}
				return success("LcIssuance", ValidationType.CARDINALITY, "LcIssuance", path, "");
			})
			.collect(toList());
