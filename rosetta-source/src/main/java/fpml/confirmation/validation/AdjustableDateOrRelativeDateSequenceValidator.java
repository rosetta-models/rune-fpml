...
public class AdjustableDateOrRelativeDateSequenceValidator implements Validator<AdjustableDateOrRelativeDateSequence> {
	...
	public ValidationResult<AdjustableDateOrRelativeDateSequence> validate(RosettaPath path, AdjustableDateOrRelativeDateSequence o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AdjustableDateOrRelativeDateSequence", ValidationType.CARDINALITY, "AdjustableDateOrRelativeDateSequence", path, "", error);
		}
		return success("AdjustableDateOrRelativeDateSequence", ValidationType.CARDINALITY, "AdjustableDateOrRelativeDateSequence", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableDateOrRelativeDateSequence", ValidationType.CARDINALITY, "AdjustableDateOrRelativeDateSequence", path, "", res.getError());
				}
				return success("AdjustableDateOrRelativeDateSequence", ValidationType.CARDINALITY, "AdjustableDateOrRelativeDateSequence", path, "");
			})
			.collect(toList());
