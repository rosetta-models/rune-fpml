...
public class PayerModelValidator implements Validator<PayerModel> {
	...
	public ValidationResult<PayerModel> validate(RosettaPath path, PayerModel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PayerModel", ValidationType.CARDINALITY, "PayerModel", path, "", error);
		}
		return success("PayerModel", ValidationType.CARDINALITY, "PayerModel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PayerModel", ValidationType.CARDINALITY, "PayerModel", path, "", res.getError());
				}
				return success("PayerModel", ValidationType.CARDINALITY, "PayerModel", path, "");
			})
			.collect(toList());
