...
public class MessageAddressTypeFormatValidator implements Validator<MessageAddress> {
	...
	public ValidationResult<MessageAddress> validate(RosettaPath path, MessageAddress o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MessageAddress", ValidationType.TYPE_FORMAT, "MessageAddress", path, "", error);
		}
		return success("MessageAddress", ValidationType.TYPE_FORMAT, "MessageAddress", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MessageAddress", ValidationType.TYPE_FORMAT, "MessageAddress", path, "", res.getError());
				}
				return success("MessageAddress", ValidationType.TYPE_FORMAT, "MessageAddress", path, "");
			})
			.collect(toList());
