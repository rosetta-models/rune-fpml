...
public class CommodityPhysicalQuantityScheduleValidator implements Validator<CommodityPhysicalQuantitySchedule> {
	...
	public ValidationResult<CommodityPhysicalQuantitySchedule> validate(RosettaPath path, CommodityPhysicalQuantitySchedule o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityPhysicalQuantitySchedule", ValidationType.CARDINALITY, "CommodityPhysicalQuantitySchedule", path, "", error);
		}
		return success("CommodityPhysicalQuantitySchedule", ValidationType.CARDINALITY, "CommodityPhysicalQuantitySchedule", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPhysicalQuantitySchedule", ValidationType.CARDINALITY, "CommodityPhysicalQuantitySchedule", path, "", res.getError());
				}
				return success("CommodityPhysicalQuantitySchedule", ValidationType.CARDINALITY, "CommodityPhysicalQuantitySchedule", path, "");
			})
			.collect(toList());
