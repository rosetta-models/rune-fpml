...
public class BusinessCentersReferenceTypeFormatValidator implements Validator<BusinessCentersReference> {
	...
	public ValidationResult<BusinessCentersReference> validate(RosettaPath path, BusinessCentersReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BusinessCentersReference", ValidationType.TYPE_FORMAT, "BusinessCentersReference", path, "", error);
		}
		return success("BusinessCentersReference", ValidationType.TYPE_FORMAT, "BusinessCentersReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BusinessCentersReference", ValidationType.TYPE_FORMAT, "BusinessCentersReference", path, "", res.getError());
				}
				return success("BusinessCentersReference", ValidationType.TYPE_FORMAT, "BusinessCentersReference", path, "");
			})
			.collect(toList());
