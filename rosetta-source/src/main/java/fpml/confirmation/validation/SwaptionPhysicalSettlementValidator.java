...
public class SwaptionPhysicalSettlementValidator implements Validator<SwaptionPhysicalSettlement> {
	...
	private List<ComparisonResult> getComparisonResults(SwaptionPhysicalSettlement o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("clearedPhysicalSettlement", (Boolean) o.getClearedPhysicalSettlement() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<SwaptionPhysicalSettlement> validate(RosettaPath path, SwaptionPhysicalSettlement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SwaptionPhysicalSettlement", ValidationType.CARDINALITY, "SwaptionPhysicalSettlement", path, "", error);
		}
		return success("SwaptionPhysicalSettlement", ValidationType.CARDINALITY, "SwaptionPhysicalSettlement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SwaptionPhysicalSettlement", ValidationType.CARDINALITY, "SwaptionPhysicalSettlement", path, "", res.getError());
				}
				return success("SwaptionPhysicalSettlement", ValidationType.CARDINALITY, "SwaptionPhysicalSettlement", path, "");
			})
			.collect(toList());
