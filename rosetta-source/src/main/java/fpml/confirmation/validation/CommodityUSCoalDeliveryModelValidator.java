...
public class CommodityUSCoalDeliveryModelValidator implements Validator<CommodityUSCoalDeliveryModel> {

	private List<ComparisonResult> getComparisonResults(CommodityUSCoalDeliveryModel o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("quantityVariationAdjustment", (Boolean) o.getQuantityVariationAdjustment() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<CommodityUSCoalDeliveryModel> validate(RosettaPath path, CommodityUSCoalDeliveryModel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityUSCoalDeliveryModel", ValidationType.CARDINALITY, "CommodityUSCoalDeliveryModel", path, "", error);
		}
		return success("CommodityUSCoalDeliveryModel", ValidationType.CARDINALITY, "CommodityUSCoalDeliveryModel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityUSCoalDeliveryModel", ValidationType.CARDINALITY, "CommodityUSCoalDeliveryModel", path, "", res.getError());
				}
				return success("CommodityUSCoalDeliveryModel", ValidationType.CARDINALITY, "CommodityUSCoalDeliveryModel", path, "");
			})
			.collect(toList());
