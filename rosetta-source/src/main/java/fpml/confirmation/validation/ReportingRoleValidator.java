...
public class ReportingRoleValidator implements Validator<ReportingRole> {
	...
	public ValidationResult<ReportingRole> validate(RosettaPath path, ReportingRole o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ReportingRole", ValidationType.CARDINALITY, "ReportingRole", path, "", error);
		}
		return success("ReportingRole", ValidationType.CARDINALITY, "ReportingRole", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportingRole", ValidationType.CARDINALITY, "ReportingRole", path, "", res.getError());
				}
				return success("ReportingRole", ValidationType.CARDINALITY, "ReportingRole", path, "");
			})
			.collect(toList());
