...
public class FacilityRateChangeNotificationValidator implements Validator<FacilityRateChangeNotification> {

	private List<ComparisonResult> getComparisonResults(FacilityRateChangeNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("facilityRateChangeGroup", (AbstractFacilityContractEvent) o.getFacilityRateChangeGroup() != null ? 1 : 0, 1, 1), 
	...
	@Override
	public ValidationResult<FacilityRateChangeNotification> validate(RosettaPath path, FacilityRateChangeNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityRateChangeNotification", ValidationType.CARDINALITY, "FacilityRateChangeNotification", path, "", error);
		}
		return success("FacilityRateChangeNotification", ValidationType.CARDINALITY, "FacilityRateChangeNotification", path, "");
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, FacilityRateChangeNotification o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityRateChangeNotification", ValidationType.CARDINALITY, "FacilityRateChangeNotification", path, "", res.getError());
				}
				return success("FacilityRateChangeNotification", ValidationType.CARDINALITY, "FacilityRateChangeNotification", path, "");
			})
			.collect(toList());
