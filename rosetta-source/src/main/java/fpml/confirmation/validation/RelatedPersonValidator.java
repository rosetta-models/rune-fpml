...
public class RelatedPersonValidator implements Validator<RelatedPerson> {
	...
	public ValidationResult<RelatedPerson> validate(RosettaPath path, RelatedPerson o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RelatedPerson", ValidationType.CARDINALITY, "RelatedPerson", path, "", error);
		}
		return success("RelatedPerson", ValidationType.CARDINALITY, "RelatedPerson", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RelatedPerson", ValidationType.CARDINALITY, "RelatedPerson", path, "", res.getError());
				}
				return success("RelatedPerson", ValidationType.CARDINALITY, "RelatedPerson", path, "");
			})
			.collect(toList());
