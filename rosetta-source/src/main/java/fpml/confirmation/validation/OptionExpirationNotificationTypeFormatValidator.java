...
public class OptionExpirationNotificationTypeFormatValidator implements Validator<OptionExpirationNotification> {
	...
	public ValidationResult<OptionExpirationNotification> validate(RosettaPath path, OptionExpirationNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("OptionExpirationNotification", ValidationType.TYPE_FORMAT, "OptionExpirationNotification", path, "", error);
		}
		return success("OptionExpirationNotification", ValidationType.TYPE_FORMAT, "OptionExpirationNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionExpirationNotification", ValidationType.TYPE_FORMAT, "OptionExpirationNotification", path, "", res.getError());
				}
				return success("OptionExpirationNotification", ValidationType.TYPE_FORMAT, "OptionExpirationNotification", path, "");
			})
			.collect(toList());
