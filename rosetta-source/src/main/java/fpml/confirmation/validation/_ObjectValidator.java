...
public class _ObjectValidator implements Validator<_Object> {
	...
	@Override
	public ValidationResult<_Object> validate(RosettaPath path, _Object o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("_Object", ValidationType.CARDINALITY, "_Object", path, "", error);
		}
		return success("_Object", ValidationType.CARDINALITY, "_Object", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("_Object", ValidationType.CARDINALITY, "_Object", path, "", res.getError());
				}
				return success("_Object", ValidationType.CARDINALITY, "_Object", path, "");
			})
			.collect(toList());
