...
public class QuotationModelValidator implements Validator<QuotationModel> {
	...
	public ValidationResult<QuotationModel> validate(RosettaPath path, QuotationModel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("QuotationModel", ValidationType.CARDINALITY, "QuotationModel", path, "", error);
		}
		return success("QuotationModel", ValidationType.CARDINALITY, "QuotationModel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuotationModel", ValidationType.CARDINALITY, "QuotationModel", path, "", res.getError());
				}
				return success("QuotationModel", ValidationType.CARDINALITY, "QuotationModel", path, "");
			})
			.collect(toList());
