...
public class ConsentAcknowledgementValidator implements Validator<ConsentAcknowledgement> {
	...
	public ValidationResult<ConsentAcknowledgement> validate(RosettaPath path, ConsentAcknowledgement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ConsentAcknowledgement", ValidationType.CARDINALITY, "ConsentAcknowledgement", path, "", error);
		}
		return success("ConsentAcknowledgement", ValidationType.CARDINALITY, "ConsentAcknowledgement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentAcknowledgement", ValidationType.CARDINALITY, "ConsentAcknowledgement", path, "", res.getError());
				}
				return success("ConsentAcknowledgement", ValidationType.CARDINALITY, "ConsentAcknowledgement", path, "");
			})
			.collect(toList());
