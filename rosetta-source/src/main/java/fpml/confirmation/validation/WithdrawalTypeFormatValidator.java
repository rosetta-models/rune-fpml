...
public class WithdrawalTypeFormatValidator implements Validator<Withdrawal> {
	...
	public ValidationResult<Withdrawal> validate(RosettaPath path, Withdrawal o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Withdrawal", ValidationType.TYPE_FORMAT, "Withdrawal", path, "", error);
		}
		return success("Withdrawal", ValidationType.TYPE_FORMAT, "Withdrawal", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Withdrawal", ValidationType.TYPE_FORMAT, "Withdrawal", path, "", res.getError());
				}
				return success("Withdrawal", ValidationType.TYPE_FORMAT, "Withdrawal", path, "");
			})
			.collect(toList());
