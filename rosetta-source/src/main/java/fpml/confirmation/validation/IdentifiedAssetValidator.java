...
public class IdentifiedAssetValidator implements Validator<IdentifiedAsset> {
	...
	@Override
	public ValidationResult<IdentifiedAsset> validate(RosettaPath path, IdentifiedAsset o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("IdentifiedAsset", ValidationType.CARDINALITY, "IdentifiedAsset", path, "", error);
		}
		return success("IdentifiedAsset", ValidationType.CARDINALITY, "IdentifiedAsset", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IdentifiedAsset", ValidationType.CARDINALITY, "IdentifiedAsset", path, "", res.getError());
				}
				return success("IdentifiedAsset", ValidationType.CARDINALITY, "IdentifiedAsset", path, "");
			})
			.collect(toList());
