...
public class ClearingEligibilityTypeFormatValidator implements Validator<ClearingEligibility> {
	...
	public ValidationResult<ClearingEligibility> validate(RosettaPath path, ClearingEligibility o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ClearingEligibility", ValidationType.TYPE_FORMAT, "ClearingEligibility", path, "", error);
		}
		return success("ClearingEligibility", ValidationType.TYPE_FORMAT, "ClearingEligibility", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingEligibility", ValidationType.TYPE_FORMAT, "ClearingEligibility", path, "", res.getError());
				}
				return success("ClearingEligibility", ValidationType.TYPE_FORMAT, "ClearingEligibility", path, "");
			})
			.collect(toList());
