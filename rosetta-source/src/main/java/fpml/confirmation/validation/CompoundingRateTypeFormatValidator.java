...
public class CompoundingRateTypeFormatValidator implements Validator<CompoundingRate> {
	...
	public ValidationResult<CompoundingRate> validate(RosettaPath path, CompoundingRate o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CompoundingRate", ValidationType.TYPE_FORMAT, "CompoundingRate", path, "", error);
		}
		return success("CompoundingRate", ValidationType.TYPE_FORMAT, "CompoundingRate", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CompoundingRate", ValidationType.TYPE_FORMAT, "CompoundingRate", path, "", res.getError());
				}
				return success("CompoundingRate", ValidationType.TYPE_FORMAT, "CompoundingRate", path, "");
			})
			.collect(toList());
