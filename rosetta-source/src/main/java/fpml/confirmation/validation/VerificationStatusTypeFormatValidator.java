...
public class VerificationStatusTypeFormatValidator implements Validator<VerificationStatus> {
	...
	public ValidationResult<VerificationStatus> validate(RosettaPath path, VerificationStatus o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("VerificationStatus", ValidationType.TYPE_FORMAT, "VerificationStatus", path, "", error);
		}
		return success("VerificationStatus", ValidationType.TYPE_FORMAT, "VerificationStatus", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VerificationStatus", ValidationType.TYPE_FORMAT, "VerificationStatus", path, "", res.getError());
				}
				return success("VerificationStatus", ValidationType.TYPE_FORMAT, "VerificationStatus", path, "");
			})
			.collect(toList());
