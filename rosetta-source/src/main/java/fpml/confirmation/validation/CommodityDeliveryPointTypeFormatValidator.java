...
public class CommodityDeliveryPointTypeFormatValidator implements Validator<CommodityDeliveryPoint> {
	...
	public ValidationResult<CommodityDeliveryPoint> validate(RosettaPath path, CommodityDeliveryPoint o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityDeliveryPoint", ValidationType.TYPE_FORMAT, "CommodityDeliveryPoint", path, "", error);
		}
		return success("CommodityDeliveryPoint", ValidationType.TYPE_FORMAT, "CommodityDeliveryPoint", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityDeliveryPoint", ValidationType.TYPE_FORMAT, "CommodityDeliveryPoint", path, "", res.getError());
				}
				return success("CommodityDeliveryPoint", ValidationType.TYPE_FORMAT, "CommodityDeliveryPoint", path, "");
			})
			.collect(toList());
