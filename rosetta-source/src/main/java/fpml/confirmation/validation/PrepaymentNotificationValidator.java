...
public class PrepaymentNotificationValidator implements Validator<PrepaymentNotification> {

	private List<ComparisonResult> getComparisonResults(PrepaymentNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("prepayment", (LoanPrepayment) o.getPrepayment() != null ? 1 : 0, 1, 1), 
				checkCardinality("facilityDetailsModel", (FacilityDetailsModel) o.getFacilityDetailsModel() != null ? 1 : 0, 1, 1), 
				checkCardinality("loanContractDetailsModel", (LoanContractDetailsModel) o.getLoanContractDetailsModel() != null ? 1 : 0, 1, 1), 
				checkCardinality("party", (List<? extends Party>) o.getParty() == null ? 0 : ((List<? extends Party>) o.getParty()).size(), 1, 0)
			);
	}

	@Override
	public ValidationResult<PrepaymentNotification> validate(RosettaPath path, PrepaymentNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PrepaymentNotification", ValidationType.CARDINALITY, "PrepaymentNotification", path, "", error);
		}
		return success("PrepaymentNotification", ValidationType.CARDINALITY, "PrepaymentNotification", path, "");
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, PrepaymentNotification o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PrepaymentNotification", ValidationType.CARDINALITY, "PrepaymentNotification", path, "", res.getError());
				}
				return success("PrepaymentNotification", ValidationType.CARDINALITY, "PrepaymentNotification", path, "");
			})
			.collect(toList());
