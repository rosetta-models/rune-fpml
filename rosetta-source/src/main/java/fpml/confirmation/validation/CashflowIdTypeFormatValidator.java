...
public class CashflowIdTypeFormatValidator implements Validator<CashflowId> {
	...
	public ValidationResult<CashflowId> validate(RosettaPath path, CashflowId o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CashflowId", ValidationType.TYPE_FORMAT, "CashflowId", path, "", error);
		}
		return success("CashflowId", ValidationType.TYPE_FORMAT, "CashflowId", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashflowId", ValidationType.TYPE_FORMAT, "CashflowId", path, "", res.getError());
				}
				return success("CashflowId", ValidationType.TYPE_FORMAT, "CashflowId", path, "");
			})
			.collect(toList());
