...
public class QuotedAssetSetTypeFormatValidator implements Validator<QuotedAssetSet> {
	...
	public ValidationResult<QuotedAssetSet> validate(RosettaPath path, QuotedAssetSet o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("QuotedAssetSet", ValidationType.TYPE_FORMAT, "QuotedAssetSet", path, "", error);
		}
		return success("QuotedAssetSet", ValidationType.TYPE_FORMAT, "QuotedAssetSet", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuotedAssetSet", ValidationType.TYPE_FORMAT, "QuotedAssetSet", path, "", res.getError());
				}
				return success("QuotedAssetSet", ValidationType.TYPE_FORMAT, "QuotedAssetSet", path, "");
			})
			.collect(toList());
