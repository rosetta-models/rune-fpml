...
public class LoanAllocationConfirmationNotificationValidator implements Validator<LoanAllocationConfirmationNotification> {
	...
	private List<ComparisonResult> getComparisonResults(LoanAllocationConfirmationNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("confirmation", (LoanAllocationEvent) o.getConfirmation() != null ? 1 : 0, 1, 1), 
				checkCardinality("allocationIdentifier", (List<? extends LoanAllocationIdentifier>) o.getAllocationIdentifier() == null ? 0 : ((List<? extends LoanAllocationIdentifier>) o.getAllocationIdentifier()).size(), 1, 0), 
	...
	public ValidationResult<LoanAllocationConfirmationNotification> validate(RosettaPath path, LoanAllocationConfirmationNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanAllocationConfirmationNotification", ValidationType.CARDINALITY, "LoanAllocationConfirmationNotification", path, "", error);
		}
		return success("LoanAllocationConfirmationNotification", ValidationType.CARDINALITY, "LoanAllocationConfirmationNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationConfirmationNotification", ValidationType.CARDINALITY, "LoanAllocationConfirmationNotification", path, "", res.getError());
				}
				return success("LoanAllocationConfirmationNotification", ValidationType.CARDINALITY, "LoanAllocationConfirmationNotification", path, "");
			})
			.collect(toList());
