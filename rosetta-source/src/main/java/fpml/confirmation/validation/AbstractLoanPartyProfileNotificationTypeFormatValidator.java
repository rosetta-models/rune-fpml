...
public class AbstractLoanPartyProfileNotificationTypeFormatValidator implements Validator<AbstractLoanPartyProfileNotification> {
	...
	public ValidationResult<AbstractLoanPartyProfileNotification> validate(RosettaPath path, AbstractLoanPartyProfileNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AbstractLoanPartyProfileNotification", ValidationType.TYPE_FORMAT, "AbstractLoanPartyProfileNotification", path, "", error);
		}
		return success("AbstractLoanPartyProfileNotification", ValidationType.TYPE_FORMAT, "AbstractLoanPartyProfileNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanPartyProfileNotification", ValidationType.TYPE_FORMAT, "AbstractLoanPartyProfileNotification", path, "", res.getError());
				}
				return success("AbstractLoanPartyProfileNotification", ValidationType.TYPE_FORMAT, "AbstractLoanPartyProfileNotification", path, "");
			})
			.collect(toList());
