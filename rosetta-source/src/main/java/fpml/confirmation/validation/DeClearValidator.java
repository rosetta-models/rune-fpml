...
public class DeClearValidator implements Validator<DeClear> {
	...
	public ValidationResult<DeClear> validate(RosettaPath path, DeClear o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DeClear", ValidationType.CARDINALITY, "DeClear", path, "", error);
		}
		return success("DeClear", ValidationType.CARDINALITY, "DeClear", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeClear", ValidationType.CARDINALITY, "DeClear", path, "", res.getError());
				}
				return success("DeClear", ValidationType.CARDINALITY, "DeClear", path, "");
			})
			.collect(toList());
