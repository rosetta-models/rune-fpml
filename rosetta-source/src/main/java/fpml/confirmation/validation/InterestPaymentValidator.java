...
public class InterestPaymentValidator implements Validator<InterestPayment> {
	...
	public ValidationResult<InterestPayment> validate(RosettaPath path, InterestPayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InterestPayment", ValidationType.CARDINALITY, "InterestPayment", path, "", error);
		}
		return success("InterestPayment", ValidationType.CARDINALITY, "InterestPayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestPayment", ValidationType.CARDINALITY, "InterestPayment", path, "", res.getError());
				}
				return success("InterestPayment", ValidationType.CARDINALITY, "InterestPayment", path, "");
			})
			.collect(toList());
