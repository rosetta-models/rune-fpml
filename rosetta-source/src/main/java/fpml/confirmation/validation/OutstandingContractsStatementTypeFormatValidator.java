...
public class OutstandingContractsStatementTypeFormatValidator implements Validator<OutstandingContractsStatement> {
	...
	public ValidationResult<OutstandingContractsStatement> validate(RosettaPath path, OutstandingContractsStatement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("OutstandingContractsStatement", ValidationType.TYPE_FORMAT, "OutstandingContractsStatement", path, "", error);
		}
		return success("OutstandingContractsStatement", ValidationType.TYPE_FORMAT, "OutstandingContractsStatement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OutstandingContractsStatement", ValidationType.TYPE_FORMAT, "OutstandingContractsStatement", path, "", res.getError());
				}
				return success("OutstandingContractsStatement", ValidationType.TYPE_FORMAT, "OutstandingContractsStatement", path, "");
			})
			.collect(toList());
