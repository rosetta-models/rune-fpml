...
public class RequestCollateralAllocationTypeFormatValidator implements Validator<RequestCollateralAllocation> {
	...
	public ValidationResult<RequestCollateralAllocation> validate(RosettaPath path, RequestCollateralAllocation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestCollateralAllocation", ValidationType.TYPE_FORMAT, "RequestCollateralAllocation", path, "", error);
		}
		return success("RequestCollateralAllocation", ValidationType.TYPE_FORMAT, "RequestCollateralAllocation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestCollateralAllocation", ValidationType.TYPE_FORMAT, "RequestCollateralAllocation", path, "", res.getError());
				}
				return success("RequestCollateralAllocation", ValidationType.TYPE_FORMAT, "RequestCollateralAllocation", path, "");
			})
			.collect(toList());
