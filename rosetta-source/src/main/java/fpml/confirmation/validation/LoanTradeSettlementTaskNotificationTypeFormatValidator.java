...
public class LoanTradeSettlementTaskNotificationTypeFormatValidator implements Validator<LoanTradeSettlementTaskNotification> {
	...
	public ValidationResult<LoanTradeSettlementTaskNotification> validate(RosettaPath path, LoanTradeSettlementTaskNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanTradeSettlementTaskNotification", ValidationType.TYPE_FORMAT, "LoanTradeSettlementTaskNotification", path, "", error);
		}
		return success("LoanTradeSettlementTaskNotification", ValidationType.TYPE_FORMAT, "LoanTradeSettlementTaskNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeSettlementTaskNotification", ValidationType.TYPE_FORMAT, "LoanTradeSettlementTaskNotification", path, "", res.getError());
				}
				return success("LoanTradeSettlementTaskNotification", ValidationType.TYPE_FORMAT, "LoanTradeSettlementTaskNotification", path, "");
			})
			.collect(toList());
