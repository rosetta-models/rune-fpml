...
public class CashflowNotionalValidator implements Validator<CashflowNotional> {
	...
	public ValidationResult<CashflowNotional> validate(RosettaPath path, CashflowNotional o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CashflowNotional", ValidationType.CARDINALITY, "CashflowNotional", path, "", error);
		}
		return success("CashflowNotional", ValidationType.CARDINALITY, "CashflowNotional", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashflowNotional", ValidationType.CARDINALITY, "CashflowNotional", path, "", res.getError());
				}
				return success("CashflowNotional", ValidationType.CARDINALITY, "CashflowNotional", path, "");
			})
			.collect(toList());
