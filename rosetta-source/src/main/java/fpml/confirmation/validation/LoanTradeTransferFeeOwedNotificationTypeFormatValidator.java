...
public class LoanTradeTransferFeeOwedNotificationTypeFormatValidator implements Validator<LoanTradeTransferFeeOwedNotification> {
	...
	public ValidationResult<LoanTradeTransferFeeOwedNotification> validate(RosettaPath path, LoanTradeTransferFeeOwedNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanTradeTransferFeeOwedNotification", ValidationType.TYPE_FORMAT, "LoanTradeTransferFeeOwedNotification", path, "", error);
		}
		return success("LoanTradeTransferFeeOwedNotification", ValidationType.TYPE_FORMAT, "LoanTradeTransferFeeOwedNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeTransferFeeOwedNotification", ValidationType.TYPE_FORMAT, "LoanTradeTransferFeeOwedNotification", path, "", res.getError());
				}
				return success("LoanTradeTransferFeeOwedNotification", ValidationType.TYPE_FORMAT, "LoanTradeTransferFeeOwedNotification", path, "");
			})
			.collect(toList());
