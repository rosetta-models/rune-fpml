...
public class CommodityMarketDisruptionTypeFormatValidator implements Validator<CommodityMarketDisruption> {
	...
	public ValidationResult<CommodityMarketDisruption> validate(RosettaPath path, CommodityMarketDisruption o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityMarketDisruption", ValidationType.TYPE_FORMAT, "CommodityMarketDisruption", path, "", error);
		}
		return success("CommodityMarketDisruption", ValidationType.TYPE_FORMAT, "CommodityMarketDisruption", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityMarketDisruption", ValidationType.TYPE_FORMAT, "CommodityMarketDisruption", path, "", res.getError());
				}
				return success("CommodityMarketDisruption", ValidationType.TYPE_FORMAT, "CommodityMarketDisruption", path, "");
			})
			.collect(toList());
