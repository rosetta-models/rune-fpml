...
public class FxSettlementPeriodBarrierValidator implements Validator<FxSettlementPeriodBarrier> {
	...
	public ValidationResult<FxSettlementPeriodBarrier> validate(RosettaPath path, FxSettlementPeriodBarrier o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxSettlementPeriodBarrier", ValidationType.CARDINALITY, "FxSettlementPeriodBarrier", path, "", error);
		}
		return success("FxSettlementPeriodBarrier", ValidationType.CARDINALITY, "FxSettlementPeriodBarrier", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSettlementPeriodBarrier", ValidationType.CARDINALITY, "FxSettlementPeriodBarrier", path, "", res.getError());
				}
				return success("FxSettlementPeriodBarrier", ValidationType.CARDINALITY, "FxSettlementPeriodBarrier", path, "");
			})
			.collect(toList());
