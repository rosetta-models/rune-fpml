...
public class KeyInfoTypeChoiceValidator implements Validator<KeyInfoTypeChoice> {
	...
	public ValidationResult<KeyInfoTypeChoice> validate(RosettaPath path, KeyInfoTypeChoice o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("KeyInfoTypeChoice", ValidationType.CARDINALITY, "KeyInfoTypeChoice", path, "", error);
		}
		return success("KeyInfoTypeChoice", ValidationType.CARDINALITY, "KeyInfoTypeChoice", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("KeyInfoTypeChoice", ValidationType.CARDINALITY, "KeyInfoTypeChoice", path, "", res.getError());
				}
				return success("KeyInfoTypeChoice", ValidationType.CARDINALITY, "KeyInfoTypeChoice", path, "");
			})
			.collect(toList());
