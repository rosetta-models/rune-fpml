...
public class RequestConsentTypeFormatValidator implements Validator<RequestConsent> {
	...
	public ValidationResult<RequestConsent> validate(RosettaPath path, RequestConsent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestConsent", ValidationType.TYPE_FORMAT, "RequestConsent", path, "", error);
		}
		return success("RequestConsent", ValidationType.TYPE_FORMAT, "RequestConsent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConsent", ValidationType.TYPE_FORMAT, "RequestConsent", path, "", res.getError());
				}
				return success("RequestConsent", ValidationType.TYPE_FORMAT, "RequestConsent", path, "");
			})
			.collect(toList());
