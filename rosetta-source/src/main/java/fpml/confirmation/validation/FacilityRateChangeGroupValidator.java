...
public class FacilityRateChangeGroupValidator implements Validator<FacilityRateChangeGroup> {
	...
	public ValidationResult<FacilityRateChangeGroup> validate(RosettaPath path, FacilityRateChangeGroup o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityRateChangeGroup", ValidationType.CARDINALITY, "FacilityRateChangeGroup", path, "", error);
		}
		return success("FacilityRateChangeGroup", ValidationType.CARDINALITY, "FacilityRateChangeGroup", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityRateChangeGroup", ValidationType.CARDINALITY, "FacilityRateChangeGroup", path, "", res.getError());
				}
				return success("FacilityRateChangeGroup", ValidationType.CARDINALITY, "FacilityRateChangeGroup", path, "");
			})
			.collect(toList());
