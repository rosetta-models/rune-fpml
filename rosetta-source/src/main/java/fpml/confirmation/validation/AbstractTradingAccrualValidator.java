...
public class AbstractTradingAccrualValidator implements Validator<AbstractTradingAccrual> {
	...
	public ValidationResult<AbstractTradingAccrual> validate(RosettaPath path, AbstractTradingAccrual o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AbstractTradingAccrual", ValidationType.CARDINALITY, "AbstractTradingAccrual", path, "", error);
		}
		return success("AbstractTradingAccrual", ValidationType.CARDINALITY, "AbstractTradingAccrual", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractTradingAccrual", ValidationType.CARDINALITY, "AbstractTradingAccrual", path, "", res.getError());
				}
				return success("AbstractTradingAccrual", ValidationType.CARDINALITY, "AbstractTradingAccrual", path, "");
			})
			.collect(toList());
