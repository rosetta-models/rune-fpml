...
public class SettlementMethodTypeFormatValidator implements Validator<SettlementMethod> {
	...
	public ValidationResult<SettlementMethod> validate(RosettaPath path, SettlementMethod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SettlementMethod", ValidationType.TYPE_FORMAT, "SettlementMethod", path, "", error);
		}
		return success("SettlementMethod", ValidationType.TYPE_FORMAT, "SettlementMethod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementMethod", ValidationType.TYPE_FORMAT, "SettlementMethod", path, "", res.getError());
				}
				return success("SettlementMethod", ValidationType.TYPE_FORMAT, "SettlementMethod", path, "");
			})
			.collect(toList());
