...
public class DaysModelSequence0TypeFormatValidator implements Validator<DaysModelSequence0> {
	...
	public ValidationResult<DaysModelSequence0> validate(RosettaPath path, DaysModelSequence0 o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DaysModelSequence0", ValidationType.TYPE_FORMAT, "DaysModelSequence0", path, "", error);
		}
		return success("DaysModelSequence0", ValidationType.TYPE_FORMAT, "DaysModelSequence0", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DaysModelSequence0", ValidationType.TYPE_FORMAT, "DaysModelSequence0", path, "", res.getError());
				}
				return success("DaysModelSequence0", ValidationType.TYPE_FORMAT, "DaysModelSequence0", path, "");
			})
			.collect(toList());
