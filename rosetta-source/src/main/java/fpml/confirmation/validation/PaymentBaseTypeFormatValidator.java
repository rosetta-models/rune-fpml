...
public class PaymentBaseTypeFormatValidator implements Validator<PaymentBase> {
	...
	public ValidationResult<PaymentBase> validate(RosettaPath path, PaymentBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PaymentBase", ValidationType.TYPE_FORMAT, "PaymentBase", path, "", error);
		}
		return success("PaymentBase", ValidationType.TYPE_FORMAT, "PaymentBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaymentBase", ValidationType.TYPE_FORMAT, "PaymentBase", path, "", res.getError());
				}
				return success("PaymentBase", ValidationType.TYPE_FORMAT, "PaymentBase", path, "");
			})
			.collect(toList());
