...
public class PartyMessageInformationValidator implements Validator<PartyMessageInformation> {
	...
	public ValidationResult<PartyMessageInformation> validate(RosettaPath path, PartyMessageInformation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PartyMessageInformation", ValidationType.CARDINALITY, "PartyMessageInformation", path, "", error);
		}
		return success("PartyMessageInformation", ValidationType.CARDINALITY, "PartyMessageInformation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyMessageInformation", ValidationType.CARDINALITY, "PartyMessageInformation", path, "", res.getError());
				}
				return success("PartyMessageInformation", ValidationType.CARDINALITY, "PartyMessageInformation", path, "");
			})
			.collect(toList());
