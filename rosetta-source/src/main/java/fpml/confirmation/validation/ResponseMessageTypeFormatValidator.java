...
public class ResponseMessageTypeFormatValidator implements Validator<ResponseMessage> {
	...
	public ValidationResult<ResponseMessage> validate(RosettaPath path, ResponseMessage o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ResponseMessage", ValidationType.TYPE_FORMAT, "ResponseMessage", path, "", error);
		}
		return success("ResponseMessage", ValidationType.TYPE_FORMAT, "ResponseMessage", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResponseMessage", ValidationType.TYPE_FORMAT, "ResponseMessage", path, "", res.getError());
				}
				return success("ResponseMessage", ValidationType.TYPE_FORMAT, "ResponseMessage", path, "");
			})
			.collect(toList());
