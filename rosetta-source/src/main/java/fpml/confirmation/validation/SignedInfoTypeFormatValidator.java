...
public class SignedInfoTypeFormatValidator implements Validator<SignedInfo> {
	...
	public ValidationResult<SignedInfo> validate(RosettaPath path, SignedInfo o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SignedInfo", ValidationType.TYPE_FORMAT, "SignedInfo", path, "", error);
		}
		return success("SignedInfo", ValidationType.TYPE_FORMAT, "SignedInfo", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignedInfo", ValidationType.TYPE_FORMAT, "SignedInfo", path, "", res.getError());
				}
				return success("SignedInfo", ValidationType.TYPE_FORMAT, "SignedInfo", path, "");
			})
			.collect(toList());
