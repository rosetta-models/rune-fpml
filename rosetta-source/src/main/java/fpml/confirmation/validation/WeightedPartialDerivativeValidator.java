...
public class WeightedPartialDerivativeValidator implements Validator<WeightedPartialDerivative> {
	...
	private List<ComparisonResult> getComparisonResults(WeightedPartialDerivative o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("partialDerivativeReference", (PricingParameterDerivativeReference) o.getPartialDerivativeReference() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<WeightedPartialDerivative> validate(RosettaPath path, WeightedPartialDerivative o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("WeightedPartialDerivative", ValidationType.CARDINALITY, "WeightedPartialDerivative", path, "", error);
		}
		return success("WeightedPartialDerivative", ValidationType.CARDINALITY, "WeightedPartialDerivative", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeightedPartialDerivative", ValidationType.CARDINALITY, "WeightedPartialDerivative", path, "", res.getError());
				}
				return success("WeightedPartialDerivative", ValidationType.CARDINALITY, "WeightedPartialDerivative", path, "");
			})
			.collect(toList());
