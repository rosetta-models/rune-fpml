...
public class CommitmentScheduleTypeFormatValidator implements Validator<CommitmentSchedule> {
	...
	public ValidationResult<CommitmentSchedule> validate(RosettaPath path, CommitmentSchedule o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommitmentSchedule", ValidationType.TYPE_FORMAT, "CommitmentSchedule", path, "", error);
		}
		return success("CommitmentSchedule", ValidationType.TYPE_FORMAT, "CommitmentSchedule", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommitmentSchedule", ValidationType.TYPE_FORMAT, "CommitmentSchedule", path, "", res.getError());
				}
				return success("CommitmentSchedule", ValidationType.TYPE_FORMAT, "CommitmentSchedule", path, "");
			})
			.collect(toList());
