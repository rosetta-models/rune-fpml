...
public class DateOffsetTypeFormatValidator implements Validator<DateOffset> {
	...
	public ValidationResult<DateOffset> validate(RosettaPath path, DateOffset o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DateOffset", ValidationType.TYPE_FORMAT, "DateOffset", path, "", error);
		}
		return success("DateOffset", ValidationType.TYPE_FORMAT, "DateOffset", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DateOffset", ValidationType.TYPE_FORMAT, "DateOffset", path, "", res.getError());
				}
				return success("DateOffset", ValidationType.TYPE_FORMAT, "DateOffset", path, "");
			})
			.collect(toList());
