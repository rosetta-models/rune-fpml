...
public class FundingFeePaymentValidator implements Validator<FundingFeePayment> {
	...
	public ValidationResult<FundingFeePayment> validate(RosettaPath path, FundingFeePayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FundingFeePayment", ValidationType.CARDINALITY, "FundingFeePayment", path, "", error);
		}
		return success("FundingFeePayment", ValidationType.CARDINALITY, "FundingFeePayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FundingFeePayment", ValidationType.CARDINALITY, "FundingFeePayment", path, "", res.getError());
				}
				return success("FundingFeePayment", ValidationType.CARDINALITY, "FundingFeePayment", path, "");
			})
			.collect(toList());
