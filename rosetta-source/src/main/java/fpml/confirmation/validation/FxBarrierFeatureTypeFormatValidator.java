...
public class FxBarrierFeatureTypeFormatValidator implements Validator<FxBarrierFeature> {
	...
	public ValidationResult<FxBarrierFeature> validate(RosettaPath path, FxBarrierFeature o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxBarrierFeature", ValidationType.TYPE_FORMAT, "FxBarrierFeature", path, "", error);
		}
		return success("FxBarrierFeature", ValidationType.TYPE_FORMAT, "FxBarrierFeature", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxBarrierFeature", ValidationType.TYPE_FORMAT, "FxBarrierFeature", path, "", res.getError());
				}
				return success("FxBarrierFeature", ValidationType.TYPE_FORMAT, "FxBarrierFeature", path, "");
			})
			.collect(toList());
