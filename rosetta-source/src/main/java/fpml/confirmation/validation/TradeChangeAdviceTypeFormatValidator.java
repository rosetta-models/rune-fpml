...
public class TradeChangeAdviceTypeFormatValidator implements Validator<TradeChangeAdvice> {
	...
	public ValidationResult<TradeChangeAdvice> validate(RosettaPath path, TradeChangeAdvice o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TradeChangeAdvice", ValidationType.TYPE_FORMAT, "TradeChangeAdvice", path, "", error);
		}
		return success("TradeChangeAdvice", ValidationType.TYPE_FORMAT, "TradeChangeAdvice", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdvice", ValidationType.TYPE_FORMAT, "TradeChangeAdvice", path, "", res.getError());
				}
				return success("TradeChangeAdvice", ValidationType.TYPE_FORMAT, "TradeChangeAdvice", path, "");
			})
			.collect(toList());
