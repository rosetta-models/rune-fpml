...
public class RequestConfirmationRetractedValidator implements Validator<RequestConfirmationRetracted> {
	...
	public ValidationResult<RequestConfirmationRetracted> validate(RosettaPath path, RequestConfirmationRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestConfirmationRetracted", ValidationType.CARDINALITY, "RequestConfirmationRetracted", path, "", error);
		}
		return success("RequestConfirmationRetracted", ValidationType.CARDINALITY, "RequestConfirmationRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConfirmationRetracted", ValidationType.CARDINALITY, "RequestConfirmationRetracted", path, "", res.getError());
				}
				return success("RequestConfirmationRetracted", ValidationType.CARDINALITY, "RequestConfirmationRetracted", path, "");
			})
			.collect(toList());
