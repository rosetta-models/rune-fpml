...
public class AllocationExceptionValidator implements Validator<AllocationException> {
	...
	public ValidationResult<AllocationException> validate(RosettaPath path, AllocationException o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AllocationException", ValidationType.CARDINALITY, "AllocationException", path, "", error);
		}
		return success("AllocationException", ValidationType.CARDINALITY, "AllocationException", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationException", ValidationType.CARDINALITY, "AllocationException", path, "", res.getError());
				}
				return success("AllocationException", ValidationType.CARDINALITY, "AllocationException", path, "");
			})
			.collect(toList());
