...
public class RolloverNotificationValidator implements Validator<RolloverNotification> {

	private List<ComparisonResult> getComparisonResults(RolloverNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("rollover", (Rollover) o.getRollover() != null ? 1 : 0, 1, 1), 
	...
	@Override
	public ValidationResult<RolloverNotification> validate(RosettaPath path, RolloverNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RolloverNotification", ValidationType.CARDINALITY, "RolloverNotification", path, "", error);
		}
		return success("RolloverNotification", ValidationType.CARDINALITY, "RolloverNotification", path, "");
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, RolloverNotification o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RolloverNotification", ValidationType.CARDINALITY, "RolloverNotification", path, "", res.getError());
				}
				return success("RolloverNotification", ValidationType.CARDINALITY, "RolloverNotification", path, "");
			})
			.collect(toList());
