...
public class FxDisruptionEventValidator implements Validator<FxDisruptionEvent> {
	...
	public ValidationResult<FxDisruptionEvent> validate(RosettaPath path, FxDisruptionEvent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxDisruptionEvent", ValidationType.CARDINALITY, "FxDisruptionEvent", path, "", error);
		}
		return success("FxDisruptionEvent", ValidationType.CARDINALITY, "FxDisruptionEvent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxDisruptionEvent", ValidationType.CARDINALITY, "FxDisruptionEvent", path, "", res.getError());
				}
				return success("FxDisruptionEvent", ValidationType.CARDINALITY, "FxDisruptionEvent", path, "");
			})
			.collect(toList());
