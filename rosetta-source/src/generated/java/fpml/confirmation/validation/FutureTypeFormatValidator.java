...
public class FutureTypeFormatValidator implements Validator<Future> {
	...
	private List<ComparisonResult> getComparisonResults(Future o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkString("futureContractReference", o.getFutureContractReference(), 0, of(255), empty()), 
				checkNumber("contractYearMonth", o.getContractYearMonth(), empty(), of(0), empty(), empty())
			);
	}
	...
	public ValidationResult<Future> validate(RosettaPath path, Future o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Future", ValidationType.TYPE_FORMAT, "Future", path, "", error);
		}
		return success("Future", ValidationType.TYPE_FORMAT, "Future", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Future", ValidationType.TYPE_FORMAT, "Future", path, "", res.getError());
				}
				return success("Future", ValidationType.TYPE_FORMAT, "Future", path, "");
			})
			.collect(toList());
