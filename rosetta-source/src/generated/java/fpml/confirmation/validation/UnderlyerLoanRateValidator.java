...
public class UnderlyerLoanRateValidator implements Validator<UnderlyerLoanRate> {
	...
	private List<ComparisonResult> getComparisonResults(UnderlyerLoanRate o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("stockLoanModel", (StockLoanModel) o.getStockLoanModel() != null ? 1 : 0, 1, 1)
			);
	}
	...
	public ValidationResult<UnderlyerLoanRate> validate(RosettaPath path, UnderlyerLoanRate o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("UnderlyerLoanRate", ValidationType.CARDINALITY, "UnderlyerLoanRate", path, "", error);
		}
		return success("UnderlyerLoanRate", ValidationType.CARDINALITY, "UnderlyerLoanRate", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UnderlyerLoanRate", ValidationType.CARDINALITY, "UnderlyerLoanRate", path, "", res.getError());
				}
				return success("UnderlyerLoanRate", ValidationType.CARDINALITY, "UnderlyerLoanRate", path, "");
			})
			.collect(toList());
