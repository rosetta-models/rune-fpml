...
public class RSAKeyValueTypeFormatValidator implements Validator<RSAKeyValue> {
	...
	public ValidationResult<RSAKeyValue> validate(RosettaPath path, RSAKeyValue o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RSAKeyValue", ValidationType.TYPE_FORMAT, "RSAKeyValue", path, "", error);
		}
		return success("RSAKeyValue", ValidationType.TYPE_FORMAT, "RSAKeyValue", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RSAKeyValue", ValidationType.TYPE_FORMAT, "RSAKeyValue", path, "", res.getError());
				}
				return success("RSAKeyValue", ValidationType.TYPE_FORMAT, "RSAKeyValue", path, "");
			})
			.collect(toList());
