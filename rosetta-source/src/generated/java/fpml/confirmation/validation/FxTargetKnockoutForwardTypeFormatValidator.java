...
public class FxTargetKnockoutForwardTypeFormatValidator implements Validator<FxTargetKnockoutForward> {
	...
	public ValidationResult<FxTargetKnockoutForward> validate(RosettaPath path, FxTargetKnockoutForward o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxTargetKnockoutForward", ValidationType.TYPE_FORMAT, "FxTargetKnockoutForward", path, "", error);
		}
		return success("FxTargetKnockoutForward", ValidationType.TYPE_FORMAT, "FxTargetKnockoutForward", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetKnockoutForward", ValidationType.TYPE_FORMAT, "FxTargetKnockoutForward", path, "", res.getError());
				}
				return success("FxTargetKnockoutForward", ValidationType.TYPE_FORMAT, "FxTargetKnockoutForward", path, "");
			})
			.collect(toList());
