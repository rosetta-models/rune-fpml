...
public class AbstractFacilityEventValidator implements Validator<AbstractFacilityEvent> {
	...
	public ValidationResult<AbstractFacilityEvent> validate(RosettaPath path, AbstractFacilityEvent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AbstractFacilityEvent", ValidationType.CARDINALITY, "AbstractFacilityEvent", path, "", error);
		}
		return success("AbstractFacilityEvent", ValidationType.CARDINALITY, "AbstractFacilityEvent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityEvent", ValidationType.CARDINALITY, "AbstractFacilityEvent", path, "", res.getError());
				}
				return success("AbstractFacilityEvent", ValidationType.CARDINALITY, "AbstractFacilityEvent", path, "");
			})
			.collect(toList());
