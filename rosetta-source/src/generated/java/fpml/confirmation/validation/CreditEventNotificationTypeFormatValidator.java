...
public class CreditEventNotificationTypeFormatValidator implements Validator<CreditEventNotification> {
	...
	public ValidationResult<CreditEventNotification> validate(RosettaPath path, CreditEventNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CreditEventNotification", ValidationType.TYPE_FORMAT, "CreditEventNotification", path, "", error);
		}
		return success("CreditEventNotification", ValidationType.TYPE_FORMAT, "CreditEventNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventNotification", ValidationType.TYPE_FORMAT, "CreditEventNotification", path, "", res.getError());
				}
				return success("CreditEventNotification", ValidationType.TYPE_FORMAT, "CreditEventNotification", path, "");
			})
			.collect(toList());
