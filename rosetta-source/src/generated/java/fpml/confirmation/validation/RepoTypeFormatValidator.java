...
public class RepoTypeFormatValidator implements Validator<Repo> {
	...
	public ValidationResult<Repo> validate(RosettaPath path, Repo o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Repo", ValidationType.TYPE_FORMAT, "Repo", path, "", error);
		}
		return success("Repo", ValidationType.TYPE_FORMAT, "Repo", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Repo", ValidationType.TYPE_FORMAT, "Repo", path, "", res.getError());
				}
				return success("Repo", ValidationType.TYPE_FORMAT, "Repo", path, "");
			})
			.collect(toList());
