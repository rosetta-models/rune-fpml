...
public class LegAmountTypeFormatValidator implements Validator<LegAmount> {
	...
	public ValidationResult<LegAmount> validate(RosettaPath path, LegAmount o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LegAmount", ValidationType.TYPE_FORMAT, "LegAmount", path, "", error);
		}
		return success("LegAmount", ValidationType.TYPE_FORMAT, "LegAmount", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegAmount", ValidationType.TYPE_FORMAT, "LegAmount", path, "", res.getError());
				}
				return success("LegAmount", ValidationType.TYPE_FORMAT, "LegAmount", path, "");
			})
			.collect(toList());
