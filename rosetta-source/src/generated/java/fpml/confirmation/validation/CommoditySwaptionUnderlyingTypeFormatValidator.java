...
public class CommoditySwaptionUnderlyingTypeFormatValidator implements Validator<CommoditySwaptionUnderlying> {
	...
	public ValidationResult<CommoditySwaptionUnderlying> validate(RosettaPath path, CommoditySwaptionUnderlying o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommoditySwaptionUnderlying", ValidationType.TYPE_FORMAT, "CommoditySwaptionUnderlying", path, "", error);
		}
		return success("CommoditySwaptionUnderlying", ValidationType.TYPE_FORMAT, "CommoditySwaptionUnderlying", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySwaptionUnderlying", ValidationType.TYPE_FORMAT, "CommoditySwaptionUnderlying", path, "", res.getError());
				}
				return success("CommoditySwaptionUnderlying", ValidationType.TYPE_FORMAT, "CommoditySwaptionUnderlying", path, "");
			})
			.collect(toList());
