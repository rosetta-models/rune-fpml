...
public class ConsentRefusedTypeFormatValidator implements Validator<ConsentRefused> {
	...
	public ValidationResult<ConsentRefused> validate(RosettaPath path, ConsentRefused o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ConsentRefused", ValidationType.TYPE_FORMAT, "ConsentRefused", path, "", error);
		}
		return success("ConsentRefused", ValidationType.TYPE_FORMAT, "ConsentRefused", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentRefused", ValidationType.TYPE_FORMAT, "ConsentRefused", path, "", res.getError());
				}
				return success("ConsentRefused", ValidationType.TYPE_FORMAT, "ConsentRefused", path, "");
			})
			.collect(toList());
