...
public class ReportingCurrencyTypeTypeFormatValidator implements Validator<ReportingCurrencyType> {
	...
	public ValidationResult<ReportingCurrencyType> validate(RosettaPath path, ReportingCurrencyType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ReportingCurrencyType", ValidationType.TYPE_FORMAT, "ReportingCurrencyType", path, "", error);
		}
		return success("ReportingCurrencyType", ValidationType.TYPE_FORMAT, "ReportingCurrencyType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportingCurrencyType", ValidationType.TYPE_FORMAT, "ReportingCurrencyType", path, "", res.getError());
				}
				return success("ReportingCurrencyType", ValidationType.TYPE_FORMAT, "ReportingCurrencyType", path, "");
			})
			.collect(toList());
