...
public class ValuationDocumentTypeFormatValidator implements Validator<ValuationDocument> {
	...
	public ValidationResult<ValuationDocument> validate(RosettaPath path, ValuationDocument o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ValuationDocument", ValidationType.TYPE_FORMAT, "ValuationDocument", path, "", error);
		}
		return success("ValuationDocument", ValidationType.TYPE_FORMAT, "ValuationDocument", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationDocument", ValidationType.TYPE_FORMAT, "ValuationDocument", path, "", res.getError());
				}
				return success("ValuationDocument", ValidationType.TYPE_FORMAT, "ValuationDocument", path, "");
			})
			.collect(toList());
