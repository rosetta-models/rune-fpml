...
public class PartyPortfolioNameValidator implements Validator<PartyPortfolioName> {
	...
	private List<ComparisonResult> getComparisonResults(PartyPortfolioName o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("portfolioName", (List<? extends PortfolioName>) o.getPortfolioName() == null ? 0 : ((List<? extends PortfolioName>) o.getPortfolioName()).size(), 1, 0), 
	...
	public ValidationResult<PartyPortfolioName> validate(RosettaPath path, PartyPortfolioName o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PartyPortfolioName", ValidationType.CARDINALITY, "PartyPortfolioName", path, "", error);
		}
		return success("PartyPortfolioName", ValidationType.CARDINALITY, "PartyPortfolioName", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyPortfolioName", ValidationType.CARDINALITY, "PartyPortfolioName", path, "", res.getError());
				}
				return success("PartyPortfolioName", ValidationType.CARDINALITY, "PartyPortfolioName", path, "");
			})
			.collect(toList());
