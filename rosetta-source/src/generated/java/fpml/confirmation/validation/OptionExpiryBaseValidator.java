...
public class OptionExpiryBaseValidator implements Validator<OptionExpiryBase> {
	...
	public ValidationResult<OptionExpiryBase> validate(RosettaPath path, OptionExpiryBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("OptionExpiryBase", ValidationType.CARDINALITY, "OptionExpiryBase", path, "", error);
		}
		return success("OptionExpiryBase", ValidationType.CARDINALITY, "OptionExpiryBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionExpiryBase", ValidationType.CARDINALITY, "OptionExpiryBase", path, "", res.getError());
				}
				return success("OptionExpiryBase", ValidationType.CARDINALITY, "OptionExpiryBase", path, "");
			})
			.collect(toList());
