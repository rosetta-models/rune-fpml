...
public class FixedLegValidator implements Validator<FixedLeg> {
	...
	public ValidationResult<FixedLeg> validate(RosettaPath path, FixedLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FixedLeg", ValidationType.CARDINALITY, "FixedLeg", path, "", error);
		}
		return success("FixedLeg", ValidationType.CARDINALITY, "FixedLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedLeg", ValidationType.CARDINALITY, "FixedLeg", path, "", res.getError());
				}
				return success("FixedLeg", ValidationType.CARDINALITY, "FixedLeg", path, "");
			})
			.collect(toList());
