...
public class CreditOptionStrikeValidator implements Validator<CreditOptionStrike> {
	...
	public ValidationResult<CreditOptionStrike> validate(RosettaPath path, CreditOptionStrike o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CreditOptionStrike", ValidationType.CARDINALITY, "CreditOptionStrike", path, "", error);
		}
		return success("CreditOptionStrike", ValidationType.CARDINALITY, "CreditOptionStrike", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditOptionStrike", ValidationType.CARDINALITY, "CreditOptionStrike", path, "", res.getError());
				}
				return success("CreditOptionStrike", ValidationType.CARDINALITY, "CreditOptionStrike", path, "");
			})
			.collect(toList());
