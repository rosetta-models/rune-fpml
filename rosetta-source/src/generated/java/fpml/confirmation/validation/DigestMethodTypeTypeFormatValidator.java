...
public class DigestMethodTypeTypeFormatValidator implements Validator<DigestMethodType> {
	...
	public ValidationResult<DigestMethodType> validate(RosettaPath path, DigestMethodType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DigestMethodType", ValidationType.TYPE_FORMAT, "DigestMethodType", path, "", error);
		}
		return success("DigestMethodType", ValidationType.TYPE_FORMAT, "DigestMethodType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DigestMethodType", ValidationType.TYPE_FORMAT, "DigestMethodType", path, "", res.getError());
				}
				return success("DigestMethodType", ValidationType.TYPE_FORMAT, "DigestMethodType", path, "");
			})
			.collect(toList());
