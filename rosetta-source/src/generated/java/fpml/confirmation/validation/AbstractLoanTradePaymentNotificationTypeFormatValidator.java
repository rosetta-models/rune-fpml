...
public class AbstractLoanTradePaymentNotificationTypeFormatValidator implements Validator<AbstractLoanTradePaymentNotification> {
	...
	public ValidationResult<AbstractLoanTradePaymentNotification> validate(RosettaPath path, AbstractLoanTradePaymentNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AbstractLoanTradePaymentNotification", ValidationType.TYPE_FORMAT, "AbstractLoanTradePaymentNotification", path, "", error);
		}
		return success("AbstractLoanTradePaymentNotification", ValidationType.TYPE_FORMAT, "AbstractLoanTradePaymentNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanTradePaymentNotification", ValidationType.TYPE_FORMAT, "AbstractLoanTradePaymentNotification", path, "", res.getError());
				}
				return success("AbstractLoanTradePaymentNotification", ValidationType.TYPE_FORMAT, "AbstractLoanTradePaymentNotification", path, "");
			})
			.collect(toList());
