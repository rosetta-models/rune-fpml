...
public class GenericFrequencyTypeFormatValidator implements Validator<GenericFrequency> {
	...
	public ValidationResult<GenericFrequency> validate(RosettaPath path, GenericFrequency o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("GenericFrequency", ValidationType.TYPE_FORMAT, "GenericFrequency", path, "", error);
		}
		return success("GenericFrequency", ValidationType.TYPE_FORMAT, "GenericFrequency", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericFrequency", ValidationType.TYPE_FORMAT, "GenericFrequency", path, "", res.getError());
				}
				return success("GenericFrequency", ValidationType.TYPE_FORMAT, "GenericFrequency", path, "");
			})
			.collect(toList());
