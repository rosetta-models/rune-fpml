...
public class KnockOutRateObservationValidator implements Validator<KnockOutRateObservation> {
	...
	public ValidationResult<KnockOutRateObservation> validate(RosettaPath path, KnockOutRateObservation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("KnockOutRateObservation", ValidationType.CARDINALITY, "KnockOutRateObservation", path, "", error);
		}
		return success("KnockOutRateObservation", ValidationType.CARDINALITY, "KnockOutRateObservation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("KnockOutRateObservation", ValidationType.CARDINALITY, "KnockOutRateObservation", path, "", res.getError());
				}
				return success("KnockOutRateObservation", ValidationType.CARDINALITY, "KnockOutRateObservation", path, "");
			})
			.collect(toList());
