...
public class PeriodicPaymentTypeFormatValidator implements Validator<PeriodicPayment> {
	...
	public ValidationResult<PeriodicPayment> validate(RosettaPath path, PeriodicPayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PeriodicPayment", ValidationType.TYPE_FORMAT, "PeriodicPayment", path, "", error);
		}
		return success("PeriodicPayment", ValidationType.TYPE_FORMAT, "PeriodicPayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PeriodicPayment", ValidationType.TYPE_FORMAT, "PeriodicPayment", path, "", res.getError());
				}
				return success("PeriodicPayment", ValidationType.TYPE_FORMAT, "PeriodicPayment", path, "");
			})
			.collect(toList());
