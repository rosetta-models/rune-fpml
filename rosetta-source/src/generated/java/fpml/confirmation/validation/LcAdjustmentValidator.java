...
public class LcAdjustmentValidator implements Validator<LcAdjustment> {
	...
	public ValidationResult<LcAdjustment> validate(RosettaPath path, LcAdjustment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcAdjustment", ValidationType.CARDINALITY, "LcAdjustment", path, "", error);
		}
		return success("LcAdjustment", ValidationType.CARDINALITY, "LcAdjustment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcAdjustment", ValidationType.CARDINALITY, "LcAdjustment", path, "", res.getError());
				}
				return success("LcAdjustment", ValidationType.CARDINALITY, "LcAdjustment", path, "");
			})
			.collect(toList());
