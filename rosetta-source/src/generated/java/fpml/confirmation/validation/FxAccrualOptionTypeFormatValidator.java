...
public class FxAccrualOptionTypeFormatValidator implements Validator<FxAccrualOption> {
	...
	public ValidationResult<FxAccrualOption> validate(RosettaPath path, FxAccrualOption o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxAccrualOption", ValidationType.TYPE_FORMAT, "FxAccrualOption", path, "", error);
		}
		return success("FxAccrualOption", ValidationType.TYPE_FORMAT, "FxAccrualOption", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualOption", ValidationType.TYPE_FORMAT, "FxAccrualOption", path, "", res.getError());
				}
				return success("FxAccrualOption", ValidationType.TYPE_FORMAT, "FxAccrualOption", path, "");
			})
			.collect(toList());
