...
public class LcOptionChangeValidator implements Validator<LcOptionChange> {

	private List<ComparisonResult> getComparisonResults(LcOptionChange o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("lcOption", (LcOption) o.getLcOption() != null ? 1 : 0, 1, 1)
			);
	}

	@Override
	public ValidationResult<LcOptionChange> validate(RosettaPath path, LcOptionChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcOptionChange", ValidationType.CARDINALITY, "LcOptionChange", path, "", error);
		}
		return success("LcOptionChange", ValidationType.CARDINALITY, "LcOptionChange", path, "");
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, LcOptionChange o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcOptionChange", ValidationType.CARDINALITY, "LcOptionChange", path, "", res.getError());
				}
				return success("LcOptionChange", ValidationType.CARDINALITY, "LcOptionChange", path, "");
			})
			.collect(toList());
