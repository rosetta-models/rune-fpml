...
public class ExecutionNotificationValidator implements Validator<ExecutionNotification> {
	...
	public ValidationResult<ExecutionNotification> validate(RosettaPath path, ExecutionNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExecutionNotification", ValidationType.CARDINALITY, "ExecutionNotification", path, "", error);
		}
		return success("ExecutionNotification", ValidationType.CARDINALITY, "ExecutionNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionNotification", ValidationType.CARDINALITY, "ExecutionNotification", path, "", res.getError());
				}
				return success("ExecutionNotification", ValidationType.CARDINALITY, "ExecutionNotification", path, "");
			})
			.collect(toList());
