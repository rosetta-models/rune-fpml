...
public class FeeRateOptionBaseTypeFormatValidator implements Validator<FeeRateOptionBase> {
	...
	public ValidationResult<FeeRateOptionBase> validate(RosettaPath path, FeeRateOptionBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FeeRateOptionBase", ValidationType.TYPE_FORMAT, "FeeRateOptionBase", path, "", error);
		}
		return success("FeeRateOptionBase", ValidationType.TYPE_FORMAT, "FeeRateOptionBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FeeRateOptionBase", ValidationType.TYPE_FORMAT, "FeeRateOptionBase", path, "", res.getError());
				}
				return success("FeeRateOptionBase", ValidationType.TYPE_FORMAT, "FeeRateOptionBase", path, "");
			})
			.collect(toList());
