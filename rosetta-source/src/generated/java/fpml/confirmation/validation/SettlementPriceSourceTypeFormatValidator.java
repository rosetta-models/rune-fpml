...
public class SettlementPriceSourceTypeFormatValidator implements Validator<SettlementPriceSource> {
	...
	public ValidationResult<SettlementPriceSource> validate(RosettaPath path, SettlementPriceSource o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SettlementPriceSource", ValidationType.TYPE_FORMAT, "SettlementPriceSource", path, "", error);
		}
		return success("SettlementPriceSource", ValidationType.TYPE_FORMAT, "SettlementPriceSource", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPriceSource", ValidationType.TYPE_FORMAT, "SettlementPriceSource", path, "", res.getError());
				}
				return success("SettlementPriceSource", ValidationType.TYPE_FORMAT, "SettlementPriceSource", path, "");
			})
			.collect(toList());
