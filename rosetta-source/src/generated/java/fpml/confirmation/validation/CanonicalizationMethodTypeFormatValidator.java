...
public class CanonicalizationMethodTypeFormatValidator implements Validator<CanonicalizationMethod> {
	...
	public ValidationResult<CanonicalizationMethod> validate(RosettaPath path, CanonicalizationMethod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CanonicalizationMethod", ValidationType.TYPE_FORMAT, "CanonicalizationMethod", path, "", error);
		}
		return success("CanonicalizationMethod", ValidationType.TYPE_FORMAT, "CanonicalizationMethod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CanonicalizationMethod", ValidationType.TYPE_FORMAT, "CanonicalizationMethod", path, "", res.getError());
				}
				return success("CanonicalizationMethod", ValidationType.TYPE_FORMAT, "CanonicalizationMethod", path, "");
			})
			.collect(toList());
