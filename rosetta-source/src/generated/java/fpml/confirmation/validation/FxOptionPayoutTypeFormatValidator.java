...
public class FxOptionPayoutTypeFormatValidator implements Validator<FxOptionPayout> {
	...
	public ValidationResult<FxOptionPayout> validate(RosettaPath path, FxOptionPayout o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxOptionPayout", ValidationType.TYPE_FORMAT, "FxOptionPayout", path, "", error);
		}
		return success("FxOptionPayout", ValidationType.TYPE_FORMAT, "FxOptionPayout", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOptionPayout", ValidationType.TYPE_FORMAT, "FxOptionPayout", path, "", res.getError());
				}
				return success("FxOptionPayout", ValidationType.TYPE_FORMAT, "FxOptionPayout", path, "");
			})
			.collect(toList());
