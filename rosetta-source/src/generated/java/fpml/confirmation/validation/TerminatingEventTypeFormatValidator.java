...
public class TerminatingEventTypeFormatValidator implements Validator<TerminatingEvent> {
	...
	public ValidationResult<TerminatingEvent> validate(RosettaPath path, TerminatingEvent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TerminatingEvent", ValidationType.TYPE_FORMAT, "TerminatingEvent", path, "", error);
		}
		return success("TerminatingEvent", ValidationType.TYPE_FORMAT, "TerminatingEvent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TerminatingEvent", ValidationType.TYPE_FORMAT, "TerminatingEvent", path, "", res.getError());
				}
				return success("TerminatingEvent", ValidationType.TYPE_FORMAT, "TerminatingEvent", path, "");
			})
			.collect(toList());
