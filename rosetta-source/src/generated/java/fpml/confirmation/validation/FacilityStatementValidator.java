...
public class FacilityStatementValidator implements Validator<FacilityStatement> {
	...
	public ValidationResult<FacilityStatement> validate(RosettaPath path, FacilityStatement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityStatement", ValidationType.CARDINALITY, "FacilityStatement", path, "", error);
		}
		return success("FacilityStatement", ValidationType.CARDINALITY, "FacilityStatement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityStatement", ValidationType.CARDINALITY, "FacilityStatement", path, "", res.getError());
				}
				return success("FacilityStatement", ValidationType.CARDINALITY, "FacilityStatement", path, "");
			})
			.collect(toList());
