...
public class CollateralAllocationAcceptedTypeFormatValidator implements Validator<CollateralAllocationAccepted> {
	...
	public ValidationResult<CollateralAllocationAccepted> validate(RosettaPath path, CollateralAllocationAccepted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CollateralAllocationAccepted", ValidationType.TYPE_FORMAT, "CollateralAllocationAccepted", path, "", error);
		}
		return success("CollateralAllocationAccepted", ValidationType.TYPE_FORMAT, "CollateralAllocationAccepted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAllocationAccepted", ValidationType.TYPE_FORMAT, "CollateralAllocationAccepted", path, "", res.getError());
				}
				return success("CollateralAllocationAccepted", ValidationType.TYPE_FORMAT, "CollateralAllocationAccepted", path, "");
			})
			.collect(toList());
