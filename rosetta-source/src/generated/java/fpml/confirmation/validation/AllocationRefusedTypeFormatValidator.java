...
public class AllocationRefusedTypeFormatValidator implements Validator<AllocationRefused> {
	...
	public ValidationResult<AllocationRefused> validate(RosettaPath path, AllocationRefused o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AllocationRefused", ValidationType.TYPE_FORMAT, "AllocationRefused", path, "", error);
		}
		return success("AllocationRefused", ValidationType.TYPE_FORMAT, "AllocationRefused", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationRefused", ValidationType.TYPE_FORMAT, "AllocationRefused", path, "", res.getError());
				}
				return success("AllocationRefused", ValidationType.TYPE_FORMAT, "AllocationRefused", path, "");
			})
			.collect(toList());
