...
public class CreditEventExceptionTypeFormatValidator implements Validator<CreditEventException> {
	...
	public ValidationResult<CreditEventException> validate(RosettaPath path, CreditEventException o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CreditEventException", ValidationType.TYPE_FORMAT, "CreditEventException", path, "", error);
		}
		return success("CreditEventException", ValidationType.TYPE_FORMAT, "CreditEventException", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventException", ValidationType.TYPE_FORMAT, "CreditEventException", path, "", res.getError());
				}
				return success("CreditEventException", ValidationType.TYPE_FORMAT, "CreditEventException", path, "");
			})
			.collect(toList());
