...
public class RoutingValidator implements Validator<Routing> {
	...
	public ValidationResult<Routing> validate(RosettaPath path, Routing o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Routing", ValidationType.CARDINALITY, "Routing", path, "", error);
		}
		return success("Routing", ValidationType.CARDINALITY, "Routing", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Routing", ValidationType.CARDINALITY, "Routing", path, "", res.getError());
				}
				return success("Routing", ValidationType.CARDINALITY, "Routing", path, "");
			})
			.collect(toList());
