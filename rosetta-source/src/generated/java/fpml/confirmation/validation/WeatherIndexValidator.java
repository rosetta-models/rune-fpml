...
public class WeatherIndexValidator implements Validator<WeatherIndex> {
	...
	public ValidationResult<WeatherIndex> validate(RosettaPath path, WeatherIndex o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("WeatherIndex", ValidationType.CARDINALITY, "WeatherIndex", path, "", error);
		}
		return success("WeatherIndex", ValidationType.CARDINALITY, "WeatherIndex", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeatherIndex", ValidationType.CARDINALITY, "WeatherIndex", path, "", res.getError());
				}
				return success("WeatherIndex", ValidationType.CARDINALITY, "WeatherIndex", path, "");
			})
			.collect(toList());
