...
public class SensitivityDefinitionTypeFormatValidator implements Validator<SensitivityDefinition> {
	...
	public ValidationResult<SensitivityDefinition> validate(RosettaPath path, SensitivityDefinition o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SensitivityDefinition", ValidationType.TYPE_FORMAT, "SensitivityDefinition", path, "", error);
		}
		return success("SensitivityDefinition", ValidationType.TYPE_FORMAT, "SensitivityDefinition", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SensitivityDefinition", ValidationType.TYPE_FORMAT, "SensitivityDefinition", path, "", res.getError());
				}
				return success("SensitivityDefinition", ValidationType.TYPE_FORMAT, "SensitivityDefinition", path, "");
			})
			.collect(toList());
