...
public class CoalDeliveryValidator implements Validator<CoalDelivery> {
	...
	private List<ComparisonResult> getComparisonResults(CoalDelivery o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("commodityUSCoalDeliveryModel", (CommodityUSCoalDeliveryModel) o.getCommodityUSCoalDeliveryModel() != null ? 1 : 0, 0, 1)
			);
	}
	...
	public ValidationResult<CoalDelivery> validate(RosettaPath path, CoalDelivery o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CoalDelivery", ValidationType.CARDINALITY, "CoalDelivery", path, "", error);
		}
		return success("CoalDelivery", ValidationType.CARDINALITY, "CoalDelivery", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalDelivery", ValidationType.CARDINALITY, "CoalDelivery", path, "", res.getError());
				}
				return success("CoalDelivery", ValidationType.CARDINALITY, "CoalDelivery", path, "");
			})
			.collect(toList());
