...
public class RelevantUnderlyingDateReferenceValidator implements Validator<RelevantUnderlyingDateReference> {
	...
	public ValidationResult<RelevantUnderlyingDateReference> validate(RosettaPath path, RelevantUnderlyingDateReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RelevantUnderlyingDateReference", ValidationType.CARDINALITY, "RelevantUnderlyingDateReference", path, "", error);
		}
		return success("RelevantUnderlyingDateReference", ValidationType.CARDINALITY, "RelevantUnderlyingDateReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RelevantUnderlyingDateReference", ValidationType.CARDINALITY, "RelevantUnderlyingDateReference", path, "", res.getError());
				}
				return success("RelevantUnderlyingDateReference", ValidationType.CARDINALITY, "RelevantUnderlyingDateReference", path, "");
			})
			.collect(toList());
