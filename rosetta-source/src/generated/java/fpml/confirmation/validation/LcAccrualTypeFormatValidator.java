...
public class LcAccrualTypeFormatValidator implements Validator<LcAccrual> {
	...
	public ValidationResult<LcAccrual> validate(RosettaPath path, LcAccrual o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcAccrual", ValidationType.TYPE_FORMAT, "LcAccrual", path, "", error);
		}
		return success("LcAccrual", ValidationType.TYPE_FORMAT, "LcAccrual", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcAccrual", ValidationType.TYPE_FORMAT, "LcAccrual", path, "", res.getError());
				}
				return success("LcAccrual", ValidationType.TYPE_FORMAT, "LcAccrual", path, "");
			})
			.collect(toList());
