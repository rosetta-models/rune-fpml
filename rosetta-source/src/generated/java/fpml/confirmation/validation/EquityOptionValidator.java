...
public class EquityOptionValidator implements Validator<EquityOption> {
	...
	public ValidationResult<EquityOption> validate(RosettaPath path, EquityOption o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EquityOption", ValidationType.CARDINALITY, "EquityOption", path, "", error);
		}
		return success("EquityOption", ValidationType.CARDINALITY, "EquityOption", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityOption", ValidationType.CARDINALITY, "EquityOption", path, "", res.getError());
				}
				return success("EquityOption", ValidationType.CARDINALITY, "EquityOption", path, "");
			})
			.collect(toList());
