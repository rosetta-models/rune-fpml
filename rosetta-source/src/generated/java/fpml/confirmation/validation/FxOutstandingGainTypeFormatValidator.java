...
public class FxOutstandingGainTypeFormatValidator implements Validator<FxOutstandingGain> {
	...
	public ValidationResult<FxOutstandingGain> validate(RosettaPath path, FxOutstandingGain o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxOutstandingGain", ValidationType.TYPE_FORMAT, "FxOutstandingGain", path, "", error);
		}
		return success("FxOutstandingGain", ValidationType.TYPE_FORMAT, "FxOutstandingGain", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOutstandingGain", ValidationType.TYPE_FORMAT, "FxOutstandingGain", path, "", res.getError());
				}
				return success("FxOutstandingGain", ValidationType.TYPE_FORMAT, "FxOutstandingGain", path, "");
			})
			.collect(toList());
