...
public class NoTouchRateObservationTypeFormatValidator implements Validator<NoTouchRateObservation> {
	...
	public ValidationResult<NoTouchRateObservation> validate(RosettaPath path, NoTouchRateObservation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("NoTouchRateObservation", ValidationType.TYPE_FORMAT, "NoTouchRateObservation", path, "", error);
		}
		return success("NoTouchRateObservation", ValidationType.TYPE_FORMAT, "NoTouchRateObservation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NoTouchRateObservation", ValidationType.TYPE_FORMAT, "NoTouchRateObservation", path, "", res.getError());
				}
				return success("NoTouchRateObservation", ValidationType.TYPE_FORMAT, "NoTouchRateObservation", path, "");
			})
			.collect(toList());
