...
public class CollateralAllocationRejectedTypeFormatValidator implements Validator<CollateralAllocationRejected> {
	...
	public ValidationResult<CollateralAllocationRejected> validate(RosettaPath path, CollateralAllocationRejected o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CollateralAllocationRejected", ValidationType.TYPE_FORMAT, "CollateralAllocationRejected", path, "", error);
		}
		return success("CollateralAllocationRejected", ValidationType.TYPE_FORMAT, "CollateralAllocationRejected", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAllocationRejected", ValidationType.TYPE_FORMAT, "CollateralAllocationRejected", path, "", res.getError());
				}
				return success("CollateralAllocationRejected", ValidationType.TYPE_FORMAT, "CollateralAllocationRejected", path, "");
			})
			.collect(toList());
