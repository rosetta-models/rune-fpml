...
public class AllocationReportingStatusTypeFormatValidator implements Validator<AllocationReportingStatus> {
	...
	@Override
	public ValidationResult<AllocationReportingStatus> validate(RosettaPath path, AllocationReportingStatus o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AllocationReportingStatus", ValidationType.TYPE_FORMAT, "AllocationReportingStatus", path, "", error);
		}
		return success("AllocationReportingStatus", ValidationType.TYPE_FORMAT, "AllocationReportingStatus", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationReportingStatus", ValidationType.TYPE_FORMAT, "AllocationReportingStatus", path, "", res.getError());
				}
				return success("AllocationReportingStatus", ValidationType.TYPE_FORMAT, "AllocationReportingStatus", path, "");
			})
			.collect(toList());
