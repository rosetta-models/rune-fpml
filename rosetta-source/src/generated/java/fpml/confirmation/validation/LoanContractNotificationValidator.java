...
public class LoanContractNotificationValidator implements Validator<LoanContractNotification> {
	...
	private List<ComparisonResult> getComparisonResults(LoanContractNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("loanContractEventGroup", (AbstractLoanContractEvent) o.getLoanContractEventGroup() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<LoanContractNotification> validate(RosettaPath path, LoanContractNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanContractNotification", ValidationType.CARDINALITY, "LoanContractNotification", path, "", error);
		}
		return success("LoanContractNotification", ValidationType.CARDINALITY, "LoanContractNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractNotification", ValidationType.CARDINALITY, "LoanContractNotification", path, "", res.getError());
				}
				return success("LoanContractNotification", ValidationType.CARDINALITY, "LoanContractNotification", path, "");
			})
			.collect(toList());
