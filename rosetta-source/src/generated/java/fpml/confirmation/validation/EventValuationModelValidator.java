...
public class EventValuationModelValidator implements Validator<EventValuationModel> {
	...
	public ValidationResult<EventValuationModel> validate(RosettaPath path, EventValuationModel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EventValuationModel", ValidationType.CARDINALITY, "EventValuationModel", path, "", error);
		}
		return success("EventValuationModel", ValidationType.CARDINALITY, "EventValuationModel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventValuationModel", ValidationType.CARDINALITY, "EventValuationModel", path, "", res.getError());
				}
				return success("EventValuationModel", ValidationType.CARDINALITY, "EventValuationModel", path, "");
			})
			.collect(toList());
