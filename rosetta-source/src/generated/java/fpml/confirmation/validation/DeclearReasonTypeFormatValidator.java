...
public class DeclearReasonTypeFormatValidator implements Validator<DeclearReason> {
	...
	public ValidationResult<DeclearReason> validate(RosettaPath path, DeclearReason o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DeclearReason", ValidationType.TYPE_FORMAT, "DeclearReason", path, "", error);
		}
		return success("DeclearReason", ValidationType.TYPE_FORMAT, "DeclearReason", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeclearReason", ValidationType.TYPE_FORMAT, "DeclearReason", path, "", res.getError());
				}
				return success("DeclearReason", ValidationType.TYPE_FORMAT, "DeclearReason", path, "");
			})
			.collect(toList());
