...
public class QuotationCharacteristicsTypeFormatValidator implements Validator<QuotationCharacteristics> {
	...
	public ValidationResult<QuotationCharacteristics> validate(RosettaPath path, QuotationCharacteristics o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("QuotationCharacteristics", ValidationType.TYPE_FORMAT, "QuotationCharacteristics", path, "", error);
		}
		return success("QuotationCharacteristics", ValidationType.TYPE_FORMAT, "QuotationCharacteristics", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuotationCharacteristics", ValidationType.TYPE_FORMAT, "QuotationCharacteristics", path, "", res.getError());
				}
				return success("QuotationCharacteristics", ValidationType.TYPE_FORMAT, "QuotationCharacteristics", path, "");
			})
			.collect(toList());
