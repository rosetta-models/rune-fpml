...
public class InterconnectionPointTypeFormatValidator implements Validator<InterconnectionPoint> {
	...
	public ValidationResult<InterconnectionPoint> validate(RosettaPath path, InterconnectionPoint o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InterconnectionPoint", ValidationType.TYPE_FORMAT, "InterconnectionPoint", path, "", error);
		}
		return success("InterconnectionPoint", ValidationType.TYPE_FORMAT, "InterconnectionPoint", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterconnectionPoint", ValidationType.TYPE_FORMAT, "InterconnectionPoint", path, "", res.getError());
				}
				return success("InterconnectionPoint", ValidationType.TYPE_FORMAT, "InterconnectionPoint", path, "");
			})
			.collect(toList());
