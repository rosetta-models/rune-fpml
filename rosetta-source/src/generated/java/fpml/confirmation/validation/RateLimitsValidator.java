...
public class RateLimitsValidator implements Validator<RateLimits> {
	...
	public ValidationResult<RateLimits> validate(RosettaPath path, RateLimits o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RateLimits", ValidationType.CARDINALITY, "RateLimits", path, "", error);
		}
		return success("RateLimits", ValidationType.CARDINALITY, "RateLimits", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RateLimits", ValidationType.CARDINALITY, "RateLimits", path, "", res.getError());
				}
				return success("RateLimits", ValidationType.CARDINALITY, "RateLimits", path, "");
			})
			.collect(toList());
