...
public class NotificationMessageValidator implements Validator<NotificationMessage> {
	...
	public ValidationResult<NotificationMessage> validate(RosettaPath path, NotificationMessage o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("NotificationMessage", ValidationType.CARDINALITY, "NotificationMessage", path, "", error);
		}
		return success("NotificationMessage", ValidationType.CARDINALITY, "NotificationMessage", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotificationMessage", ValidationType.CARDINALITY, "NotificationMessage", path, "", res.getError());
				}
				return success("NotificationMessage", ValidationType.CARDINALITY, "NotificationMessage", path, "");
			})
			.collect(toList());
