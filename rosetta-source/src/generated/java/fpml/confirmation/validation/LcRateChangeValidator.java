...
public class LcRateChangeValidator implements Validator<LcRateChange> {
	...
	public ValidationResult<LcRateChange> validate(RosettaPath path, LcRateChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcRateChange", ValidationType.CARDINALITY, "LcRateChange", path, "", error);
		}
		return success("LcRateChange", ValidationType.CARDINALITY, "LcRateChange", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcRateChange", ValidationType.CARDINALITY, "LcRateChange", path, "", res.getError());
				}
				return success("LcRateChange", ValidationType.CARDINALITY, "LcRateChange", path, "");
			})
			.collect(toList());
