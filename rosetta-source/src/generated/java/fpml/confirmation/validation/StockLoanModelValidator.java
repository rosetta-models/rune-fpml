...
public class StockLoanModelValidator implements Validator<StockLoanModel> {
	...
	public ValidationResult<StockLoanModel> validate(RosettaPath path, StockLoanModel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("StockLoanModel", ValidationType.CARDINALITY, "StockLoanModel", path, "", error);
		}
		return success("StockLoanModel", ValidationType.CARDINALITY, "StockLoanModel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StockLoanModel", ValidationType.CARDINALITY, "StockLoanModel", path, "", res.getError());
				}
				return success("StockLoanModel", ValidationType.CARDINALITY, "StockLoanModel", path, "");
			})
			.collect(toList());
