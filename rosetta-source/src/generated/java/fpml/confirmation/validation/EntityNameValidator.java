...
public class EntityNameValidator implements Validator<EntityName> {
	...
	public ValidationResult<EntityName> validate(RosettaPath path, EntityName o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EntityName", ValidationType.CARDINALITY, "EntityName", path, "", error);
		}
		return success("EntityName", ValidationType.CARDINALITY, "EntityName", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EntityName", ValidationType.CARDINALITY, "EntityName", path, "", res.getError());
				}
				return success("EntityName", ValidationType.CARDINALITY, "EntityName", path, "");
			})
			.collect(toList());
