...
public class CommodityFixedPriceScheduleTypeFormatValidator implements Validator<CommodityFixedPriceSchedule> {
	...
	public ValidationResult<CommodityFixedPriceSchedule> validate(RosettaPath path, CommodityFixedPriceSchedule o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityFixedPriceSchedule", ValidationType.TYPE_FORMAT, "CommodityFixedPriceSchedule", path, "", error);
		}
		return success("CommodityFixedPriceSchedule", ValidationType.TYPE_FORMAT, "CommodityFixedPriceSchedule", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityFixedPriceSchedule", ValidationType.TYPE_FORMAT, "CommodityFixedPriceSchedule", path, "", res.getError());
				}
				return success("CommodityFixedPriceSchedule", ValidationType.TYPE_FORMAT, "CommodityFixedPriceSchedule", path, "");
			})
			.collect(toList());
