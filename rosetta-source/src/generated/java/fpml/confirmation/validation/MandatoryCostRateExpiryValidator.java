...
public class MandatoryCostRateExpiryValidator implements Validator<MandatoryCostRateExpiry> {
	...
	public ValidationResult<MandatoryCostRateExpiry> validate(RosettaPath path, MandatoryCostRateExpiry o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MandatoryCostRateExpiry", ValidationType.CARDINALITY, "MandatoryCostRateExpiry", path, "", error);
		}
		return success("MandatoryCostRateExpiry", ValidationType.CARDINALITY, "MandatoryCostRateExpiry", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MandatoryCostRateExpiry", ValidationType.CARDINALITY, "MandatoryCostRateExpiry", path, "", res.getError());
				}
				return success("MandatoryCostRateExpiry", ValidationType.CARDINALITY, "MandatoryCostRateExpiry", path, "");
			})
			.collect(toList());
