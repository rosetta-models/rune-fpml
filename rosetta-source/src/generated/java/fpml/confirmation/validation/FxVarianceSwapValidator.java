...
public class FxVarianceSwapValidator implements Validator<FxVarianceSwap> {
	...
	public ValidationResult<FxVarianceSwap> validate(RosettaPath path, FxVarianceSwap o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxVarianceSwap", ValidationType.CARDINALITY, "FxVarianceSwap", path, "", error);
		}
		return success("FxVarianceSwap", ValidationType.CARDINALITY, "FxVarianceSwap", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxVarianceSwap", ValidationType.CARDINALITY, "FxVarianceSwap", path, "", res.getError());
				}
				return success("FxVarianceSwap", ValidationType.CARDINALITY, "FxVarianceSwap", path, "");
			})
			.collect(toList());
