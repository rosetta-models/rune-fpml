...
public class VolatilityAmountTypeFormatValidator implements Validator<VolatilityAmount> {
	...
	public ValidationResult<VolatilityAmount> validate(RosettaPath path, VolatilityAmount o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("VolatilityAmount", ValidationType.TYPE_FORMAT, "VolatilityAmount", path, "", error);
		}
		return success("VolatilityAmount", ValidationType.TYPE_FORMAT, "VolatilityAmount", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilityAmount", ValidationType.TYPE_FORMAT, "VolatilityAmount", path, "", res.getError());
				}
				return success("VolatilityAmount", ValidationType.TYPE_FORMAT, "VolatilityAmount", path, "");
			})
			.collect(toList());
