...
public class LoanAllocationTransferFeeOwedNotificationValidator implements Validator<LoanAllocationTransferFeeOwedNotification> {
	...
	private List<ComparisonResult> getComparisonResults(LoanAllocationTransferFeeOwedNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("feeOwed", (List<? extends LoanAllocationTransferFeeOwedEvent>) o.getFeeOwed() == null ? 0 : ((List<? extends LoanAllocationTransferFeeOwedEvent>) o.getFeeOwed()).size(), 1, 0), 
	...
	public ValidationResult<LoanAllocationTransferFeeOwedNotification> validate(RosettaPath path, LoanAllocationTransferFeeOwedNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanAllocationTransferFeeOwedNotification", ValidationType.CARDINALITY, "LoanAllocationTransferFeeOwedNotification", path, "", error);
		}
		return success("LoanAllocationTransferFeeOwedNotification", ValidationType.CARDINALITY, "LoanAllocationTransferFeeOwedNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationTransferFeeOwedNotification", ValidationType.CARDINALITY, "LoanAllocationTransferFeeOwedNotification", path, "", res.getError());
				}
				return success("LoanAllocationTransferFeeOwedNotification", ValidationType.CARDINALITY, "LoanAllocationTransferFeeOwedNotification", path, "");
			})
			.collect(toList());
