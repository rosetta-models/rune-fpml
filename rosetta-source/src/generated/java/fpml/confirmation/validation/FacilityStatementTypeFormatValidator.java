...
public class FacilityStatementTypeFormatValidator implements Validator<FacilityStatement> {
	...
	@Override
	public ValidationResult<FacilityStatement> validate(RosettaPath path, FacilityStatement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityStatement", ValidationType.TYPE_FORMAT, "FacilityStatement", path, "", error);
		}
		return success("FacilityStatement", ValidationType.TYPE_FORMAT, "FacilityStatement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityStatement", ValidationType.TYPE_FORMAT, "FacilityStatement", path, "", res.getError());
				}
				return success("FacilityStatement", ValidationType.TYPE_FORMAT, "FacilityStatement", path, "");
			})
			.collect(toList());
