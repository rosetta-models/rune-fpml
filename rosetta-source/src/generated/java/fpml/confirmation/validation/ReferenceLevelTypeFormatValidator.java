...
public class ReferenceLevelTypeFormatValidator implements Validator<ReferenceLevel> {
	...
	public ValidationResult<ReferenceLevel> validate(RosettaPath path, ReferenceLevel o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ReferenceLevel", ValidationType.TYPE_FORMAT, "ReferenceLevel", path, "", error);
		}
		return success("ReferenceLevel", ValidationType.TYPE_FORMAT, "ReferenceLevel", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReferenceLevel", ValidationType.TYPE_FORMAT, "ReferenceLevel", path, "", res.getError());
				}
				return success("ReferenceLevel", ValidationType.TYPE_FORMAT, "ReferenceLevel", path, "");
			})
			.collect(toList());
