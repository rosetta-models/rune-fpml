...
public class FxRateObservableReferenceTypeFormatValidator implements Validator<FxRateObservableReference> {
	...
	public ValidationResult<FxRateObservableReference> validate(RosettaPath path, FxRateObservableReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxRateObservableReference", ValidationType.TYPE_FORMAT, "FxRateObservableReference", path, "", error);
		}
		return success("FxRateObservableReference", ValidationType.TYPE_FORMAT, "FxRateObservableReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxRateObservableReference", ValidationType.TYPE_FORMAT, "FxRateObservableReference", path, "", res.getError());
				}
				return success("FxRateObservableReference", ValidationType.TYPE_FORMAT, "FxRateObservableReference", path, "");
			})
			.collect(toList());
