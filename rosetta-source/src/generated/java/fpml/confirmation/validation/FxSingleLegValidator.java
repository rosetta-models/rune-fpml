...
public class FxSingleLegValidator implements Validator<FxSingleLeg> {
	...
	public ValidationResult<FxSingleLeg> validate(RosettaPath path, FxSingleLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxSingleLeg", ValidationType.CARDINALITY, "FxSingleLeg", path, "", error);
		}
		return success("FxSingleLeg", ValidationType.CARDINALITY, "FxSingleLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSingleLeg", ValidationType.CARDINALITY, "FxSingleLeg", path, "", res.getError());
				}
				return success("FxSingleLeg", ValidationType.CARDINALITY, "FxSingleLeg", path, "");
			})
			.collect(toList());
