...
public class FxTargetPhysicalSettlementValidator implements Validator<FxTargetPhysicalSettlement> {
	...
	private List<ComparisonResult> getComparisonResults(FxTargetPhysicalSettlement o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("strikeOrStrikeReferenceModel", (StrikeOrStrikeReferenceModel) o.getStrikeOrStrikeReferenceModel() != null ? 1 : 0, 0, 1)
			);
	}
	...
	public ValidationResult<FxTargetPhysicalSettlement> validate(RosettaPath path, FxTargetPhysicalSettlement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxTargetPhysicalSettlement", ValidationType.CARDINALITY, "FxTargetPhysicalSettlement", path, "", error);
		}
		return success("FxTargetPhysicalSettlement", ValidationType.CARDINALITY, "FxTargetPhysicalSettlement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetPhysicalSettlement", ValidationType.CARDINALITY, "FxTargetPhysicalSettlement", path, "", res.getError());
				}
				return success("FxTargetPhysicalSettlement", ValidationType.CARDINALITY, "FxTargetPhysicalSettlement", path, "");
			})
			.collect(toList());
