...
public class RequestMessageHeaderValidator implements Validator<RequestMessageHeader> {
	...
	public ValidationResult<RequestMessageHeader> validate(RosettaPath path, RequestMessageHeader o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestMessageHeader", ValidationType.CARDINALITY, "RequestMessageHeader", path, "", error);
		}
		return success("RequestMessageHeader", ValidationType.CARDINALITY, "RequestMessageHeader", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestMessageHeader", ValidationType.CARDINALITY, "RequestMessageHeader", path, "", res.getError());
				}
				return success("RequestMessageHeader", ValidationType.CARDINALITY, "RequestMessageHeader", path, "");
			})
			.collect(toList());
