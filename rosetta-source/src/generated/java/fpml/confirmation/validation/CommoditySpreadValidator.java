...
public class CommoditySpreadValidator implements Validator<CommoditySpread> {
	...
	public ValidationResult<CommoditySpread> validate(RosettaPath path, CommoditySpread o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommoditySpread", ValidationType.CARDINALITY, "CommoditySpread", path, "", error);
		}
		return success("CommoditySpread", ValidationType.CARDINALITY, "CommoditySpread", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySpread", ValidationType.CARDINALITY, "CommoditySpread", path, "", res.getError());
				}
				return success("CommoditySpread", ValidationType.CARDINALITY, "CommoditySpread", path, "");
			})
			.collect(toList());
