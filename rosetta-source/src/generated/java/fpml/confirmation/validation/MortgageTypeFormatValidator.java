...
public class MortgageTypeFormatValidator implements Validator<Mortgage> {
	...
	public ValidationResult<Mortgage> validate(RosettaPath path, Mortgage o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Mortgage", ValidationType.TYPE_FORMAT, "Mortgage", path, "", error);
		}
		return success("Mortgage", ValidationType.TYPE_FORMAT, "Mortgage", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Mortgage", ValidationType.TYPE_FORMAT, "Mortgage", path, "", res.getError());
				}
				return success("Mortgage", ValidationType.TYPE_FORMAT, "Mortgage", path, "");
			})
			.collect(toList());
