...
public class MutualFundValidator implements Validator<MutualFund> {
	...
	public ValidationResult<MutualFund> validate(RosettaPath path, MutualFund o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MutualFund", ValidationType.CARDINALITY, "MutualFund", path, "", error);
		}
		return success("MutualFund", ValidationType.CARDINALITY, "MutualFund", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MutualFund", ValidationType.CARDINALITY, "MutualFund", path, "", res.getError());
				}
				return success("MutualFund", ValidationType.CARDINALITY, "MutualFund", path, "");
			})
			.collect(toList());
