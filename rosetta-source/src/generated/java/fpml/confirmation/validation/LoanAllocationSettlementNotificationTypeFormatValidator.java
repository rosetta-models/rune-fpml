...
public class LoanAllocationSettlementNotificationTypeFormatValidator implements Validator<LoanAllocationSettlementNotification> {
	...
	public ValidationResult<LoanAllocationSettlementNotification> validate(RosettaPath path, LoanAllocationSettlementNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanAllocationSettlementNotification", ValidationType.TYPE_FORMAT, "LoanAllocationSettlementNotification", path, "", error);
		}
		return success("LoanAllocationSettlementNotification", ValidationType.TYPE_FORMAT, "LoanAllocationSettlementNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationSettlementNotification", ValidationType.TYPE_FORMAT, "LoanAllocationSettlementNotification", path, "", res.getError());
				}
				return success("LoanAllocationSettlementNotification", ValidationType.TYPE_FORMAT, "LoanAllocationSettlementNotification", path, "");
			})
			.collect(toList());
