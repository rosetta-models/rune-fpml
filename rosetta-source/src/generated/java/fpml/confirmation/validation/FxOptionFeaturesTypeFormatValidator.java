...
public class FxOptionFeaturesTypeFormatValidator implements Validator<FxOptionFeatures> {
	...
	public ValidationResult<FxOptionFeatures> validate(RosettaPath path, FxOptionFeatures o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxOptionFeatures", ValidationType.TYPE_FORMAT, "FxOptionFeatures", path, "", error);
		}
		return success("FxOptionFeatures", ValidationType.TYPE_FORMAT, "FxOptionFeatures", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOptionFeatures", ValidationType.TYPE_FORMAT, "FxOptionFeatures", path, "", res.getError());
				}
				return success("FxOptionFeatures", ValidationType.TYPE_FORMAT, "FxOptionFeatures", path, "");
			})
			.collect(toList());
