...
public class InstrumentTradePricingTypeFormatValidator implements Validator<InstrumentTradePricing> {
	...
	public ValidationResult<InstrumentTradePricing> validate(RosettaPath path, InstrumentTradePricing o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InstrumentTradePricing", ValidationType.TYPE_FORMAT, "InstrumentTradePricing", path, "", error);
		}
		return success("InstrumentTradePricing", ValidationType.TYPE_FORMAT, "InstrumentTradePricing", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InstrumentTradePricing", ValidationType.TYPE_FORMAT, "InstrumentTradePricing", path, "", res.getError());
				}
				return success("InstrumentTradePricing", ValidationType.TYPE_FORMAT, "InstrumentTradePricing", path, "");
			})
			.collect(toList());
