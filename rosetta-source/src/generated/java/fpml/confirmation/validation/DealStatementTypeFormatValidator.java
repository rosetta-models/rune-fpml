...
public class DealStatementTypeFormatValidator implements Validator<DealStatement> {
	...
	public ValidationResult<DealStatement> validate(RosettaPath path, DealStatement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DealStatement", ValidationType.TYPE_FORMAT, "DealStatement", path, "", error);
		}
		return success("DealStatement", ValidationType.TYPE_FORMAT, "DealStatement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DealStatement", ValidationType.TYPE_FORMAT, "DealStatement", path, "", res.getError());
				}
				return success("DealStatement", ValidationType.TYPE_FORMAT, "DealStatement", path, "");
			})
			.collect(toList());
