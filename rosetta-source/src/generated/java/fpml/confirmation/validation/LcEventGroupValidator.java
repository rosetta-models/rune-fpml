...
public class LcEventGroupValidator implements Validator<LcEventGroup> {
	...
	public ValidationResult<LcEventGroup> validate(RosettaPath path, LcEventGroup o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LcEventGroup", ValidationType.CARDINALITY, "LcEventGroup", path, "", error);
		}
		return success("LcEventGroup", ValidationType.CARDINALITY, "LcEventGroup", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcEventGroup", ValidationType.CARDINALITY, "LcEventGroup", path, "", res.getError());
				}
				return success("LcEventGroup", ValidationType.CARDINALITY, "LcEventGroup", path, "");
			})
			.collect(toList());
