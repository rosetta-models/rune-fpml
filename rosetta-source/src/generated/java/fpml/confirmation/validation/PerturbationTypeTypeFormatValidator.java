...
public class PerturbationTypeTypeFormatValidator implements Validator<PerturbationType> {
	...
	public ValidationResult<PerturbationType> validate(RosettaPath path, PerturbationType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PerturbationType", ValidationType.TYPE_FORMAT, "PerturbationType", path, "", error);
		}
		return success("PerturbationType", ValidationType.TYPE_FORMAT, "PerturbationType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PerturbationType", ValidationType.TYPE_FORMAT, "PerturbationType", path, "", res.getError());
				}
				return success("PerturbationType", ValidationType.TYPE_FORMAT, "PerturbationType", path, "");
			})
			.collect(toList());
