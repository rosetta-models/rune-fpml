...
public class AllocationApprovedTypeFormatValidator implements Validator<AllocationApproved> {
	...
	public ValidationResult<AllocationApproved> validate(RosettaPath path, AllocationApproved o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AllocationApproved", ValidationType.TYPE_FORMAT, "AllocationApproved", path, "", error);
		}
		return success("AllocationApproved", ValidationType.TYPE_FORMAT, "AllocationApproved", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationApproved", ValidationType.TYPE_FORMAT, "AllocationApproved", path, "", res.getError());
				}
				return success("AllocationApproved", ValidationType.TYPE_FORMAT, "AllocationApproved", path, "");
			})
			.collect(toList());
