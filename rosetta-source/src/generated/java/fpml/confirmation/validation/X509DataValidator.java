...
public class X509DataValidator implements Validator<X509Data> {
	...
	public ValidationResult<X509Data> validate(RosettaPath path, X509Data o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("X509Data", ValidationType.CARDINALITY, "X509Data", path, "", error);
		}
		return success("X509Data", ValidationType.CARDINALITY, "X509Data", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("X509Data", ValidationType.CARDINALITY, "X509Data", path, "", res.getError());
				}
				return success("X509Data", ValidationType.CARDINALITY, "X509Data", path, "");
			})
			.collect(toList());
