...
public class CommodityPipelineTypeFormatValidator implements Validator<CommodityPipeline> {
	...
	public ValidationResult<CommodityPipeline> validate(RosettaPath path, CommodityPipeline o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityPipeline", ValidationType.TYPE_FORMAT, "CommodityPipeline", path, "", error);
		}
		return success("CommodityPipeline", ValidationType.TYPE_FORMAT, "CommodityPipeline", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPipeline", ValidationType.TYPE_FORMAT, "CommodityPipeline", path, "", res.getError());
				}
				return success("CommodityPipeline", ValidationType.TYPE_FORMAT, "CommodityPipeline", path, "");
			})
			.collect(toList());
