...
public class ManifestValidator implements Validator<Manifest> {
	...
	public ValidationResult<Manifest> validate(RosettaPath path, Manifest o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Manifest", ValidationType.CARDINALITY, "Manifest", path, "", error);
		}
		return success("Manifest", ValidationType.CARDINALITY, "Manifest", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Manifest", ValidationType.CARDINALITY, "Manifest", path, "", res.getError());
				}
				return success("Manifest", ValidationType.CARDINALITY, "Manifest", path, "");
			})
			.collect(toList());
