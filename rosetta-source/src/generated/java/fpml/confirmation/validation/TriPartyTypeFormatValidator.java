...
public class TriPartyTypeFormatValidator implements Validator<TriParty> {
	...
	public ValidationResult<TriParty> validate(RosettaPath path, TriParty o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TriParty", ValidationType.TYPE_FORMAT, "TriParty", path, "", error);
		}
		return success("TriParty", ValidationType.TYPE_FORMAT, "TriParty", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TriParty", ValidationType.TYPE_FORMAT, "TriParty", path, "", res.getError());
				}
				return success("TriParty", ValidationType.TYPE_FORMAT, "TriParty", path, "");
			})
			.collect(toList());
