...
public class PositiveMoneyTypeFormatValidator implements Validator<PositiveMoney> {
	...
	public ValidationResult<PositiveMoney> validate(RosettaPath path, PositiveMoney o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PositiveMoney", ValidationType.TYPE_FORMAT, "PositiveMoney", path, "", error);
		}
		return success("PositiveMoney", ValidationType.TYPE_FORMAT, "PositiveMoney", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PositiveMoney", ValidationType.TYPE_FORMAT, "PositiveMoney", path, "", res.getError());
				}
				return success("PositiveMoney", ValidationType.TYPE_FORMAT, "PositiveMoney", path, "");
			})
			.collect(toList());
