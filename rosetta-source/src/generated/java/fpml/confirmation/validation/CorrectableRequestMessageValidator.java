...
public class CorrectableRequestMessageValidator implements Validator<CorrectableRequestMessage> {
	...
	public ValidationResult<CorrectableRequestMessage> validate(RosettaPath path, CorrectableRequestMessage o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CorrectableRequestMessage", ValidationType.CARDINALITY, "CorrectableRequestMessage", path, "", error);
		}
		return success("CorrectableRequestMessage", ValidationType.CARDINALITY, "CorrectableRequestMessage", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrectableRequestMessage", ValidationType.CARDINALITY, "CorrectableRequestMessage", path, "", res.getError());
				}
				return success("CorrectableRequestMessage", ValidationType.CARDINALITY, "CorrectableRequestMessage", path, "");
			})
			.collect(toList());
