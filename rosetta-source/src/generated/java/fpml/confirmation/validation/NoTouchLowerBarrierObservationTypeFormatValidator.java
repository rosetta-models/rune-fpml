...
public class NoTouchLowerBarrierObservationTypeFormatValidator implements Validator<NoTouchLowerBarrierObservation> {
	...
	public ValidationResult<NoTouchLowerBarrierObservation> validate(RosettaPath path, NoTouchLowerBarrierObservation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("NoTouchLowerBarrierObservation", ValidationType.TYPE_FORMAT, "NoTouchLowerBarrierObservation", path, "", error);
		}
		return success("NoTouchLowerBarrierObservation", ValidationType.TYPE_FORMAT, "NoTouchLowerBarrierObservation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NoTouchLowerBarrierObservation", ValidationType.TYPE_FORMAT, "NoTouchLowerBarrierObservation", path, "", res.getError());
				}
				return success("NoTouchLowerBarrierObservation", ValidationType.TYPE_FORMAT, "NoTouchLowerBarrierObservation", path, "");
			})
			.collect(toList());
