...
public class SettlementInformationValidator implements Validator<SettlementInformation> {
	...
	@Override
	public ValidationResult<SettlementInformation> validate(RosettaPath path, SettlementInformation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SettlementInformation", ValidationType.CARDINALITY, "SettlementInformation", path, "", error);
		}
		return success("SettlementInformation", ValidationType.CARDINALITY, "SettlementInformation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementInformation", ValidationType.CARDINALITY, "SettlementInformation", path, "", res.getError());
				}
				return success("SettlementInformation", ValidationType.CARDINALITY, "SettlementInformation", path, "");
			})
			.collect(toList());
