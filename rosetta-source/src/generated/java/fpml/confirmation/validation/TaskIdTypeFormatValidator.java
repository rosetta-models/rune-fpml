...
public class TaskIdTypeFormatValidator implements Validator<TaskId> {
	...
	private List<ComparisonResult> getComparisonResults(TaskId o) {
		return Lists.<ComparisonResult>newArrayList(
				checkString("taskIdScheme", o.getTaskIdScheme(), 0, empty(), of(Pattern.compile("\\w+:(\\/?\\/?)[^\\s]+")))
			);
	}
	...
	public ValidationResult<TaskId> validate(RosettaPath path, TaskId o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TaskId", ValidationType.TYPE_FORMAT, "TaskId", path, "", error);
		}
		return success("TaskId", ValidationType.TYPE_FORMAT, "TaskId", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TaskId", ValidationType.TYPE_FORMAT, "TaskId", path, "", res.getError());
				}
				return success("TaskId", ValidationType.TYPE_FORMAT, "TaskId", path, "");
			})
			.collect(toList());
