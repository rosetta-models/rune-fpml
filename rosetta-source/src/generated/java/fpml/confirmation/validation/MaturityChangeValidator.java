...
public class MaturityChangeValidator implements Validator<MaturityChange> {
	...
	public ValidationResult<MaturityChange> validate(RosettaPath path, MaturityChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MaturityChange", ValidationType.CARDINALITY, "MaturityChange", path, "", error);
		}
		return success("MaturityChange", ValidationType.CARDINALITY, "MaturityChange", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MaturityChange", ValidationType.CARDINALITY, "MaturityChange", path, "", res.getError());
				}
				return success("MaturityChange", ValidationType.CARDINALITY, "MaturityChange", path, "");
			})
			.collect(toList());
