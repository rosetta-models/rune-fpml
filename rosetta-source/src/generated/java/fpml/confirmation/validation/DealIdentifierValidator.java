...
public class DealIdentifierValidator implements Validator<DealIdentifier> {
	...
	public ValidationResult<DealIdentifier> validate(RosettaPath path, DealIdentifier o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DealIdentifier", ValidationType.CARDINALITY, "DealIdentifier", path, "", error);
		}
		return success("DealIdentifier", ValidationType.CARDINALITY, "DealIdentifier", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DealIdentifier", ValidationType.CARDINALITY, "DealIdentifier", path, "", res.getError());
				}
				return success("DealIdentifier", ValidationType.CARDINALITY, "DealIdentifier", path, "");
			})
			.collect(toList());
