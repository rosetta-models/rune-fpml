...
public class StepTypeFormatValidator implements Validator<Step> {
	...
	public ValidationResult<Step> validate(RosettaPath path, Step o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Step", ValidationType.TYPE_FORMAT, "Step", path, "", error);
		}
		return success("Step", ValidationType.TYPE_FORMAT, "Step", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Step", ValidationType.TYPE_FORMAT, "Step", path, "", res.getError());
				}
				return success("Step", ValidationType.TYPE_FORMAT, "Step", path, "");
			})
			.collect(toList());
