...
public class WeatherStationWBANTypeFormatValidator implements Validator<WeatherStationWBAN> {
	...
	public ValidationResult<WeatherStationWBAN> validate(RosettaPath path, WeatherStationWBAN o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("WeatherStationWBAN", ValidationType.TYPE_FORMAT, "WeatherStationWBAN", path, "", error);
		}
		return success("WeatherStationWBAN", ValidationType.TYPE_FORMAT, "WeatherStationWBAN", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeatherStationWBAN", ValidationType.TYPE_FORMAT, "WeatherStationWBAN", path, "", res.getError());
				}
				return success("WeatherStationWBAN", ValidationType.TYPE_FORMAT, "WeatherStationWBAN", path, "");
			})
			.collect(toList());
