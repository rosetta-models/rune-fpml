...
public class PaymentValidator implements Validator<Payment> {
	...
	public ValidationResult<Payment> validate(RosettaPath path, Payment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Payment", ValidationType.CARDINALITY, "Payment", path, "", error);
		}
		return success("Payment", ValidationType.CARDINALITY, "Payment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Payment", ValidationType.CARDINALITY, "Payment", path, "", res.getError());
				}
				return success("Payment", ValidationType.CARDINALITY, "Payment", path, "");
			})
			.collect(toList());
