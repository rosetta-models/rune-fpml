...
public class ExecutionAdviceRetractedTypeFormatValidator implements Validator<ExecutionAdviceRetracted> {
	...
	public ValidationResult<ExecutionAdviceRetracted> validate(RosettaPath path, ExecutionAdviceRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExecutionAdviceRetracted", ValidationType.TYPE_FORMAT, "ExecutionAdviceRetracted", path, "", error);
		}
		return success("ExecutionAdviceRetracted", ValidationType.TYPE_FORMAT, "ExecutionAdviceRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdviceRetracted", ValidationType.TYPE_FORMAT, "ExecutionAdviceRetracted", path, "", res.getError());
				}
				return success("ExecutionAdviceRetracted", ValidationType.TYPE_FORMAT, "ExecutionAdviceRetracted", path, "");
			})
			.collect(toList());
