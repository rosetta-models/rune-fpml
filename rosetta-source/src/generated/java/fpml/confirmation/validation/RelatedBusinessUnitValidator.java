...
public class RelatedBusinessUnitValidator implements Validator<RelatedBusinessUnit> {
	...
	public ValidationResult<RelatedBusinessUnit> validate(RosettaPath path, RelatedBusinessUnit o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RelatedBusinessUnit", ValidationType.CARDINALITY, "RelatedBusinessUnit", path, "", error);
		}
		return success("RelatedBusinessUnit", ValidationType.CARDINALITY, "RelatedBusinessUnit", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RelatedBusinessUnit", ValidationType.CARDINALITY, "RelatedBusinessUnit", path, "", res.getError());
				}
				return success("RelatedBusinessUnit", ValidationType.CARDINALITY, "RelatedBusinessUnit", path, "");
			})
			.collect(toList());
