...
public class FxFixingScheduleSimpleTypeFormatValidator implements Validator<FxFixingScheduleSimple> {
	...
	public ValidationResult<FxFixingScheduleSimple> validate(RosettaPath path, FxFixingScheduleSimple o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FxFixingScheduleSimple", ValidationType.TYPE_FORMAT, "FxFixingScheduleSimple", path, "", error);
		}
		return success("FxFixingScheduleSimple", ValidationType.TYPE_FORMAT, "FxFixingScheduleSimple", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFixingScheduleSimple", ValidationType.TYPE_FORMAT, "FxFixingScheduleSimple", path, "", res.getError());
				}
				return success("FxFixingScheduleSimple", ValidationType.TYPE_FORMAT, "FxFixingScheduleSimple", path, "");
			})
			.collect(toList());
