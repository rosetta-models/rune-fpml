...
public class GasDeliveryPeriodsTypeFormatValidator implements Validator<GasDeliveryPeriods> {
	...
	public ValidationResult<GasDeliveryPeriods> validate(RosettaPath path, GasDeliveryPeriods o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("GasDeliveryPeriods", ValidationType.TYPE_FORMAT, "GasDeliveryPeriods", path, "", error);
		}
		return success("GasDeliveryPeriods", ValidationType.TYPE_FORMAT, "GasDeliveryPeriods", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GasDeliveryPeriods", ValidationType.TYPE_FORMAT, "GasDeliveryPeriods", path, "", res.getError());
				}
				return success("GasDeliveryPeriods", ValidationType.TYPE_FORMAT, "GasDeliveryPeriods", path, "");
			})
			.collect(toList());
