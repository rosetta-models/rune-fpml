...
public class FxTypeFormatValidator implements Validator<Fx> {
	...
	public ValidationResult<Fx> validate(RosettaPath path, Fx o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Fx", ValidationType.TYPE_FORMAT, "Fx", path, "", error);
		}
		return success("Fx", ValidationType.TYPE_FORMAT, "Fx", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Fx", ValidationType.TYPE_FORMAT, "Fx", path, "", res.getError());
				}
				return success("Fx", ValidationType.TYPE_FORMAT, "Fx", path, "");
			})
			.collect(toList());
