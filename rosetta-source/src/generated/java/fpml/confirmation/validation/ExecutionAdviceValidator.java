...
public class ExecutionAdviceValidator implements Validator<ExecutionAdvice> {
	...
	public ValidationResult<ExecutionAdvice> validate(RosettaPath path, ExecutionAdvice o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExecutionAdvice", ValidationType.CARDINALITY, "ExecutionAdvice", path, "", error);
		}
		return success("ExecutionAdvice", ValidationType.CARDINALITY, "ExecutionAdvice", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdvice", ValidationType.CARDINALITY, "ExecutionAdvice", path, "", res.getError());
				}
				return success("ExecutionAdvice", ValidationType.CARDINALITY, "ExecutionAdvice", path, "");
			})
			.collect(toList());
