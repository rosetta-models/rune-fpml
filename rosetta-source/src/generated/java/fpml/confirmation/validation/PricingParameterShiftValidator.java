...
public class PricingParameterShiftValidator implements Validator<PricingParameterShift> {
	...
	private List<ComparisonResult> getComparisonResults(PricingParameterShift o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("parameterReference", (AssetOrTermPointOrPricingStructureReference) o.getParameterReference() != null ? 1 : 0, 1, 1), 
				checkCardinality("shift", (BigDecimal) o.getShift() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<PricingParameterShift> validate(RosettaPath path, PricingParameterShift o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PricingParameterShift", ValidationType.CARDINALITY, "PricingParameterShift", path, "", error);
		}
		return success("PricingParameterShift", ValidationType.CARDINALITY, "PricingParameterShift", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingParameterShift", ValidationType.CARDINALITY, "PricingParameterShift", path, "", res.getError());
				}
				return success("PricingParameterShift", ValidationType.CARDINALITY, "PricingParameterShift", path, "");
			})
			.collect(toList());
