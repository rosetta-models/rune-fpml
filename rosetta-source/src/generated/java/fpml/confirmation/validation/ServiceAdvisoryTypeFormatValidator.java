...
public class ServiceAdvisoryTypeFormatValidator implements Validator<ServiceAdvisory> {
	...
	public ValidationResult<ServiceAdvisory> validate(RosettaPath path, ServiceAdvisory o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ServiceAdvisory", ValidationType.TYPE_FORMAT, "ServiceAdvisory", path, "", error);
		}
		return success("ServiceAdvisory", ValidationType.TYPE_FORMAT, "ServiceAdvisory", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ServiceAdvisory", ValidationType.TYPE_FORMAT, "ServiceAdvisory", path, "", res.getError());
				}
				return success("ServiceAdvisory", ValidationType.TYPE_FORMAT, "ServiceAdvisory", path, "");
			})
			.collect(toList());
