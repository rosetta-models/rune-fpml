...
public class MaturityNotificationTypeFormatValidator implements Validator<MaturityNotification> {
	...
	public ValidationResult<MaturityNotification> validate(RosettaPath path, MaturityNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MaturityNotification", ValidationType.TYPE_FORMAT, "MaturityNotification", path, "", error);
		}
		return success("MaturityNotification", ValidationType.TYPE_FORMAT, "MaturityNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MaturityNotification", ValidationType.TYPE_FORMAT, "MaturityNotification", path, "", res.getError());
				}
				return success("MaturityNotification", ValidationType.TYPE_FORMAT, "MaturityNotification", path, "");
			})
			.collect(toList());
