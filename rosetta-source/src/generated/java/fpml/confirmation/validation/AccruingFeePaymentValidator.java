...
public class AccruingFeePaymentValidator implements Validator<AccruingFeePayment> {
	...
	private List<ComparisonResult> getComparisonResults(AccruingFeePayment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("accruingFeePaymentSequence", (AccruingFeePaymentSequence) o.getAccruingFeePaymentSequence() != null ? 1 : 0, 0, 1)
			);
	}

	@Override
	public ValidationResult<AccruingFeePayment> validate(RosettaPath path, AccruingFeePayment o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AccruingFeePayment", ValidationType.CARDINALITY, "AccruingFeePayment", path, "", error);
		}
		return success("AccruingFeePayment", ValidationType.CARDINALITY, "AccruingFeePayment", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingFeePayment", ValidationType.CARDINALITY, "AccruingFeePayment", path, "", res.getError());
				}
				return success("AccruingFeePayment", ValidationType.CARDINALITY, "AccruingFeePayment", path, "");
			})
			.collect(toList());
