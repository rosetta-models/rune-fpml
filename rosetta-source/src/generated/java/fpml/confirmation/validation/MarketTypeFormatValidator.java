...
public class MarketTypeFormatValidator implements Validator<Market> {
	...
	public ValidationResult<Market> validate(RosettaPath path, Market o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Market", ValidationType.TYPE_FORMAT, "Market", path, "", error);
		}
		return success("Market", ValidationType.TYPE_FORMAT, "Market", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Market", ValidationType.TYPE_FORMAT, "Market", path, "", res.getError());
				}
				return success("Market", ValidationType.TYPE_FORMAT, "Market", path, "");
			})
			.collect(toList());
