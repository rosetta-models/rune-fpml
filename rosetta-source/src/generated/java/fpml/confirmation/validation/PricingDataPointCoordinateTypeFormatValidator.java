...
public class PricingDataPointCoordinateTypeFormatValidator implements Validator<PricingDataPointCoordinate> {
	...
	public ValidationResult<PricingDataPointCoordinate> validate(RosettaPath path, PricingDataPointCoordinate o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PricingDataPointCoordinate", ValidationType.TYPE_FORMAT, "PricingDataPointCoordinate", path, "", error);
		}
		return success("PricingDataPointCoordinate", ValidationType.TYPE_FORMAT, "PricingDataPointCoordinate", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingDataPointCoordinate", ValidationType.TYPE_FORMAT, "PricingDataPointCoordinate", path, "", res.getError());
				}
				return success("PricingDataPointCoordinate", ValidationType.TYPE_FORMAT, "PricingDataPointCoordinate", path, "");
			})
			.collect(toList());
