...
public class ExecutionRetractedValidator implements Validator<ExecutionRetracted> {
	...
	public ValidationResult<ExecutionRetracted> validate(RosettaPath path, ExecutionRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExecutionRetracted", ValidationType.CARDINALITY, "ExecutionRetracted", path, "", error);
		}
		return success("ExecutionRetracted", ValidationType.CARDINALITY, "ExecutionRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionRetracted", ValidationType.CARDINALITY, "ExecutionRetracted", path, "", res.getError());
				}
				return success("ExecutionRetracted", ValidationType.CARDINALITY, "ExecutionRetracted", path, "");
			})
			.collect(toList());
