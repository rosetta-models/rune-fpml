...
public class MoneyBaseValidator implements Validator<MoneyBase> {
	...
	public ValidationResult<MoneyBase> validate(RosettaPath path, MoneyBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MoneyBase", ValidationType.CARDINALITY, "MoneyBase", path, "", error);
		}
		return success("MoneyBase", ValidationType.CARDINALITY, "MoneyBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MoneyBase", ValidationType.CARDINALITY, "MoneyBase", path, "", res.getError());
				}
				return success("MoneyBase", ValidationType.CARDINALITY, "MoneyBase", path, "");
			})
			.collect(toList());
