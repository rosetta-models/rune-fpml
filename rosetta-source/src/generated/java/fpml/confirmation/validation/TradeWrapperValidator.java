...
public class TradeWrapperValidator implements Validator<TradeWrapper> {
	...
	private List<ComparisonResult> getComparisonResults(TradeWrapper o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("tradeOrInfoModel", (TradeOrInfoModel) o.getTradeOrInfoModel() != null ? 1 : 0, 1, 1)
			);
	}
	...
	public ValidationResult<TradeWrapper> validate(RosettaPath path, TradeWrapper o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TradeWrapper", ValidationType.CARDINALITY, "TradeWrapper", path, "", error);
		}
		return success("TradeWrapper", ValidationType.CARDINALITY, "TradeWrapper", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeWrapper", ValidationType.CARDINALITY, "TradeWrapper", path, "", res.getError());
				}
				return success("TradeWrapper", ValidationType.CARDINALITY, "TradeWrapper", path, "");
			})
			.collect(toList());
