...
public class RateReferenceTypeFormatValidator implements Validator<RateReference> {
	...
	public ValidationResult<RateReference> validate(RosettaPath path, RateReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RateReference", ValidationType.TYPE_FORMAT, "RateReference", path, "", error);
		}
		return success("RateReference", ValidationType.TYPE_FORMAT, "RateReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RateReference", ValidationType.TYPE_FORMAT, "RateReference", path, "", res.getError());
				}
				return success("RateReference", ValidationType.TYPE_FORMAT, "RateReference", path, "");
			})
			.collect(toList());
