...
public class DataProviderTypeFormatValidator implements Validator<DataProvider> {
	...
	public ValidationResult<DataProvider> validate(RosettaPath path, DataProvider o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DataProvider", ValidationType.TYPE_FORMAT, "DataProvider", path, "", error);
		}
		return success("DataProvider", ValidationType.TYPE_FORMAT, "DataProvider", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DataProvider", ValidationType.TYPE_FORMAT, "DataProvider", path, "", res.getError());
				}
				return success("DataProvider", ValidationType.TYPE_FORMAT, "DataProvider", path, "");
			})
			.collect(toList());
