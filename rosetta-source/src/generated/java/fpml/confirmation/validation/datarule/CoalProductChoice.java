...
public interface CoalProductChoice extends Validator<CoalProduct> {
	...
	String DEFINITION = "required choice ^type, coalProductSpecifications";
	...
	class Default implements CoalProductChoice {
		...
		private ComparisonResult executeDataRule(CoalProduct coalProduct) {
			try {
				return choice(MapperS.of(coalProduct), Arrays.asList("type", "coalProductSpecifications"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
