...
public interface InitialMarginCalculationChoice extends Validator<InitialMarginCalculation> {
	...
	String DEFINITION = "required choice initialMarginCalculationSequence0, initialMarginCalculationSequence1";
	...
	class Default implements InitialMarginCalculationChoice {
		...
		private ComparisonResult executeDataRule(InitialMarginCalculation initialMarginCalculation) {
			try {
				return choice(MapperS.of(initialMarginCalculation), Arrays.asList("initialMarginCalculationSequence0", "initialMarginCalculationSequence1"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
