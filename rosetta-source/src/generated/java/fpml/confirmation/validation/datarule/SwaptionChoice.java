...
public interface SwaptionChoice extends Validator<Swaption> {
	...
	ValidationResult<Swaption> validate(RosettaPath path, Swaption swaption);
	
	class Default implements SwaptionChoice {
	
		@Override
		public ValidationResult<Swaption> validate(RosettaPath path, Swaption swaption) {
			ComparisonResult result = executeDataRule(swaption);
			if (result.get()) {
				return ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "Swaption", path, DEFINITION);
			}
			...
			return ValidationResult.failure(NAME, ValidationType.DATA_RULE, "Swaption", path, DEFINITION, failureMessage);
		}
