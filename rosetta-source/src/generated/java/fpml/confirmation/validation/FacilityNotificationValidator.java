...
public class FacilityNotificationValidator implements Validator<FacilityNotification> {
	...
	public ValidationResult<FacilityNotification> validate(RosettaPath path, FacilityNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityNotification", ValidationType.CARDINALITY, "FacilityNotification", path, "", error);
		}
		return success("FacilityNotification", ValidationType.CARDINALITY, "FacilityNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityNotification", ValidationType.CARDINALITY, "FacilityNotification", path, "", res.getError());
				}
				return success("FacilityNotification", ValidationType.CARDINALITY, "FacilityNotification", path, "");
			})
			.collect(toList());
