...
public class ZeroRateCurveValidator implements Validator<ZeroRateCurve> {
	...
	private List<ComparisonResult> getComparisonResults(ZeroRateCurve o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("compoundingFrequency", (CompoundingFrequency) o.getCompoundingFrequency() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<ZeroRateCurve> validate(RosettaPath path, ZeroRateCurve o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ZeroRateCurve", ValidationType.CARDINALITY, "ZeroRateCurve", path, "", error);
		}
		return success("ZeroRateCurve", ValidationType.CARDINALITY, "ZeroRateCurve", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ZeroRateCurve", ValidationType.CARDINALITY, "ZeroRateCurve", path, "", res.getError());
				}
				return success("ZeroRateCurve", ValidationType.CARDINALITY, "ZeroRateCurve", path, "");
			})
			.collect(toList());
