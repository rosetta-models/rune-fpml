...
public class WeatherStationValidator implements Validator<WeatherStation> {
	...
	public ValidationResult<WeatherStation> validate(RosettaPath path, WeatherStation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("WeatherStation", ValidationType.CARDINALITY, "WeatherStation", path, "", error);
		}
		return success("WeatherStation", ValidationType.CARDINALITY, "WeatherStation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeatherStation", ValidationType.CARDINALITY, "WeatherStation", path, "", res.getError());
				}
				return success("WeatherStation", ValidationType.CARDINALITY, "WeatherStation", path, "");
			})
			.collect(toList());
