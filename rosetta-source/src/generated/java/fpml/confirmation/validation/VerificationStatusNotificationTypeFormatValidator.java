...
public class VerificationStatusNotificationTypeFormatValidator implements Validator<VerificationStatusNotification> {
	...
	public ValidationResult<VerificationStatusNotification> validate(RosettaPath path, VerificationStatusNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("VerificationStatusNotification", ValidationType.TYPE_FORMAT, "VerificationStatusNotification", path, "", error);
		}
		return success("VerificationStatusNotification", ValidationType.TYPE_FORMAT, "VerificationStatusNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VerificationStatusNotification", ValidationType.TYPE_FORMAT, "VerificationStatusNotification", path, "", res.getError());
				}
				return success("VerificationStatusNotification", ValidationType.TYPE_FORMAT, "VerificationStatusNotification", path, "");
			})
			.collect(toList());
