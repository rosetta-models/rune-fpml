...
public class CollateralizationTypeTypeFormatValidator implements Validator<CollateralizationType> {
	...
	public ValidationResult<CollateralizationType> validate(RosettaPath path, CollateralizationType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CollateralizationType", ValidationType.TYPE_FORMAT, "CollateralizationType", path, "", error);
		}
		return success("CollateralizationType", ValidationType.TYPE_FORMAT, "CollateralizationType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralizationType", ValidationType.TYPE_FORMAT, "CollateralizationType", path, "", res.getError());
				}
				return success("CollateralizationType", ValidationType.TYPE_FORMAT, "CollateralizationType", path, "");
			})
			.collect(toList());
