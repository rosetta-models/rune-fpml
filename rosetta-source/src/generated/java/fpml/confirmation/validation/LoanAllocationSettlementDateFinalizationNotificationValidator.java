...
public class LoanAllocationSettlementDateFinalizationNotificationValidator implements Validator<LoanAllocationSettlementDateFinalizationNotification> {

	private List<ComparisonResult> getComparisonResults(LoanAllocationSettlementDateFinalizationNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("settlementDateFinalization", (List<? extends LoanSettlementDateFinalizationEvent>) o.getSettlementDateFinalization() == null ? 0 : ((List<? extends LoanSettlementDateFinalizationEvent>) o.getSettlementDateFinalization()).size(), 1, 0), 
	...
	@Override
	public ValidationResult<LoanAllocationSettlementDateFinalizationNotification> validate(RosettaPath path, LoanAllocationSettlementDateFinalizationNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanAllocationSettlementDateFinalizationNotification", ValidationType.CARDINALITY, "LoanAllocationSettlementDateFinalizationNotification", path, "", error);
		}
		return success("LoanAllocationSettlementDateFinalizationNotification", ValidationType.CARDINALITY, "LoanAllocationSettlementDateFinalizationNotification", path, "");
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, LoanAllocationSettlementDateFinalizationNotification o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationSettlementDateFinalizationNotification", ValidationType.CARDINALITY, "LoanAllocationSettlementDateFinalizationNotification", path, "", res.getError());
				}
				return success("LoanAllocationSettlementDateFinalizationNotification", ValidationType.CARDINALITY, "LoanAllocationSettlementDateFinalizationNotification", path, "");
			})
			.collect(toList());
