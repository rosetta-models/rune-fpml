...
public class PortfolioReferenceBaseValidator implements Validator<PortfolioReferenceBase> {
	...
	public ValidationResult<PortfolioReferenceBase> validate(RosettaPath path, PortfolioReferenceBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PortfolioReferenceBase", ValidationType.CARDINALITY, "PortfolioReferenceBase", path, "", error);
		}
		return success("PortfolioReferenceBase", ValidationType.CARDINALITY, "PortfolioReferenceBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PortfolioReferenceBase", ValidationType.CARDINALITY, "PortfolioReferenceBase", path, "", res.getError());
				}
				return success("PortfolioReferenceBase", ValidationType.CARDINALITY, "PortfolioReferenceBase", path, "");
			})
			.collect(toList());
