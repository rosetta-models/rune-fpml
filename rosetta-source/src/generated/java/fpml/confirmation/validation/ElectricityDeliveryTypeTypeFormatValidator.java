...
public class ElectricityDeliveryTypeTypeFormatValidator implements Validator<ElectricityDeliveryType> {
	...
	public ValidationResult<ElectricityDeliveryType> validate(RosettaPath path, ElectricityDeliveryType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ElectricityDeliveryType", ValidationType.TYPE_FORMAT, "ElectricityDeliveryType", path, "", error);
		}
		return success("ElectricityDeliveryType", ValidationType.TYPE_FORMAT, "ElectricityDeliveryType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ElectricityDeliveryType", ValidationType.TYPE_FORMAT, "ElectricityDeliveryType", path, "", res.getError());
				}
				return success("ElectricityDeliveryType", ValidationType.TYPE_FORMAT, "ElectricityDeliveryType", path, "");
			})
			.collect(toList());
