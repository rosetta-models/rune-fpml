...
public class ReportingBooleanTypeFormatValidator implements Validator<ReportingBoolean> {
	...
	public ValidationResult<ReportingBoolean> validate(RosettaPath path, ReportingBoolean o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ReportingBoolean", ValidationType.TYPE_FORMAT, "ReportingBoolean", path, "", error);
		}
		return success("ReportingBoolean", ValidationType.TYPE_FORMAT, "ReportingBoolean", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportingBoolean", ValidationType.TYPE_FORMAT, "ReportingBoolean", path, "", res.getError());
				}
				return success("ReportingBoolean", ValidationType.TYPE_FORMAT, "ReportingBoolean", path, "");
			})
			.collect(toList());
