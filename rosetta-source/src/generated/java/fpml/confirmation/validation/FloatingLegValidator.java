...
public class FloatingLegValidator implements Validator<FloatingLeg> {
	...
	@Override
	public ValidationResult<FloatingLeg> validate(RosettaPath path, FloatingLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FloatingLeg", ValidationType.CARDINALITY, "FloatingLeg", path, "", error);
		}
		return success("FloatingLeg", ValidationType.CARDINALITY, "FloatingLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingLeg", ValidationType.CARDINALITY, "FloatingLeg", path, "", res.getError());
				}
				return success("FloatingLeg", ValidationType.CARDINALITY, "FloatingLeg", path, "");
			})
			.collect(toList());
