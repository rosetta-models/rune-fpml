...
public class KeyValueValidator implements Validator<KeyValue> {
	...
	public ValidationResult<KeyValue> validate(RosettaPath path, KeyValue o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("KeyValue", ValidationType.CARDINALITY, "KeyValue", path, "", error);
		}
		return success("KeyValue", ValidationType.CARDINALITY, "KeyValue", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("KeyValue", ValidationType.CARDINALITY, "KeyValue", path, "", res.getError());
				}
				return success("KeyValue", ValidationType.CARDINALITY, "KeyValue", path, "");
			})
			.collect(toList());
