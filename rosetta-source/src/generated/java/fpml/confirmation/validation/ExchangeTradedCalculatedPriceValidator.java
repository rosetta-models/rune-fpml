...
public class ExchangeTradedCalculatedPriceValidator implements Validator<ExchangeTradedCalculatedPrice> {
	...
	public ValidationResult<ExchangeTradedCalculatedPrice> validate(RosettaPath path, ExchangeTradedCalculatedPrice o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExchangeTradedCalculatedPrice", ValidationType.CARDINALITY, "ExchangeTradedCalculatedPrice", path, "", error);
		}
		return success("ExchangeTradedCalculatedPrice", ValidationType.CARDINALITY, "ExchangeTradedCalculatedPrice", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTradedCalculatedPrice", ValidationType.CARDINALITY, "ExchangeTradedCalculatedPrice", path, "", res.getError());
				}
				return success("ExchangeTradedCalculatedPrice", ValidationType.CARDINALITY, "ExchangeTradedCalculatedPrice", path, "");
			})
			.collect(toList());
