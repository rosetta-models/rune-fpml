...
public class DealSummaryTypeFormatValidator implements Validator<DealSummary> {
	...
	public ValidationResult<DealSummary> validate(RosettaPath path, DealSummary o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DealSummary", ValidationType.TYPE_FORMAT, "DealSummary", path, "", error);
		}
		return success("DealSummary", ValidationType.TYPE_FORMAT, "DealSummary", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DealSummary", ValidationType.TYPE_FORMAT, "DealSummary", path, "", res.getError());
				}
				return success("DealSummary", ValidationType.TYPE_FORMAT, "DealSummary", path, "");
			})
			.collect(toList());
