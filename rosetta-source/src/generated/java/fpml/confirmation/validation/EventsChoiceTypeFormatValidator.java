...
public class EventsChoiceTypeFormatValidator implements Validator<EventsChoice> {
	...
	public ValidationResult<EventsChoice> validate(RosettaPath path, EventsChoice o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EventsChoice", ValidationType.TYPE_FORMAT, "EventsChoice", path, "", error);
		}
		return success("EventsChoice", ValidationType.TYPE_FORMAT, "EventsChoice", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventsChoice", ValidationType.TYPE_FORMAT, "EventsChoice", path, "", res.getError());
				}
				return success("EventsChoice", ValidationType.TYPE_FORMAT, "EventsChoice", path, "");
			})
			.collect(toList());
