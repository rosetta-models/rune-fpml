...
public class RateIndexValidator implements Validator<RateIndex> {
	...
	public ValidationResult<RateIndex> validate(RosettaPath path, RateIndex o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RateIndex", ValidationType.CARDINALITY, "RateIndex", path, "", error);
		}
		return success("RateIndex", ValidationType.CARDINALITY, "RateIndex", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RateIndex", ValidationType.CARDINALITY, "RateIndex", path, "", res.getError());
				}
				return success("RateIndex", ValidationType.CARDINALITY, "RateIndex", path, "");
			})
			.collect(toList());
