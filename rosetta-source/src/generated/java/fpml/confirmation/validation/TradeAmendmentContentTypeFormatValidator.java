...
public class TradeAmendmentContentTypeFormatValidator implements Validator<TradeAmendmentContent> {
	...
	public ValidationResult<TradeAmendmentContent> validate(RosettaPath path, TradeAmendmentContent o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TradeAmendmentContent", ValidationType.TYPE_FORMAT, "TradeAmendmentContent", path, "", error);
		}
		return success("TradeAmendmentContent", ValidationType.TYPE_FORMAT, "TradeAmendmentContent", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeAmendmentContent", ValidationType.TYPE_FORMAT, "TradeAmendmentContent", path, "", res.getError());
				}
				return success("TradeAmendmentContent", ValidationType.TYPE_FORMAT, "TradeAmendmentContent", path, "");
			})
			.collect(toList());
