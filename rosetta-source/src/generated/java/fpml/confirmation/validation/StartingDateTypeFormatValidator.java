...
public class StartingDateTypeFormatValidator implements Validator<StartingDate> {
	...
	public ValidationResult<StartingDate> validate(RosettaPath path, StartingDate o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("StartingDate", ValidationType.TYPE_FORMAT, "StartingDate", path, "", error);
		}
		return success("StartingDate", ValidationType.TYPE_FORMAT, "StartingDate", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StartingDate", ValidationType.TYPE_FORMAT, "StartingDate", path, "", res.getError());
				}
				return success("StartingDate", ValidationType.TYPE_FORMAT, "StartingDate", path, "");
			})
			.collect(toList());
