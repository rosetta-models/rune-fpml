...
public class TradeChangeAdviceExceptionValidator implements Validator<TradeChangeAdviceException> {
	...
	public ValidationResult<TradeChangeAdviceException> validate(RosettaPath path, TradeChangeAdviceException o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TradeChangeAdviceException", ValidationType.CARDINALITY, "TradeChangeAdviceException", path, "", error);
		}
		return success("TradeChangeAdviceException", ValidationType.CARDINALITY, "TradeChangeAdviceException", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdviceException", ValidationType.CARDINALITY, "TradeChangeAdviceException", path, "", res.getError());
				}
				return success("TradeChangeAdviceException", ValidationType.CARDINALITY, "TradeChangeAdviceException", path, "");
			})
			.collect(toList());
