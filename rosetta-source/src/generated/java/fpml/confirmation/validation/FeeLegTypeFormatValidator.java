...
public class FeeLegTypeFormatValidator implements Validator<FeeLeg> {
	...
	public ValidationResult<FeeLeg> validate(RosettaPath path, FeeLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FeeLeg", ValidationType.TYPE_FORMAT, "FeeLeg", path, "", error);
		}
		return success("FeeLeg", ValidationType.TYPE_FORMAT, "FeeLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FeeLeg", ValidationType.TYPE_FORMAT, "FeeLeg", path, "", res.getError());
				}
				return success("FeeLeg", ValidationType.TYPE_FORMAT, "FeeLeg", path, "");
			})
			.collect(toList());
