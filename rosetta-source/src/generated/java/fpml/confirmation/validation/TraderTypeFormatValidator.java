...
public class TraderTypeFormatValidator implements Validator<Trader> {
	...
	public ValidationResult<Trader> validate(RosettaPath path, Trader o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Trader", ValidationType.TYPE_FORMAT, "Trader", path, "", error);
		}
		return success("Trader", ValidationType.TYPE_FORMAT, "Trader", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Trader", ValidationType.TYPE_FORMAT, "Trader", path, "", res.getError());
				}
				return success("Trader", ValidationType.TYPE_FORMAT, "Trader", path, "");
			})
			.collect(toList());
