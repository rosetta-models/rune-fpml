...
public class RequestConfirmationTypeFormatValidator implements Validator<RequestConfirmation> {
	...
	public ValidationResult<RequestConfirmation> validate(RosettaPath path, RequestConfirmation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestConfirmation", ValidationType.TYPE_FORMAT, "RequestConfirmation", path, "", error);
		}
		return success("RequestConfirmation", ValidationType.TYPE_FORMAT, "RequestConfirmation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConfirmation", ValidationType.TYPE_FORMAT, "RequestConfirmation", path, "", res.getError());
				}
				return success("RequestConfirmation", ValidationType.TYPE_FORMAT, "RequestConfirmation", path, "");
			})
			.collect(toList());
