...
public class RequestExecutionValidator implements Validator<RequestExecution> {
	...
	public ValidationResult<RequestExecution> validate(RosettaPath path, RequestExecution o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestExecution", ValidationType.CARDINALITY, "RequestExecution", path, "", error);
		}
		return success("RequestExecution", ValidationType.CARDINALITY, "RequestExecution", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestExecution", ValidationType.CARDINALITY, "RequestExecution", path, "", res.getError());
				}
				return success("RequestExecution", ValidationType.CARDINALITY, "RequestExecution", path, "");
			})
			.collect(toList());
