...
public class PhysicalSwapLegTypeFormatValidator implements Validator<PhysicalSwapLeg> {
	...
	public ValidationResult<PhysicalSwapLeg> validate(RosettaPath path, PhysicalSwapLeg o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PhysicalSwapLeg", ValidationType.TYPE_FORMAT, "PhysicalSwapLeg", path, "", error);
		}
		return success("PhysicalSwapLeg", ValidationType.TYPE_FORMAT, "PhysicalSwapLeg", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PhysicalSwapLeg", ValidationType.TYPE_FORMAT, "PhysicalSwapLeg", path, "", res.getError());
				}
				return success("PhysicalSwapLeg", ValidationType.TYPE_FORMAT, "PhysicalSwapLeg", path, "");
			})
			.collect(toList());
