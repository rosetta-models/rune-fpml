...
public class LoanTradeNotificationValidator implements Validator<LoanTradeNotification> {
	...
	private List<ComparisonResult> getComparisonResults(LoanTradeNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("tradeEvent", (LoanTradeEvent) o.getTradeEvent() != null ? 1 : 0, 1, 1), 
				checkCardinality("trade", (LoanTrade) o.getTrade() != null ? 1 : 0, 1, 1), 
	...
	public ValidationResult<LoanTradeNotification> validate(RosettaPath path, LoanTradeNotification o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LoanTradeNotification", ValidationType.CARDINALITY, "LoanTradeNotification", path, "", error);
		}
		return success("LoanTradeNotification", ValidationType.CARDINALITY, "LoanTradeNotification", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeNotification", ValidationType.CARDINALITY, "LoanTradeNotification", path, "", res.getError());
				}
				return success("LoanTradeNotification", ValidationType.CARDINALITY, "LoanTradeNotification", path, "");
			})
			.collect(toList());
