...
public class CommitmentChangeTypeFormatValidator implements Validator<CommitmentChange> {
	...
	public ValidationResult<CommitmentChange> validate(RosettaPath path, CommitmentChange o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommitmentChange", ValidationType.TYPE_FORMAT, "CommitmentChange", path, "", error);
		}
		return success("CommitmentChange", ValidationType.TYPE_FORMAT, "CommitmentChange", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommitmentChange", ValidationType.TYPE_FORMAT, "CommitmentChange", path, "", res.getError());
				}
				return success("CommitmentChange", ValidationType.TYPE_FORMAT, "CommitmentChange", path, "");
			})
			.collect(toList());
