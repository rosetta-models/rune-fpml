...
public class TaxFormTypeTypeFormatValidator implements Validator<TaxFormType> {
	...
	public ValidationResult<TaxFormType> validate(RosettaPath path, TaxFormType o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TaxFormType", ValidationType.TYPE_FORMAT, "TaxFormType", path, "", error);
		}
		return success("TaxFormType", ValidationType.TYPE_FORMAT, "TaxFormType", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TaxFormType", ValidationType.TYPE_FORMAT, "TaxFormType", path, "", res.getError());
				}
				return success("TaxFormType", ValidationType.TYPE_FORMAT, "TaxFormType", path, "");
			})
			.collect(toList());
