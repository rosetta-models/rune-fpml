...
public class InterestAccrualsCompoundingMethodValidator implements Validator<InterestAccrualsCompoundingMethod> {
	...
	private List<ComparisonResult> getComparisonResults(InterestAccrualsCompoundingMethod o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("interestAccrualsCompoundingMethodSequence", (InterestAccrualsCompoundingMethodSequence) o.getInterestAccrualsCompoundingMethodSequence() != null ? 1 : 0, 0, 1)
			);
	}
	...
	public ValidationResult<InterestAccrualsCompoundingMethod> validate(RosettaPath path, InterestAccrualsCompoundingMethod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InterestAccrualsCompoundingMethod", ValidationType.CARDINALITY, "InterestAccrualsCompoundingMethod", path, "", error);
		}
		return success("InterestAccrualsCompoundingMethod", ValidationType.CARDINALITY, "InterestAccrualsCompoundingMethod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestAccrualsCompoundingMethod", ValidationType.CARDINALITY, "InterestAccrualsCompoundingMethod", path, "", res.getError());
				}
				return success("InterestAccrualsCompoundingMethod", ValidationType.CARDINALITY, "InterestAccrualsCompoundingMethod", path, "");
			})
			.collect(toList());
