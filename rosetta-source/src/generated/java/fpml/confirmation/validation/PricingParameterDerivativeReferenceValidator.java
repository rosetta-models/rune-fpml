...
public class PricingParameterDerivativeReferenceValidator implements Validator<PricingParameterDerivativeReference> {
	...
	public ValidationResult<PricingParameterDerivativeReference> validate(RosettaPath path, PricingParameterDerivativeReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PricingParameterDerivativeReference", ValidationType.CARDINALITY, "PricingParameterDerivativeReference", path, "", error);
		}
		return success("PricingParameterDerivativeReference", ValidationType.CARDINALITY, "PricingParameterDerivativeReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingParameterDerivativeReference", ValidationType.CARDINALITY, "PricingParameterDerivativeReference", path, "", res.getError());
				}
				return success("PricingParameterDerivativeReference", ValidationType.CARDINALITY, "PricingParameterDerivativeReference", path, "");
			})
			.collect(toList());
