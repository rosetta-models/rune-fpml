...
public class BrokerEquityOptionTypeFormatValidator implements Validator<BrokerEquityOption> {
	...
	private List<ComparisonResult> getComparisonResults(BrokerEquityOption o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkString("brokerNotes", o.getBrokerNotes(), 0, of(255), empty())
			);
	}
	...
	public ValidationResult<BrokerEquityOption> validate(RosettaPath path, BrokerEquityOption o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BrokerEquityOption", ValidationType.TYPE_FORMAT, "BrokerEquityOption", path, "", error);
		}
		return success("BrokerEquityOption", ValidationType.TYPE_FORMAT, "BrokerEquityOption", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BrokerEquityOption", ValidationType.TYPE_FORMAT, "BrokerEquityOption", path, "", res.getError());
				}
				return success("BrokerEquityOption", ValidationType.TYPE_FORMAT, "BrokerEquityOption", path, "");
			})
			.collect(toList());
