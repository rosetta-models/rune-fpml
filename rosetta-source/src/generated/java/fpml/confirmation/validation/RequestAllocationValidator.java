...
public class RequestAllocationValidator implements Validator<RequestAllocation> {
	...
	public ValidationResult<RequestAllocation> validate(RosettaPath path, RequestAllocation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestAllocation", ValidationType.CARDINALITY, "RequestAllocation", path, "", error);
		}
		return success("RequestAllocation", ValidationType.CARDINALITY, "RequestAllocation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestAllocation", ValidationType.CARDINALITY, "RequestAllocation", path, "", res.getError());
				}
				return success("RequestAllocation", ValidationType.CARDINALITY, "RequestAllocation", path, "");
			})
			.collect(toList());
