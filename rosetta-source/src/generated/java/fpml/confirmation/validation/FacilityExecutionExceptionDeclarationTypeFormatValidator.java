...
public class FacilityExecutionExceptionDeclarationTypeFormatValidator implements Validator<FacilityExecutionExceptionDeclaration> {
	...
	public ValidationResult<FacilityExecutionExceptionDeclaration> validate(RosettaPath path, FacilityExecutionExceptionDeclaration o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityExecutionExceptionDeclaration", ValidationType.TYPE_FORMAT, "FacilityExecutionExceptionDeclaration", path, "", error);
		}
		return success("FacilityExecutionExceptionDeclaration", ValidationType.TYPE_FORMAT, "FacilityExecutionExceptionDeclaration", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityExecutionExceptionDeclaration", ValidationType.TYPE_FORMAT, "FacilityExecutionExceptionDeclaration", path, "", res.getError());
				}
				return success("FacilityExecutionExceptionDeclaration", ValidationType.TYPE_FORMAT, "FacilityExecutionExceptionDeclaration", path, "");
			})
			.collect(toList());
