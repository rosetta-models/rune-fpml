...
public class DataDocumentTypeFormatValidator implements Validator<DataDocument> {
	...
	public ValidationResult<DataDocument> validate(RosettaPath path, DataDocument o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DataDocument", ValidationType.TYPE_FORMAT, "DataDocument", path, "", error);
		}
		return success("DataDocument", ValidationType.TYPE_FORMAT, "DataDocument", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DataDocument", ValidationType.TYPE_FORMAT, "DataDocument", path, "", res.getError());
				}
				return success("DataDocument", ValidationType.TYPE_FORMAT, "DataDocument", path, "");
			})
			.collect(toList());
