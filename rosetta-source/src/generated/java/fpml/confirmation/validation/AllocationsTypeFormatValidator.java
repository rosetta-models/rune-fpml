...
public class AllocationsTypeFormatValidator implements Validator<Allocations> {
	...
	public ValidationResult<Allocations> validate(RosettaPath path, Allocations o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Allocations", ValidationType.TYPE_FORMAT, "Allocations", path, "", error);
		}
		return success("Allocations", ValidationType.TYPE_FORMAT, "Allocations", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Allocations", ValidationType.TYPE_FORMAT, "Allocations", path, "", res.getError());
				}
				return success("Allocations", ValidationType.TYPE_FORMAT, "Allocations", path, "");
			})
			.collect(toList());
