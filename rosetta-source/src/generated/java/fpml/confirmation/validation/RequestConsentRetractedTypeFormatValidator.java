...
public class RequestConsentRetractedTypeFormatValidator implements Validator<RequestConsentRetracted> {
	...
	public ValidationResult<RequestConsentRetracted> validate(RosettaPath path, RequestConsentRetracted o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("RequestConsentRetracted", ValidationType.TYPE_FORMAT, "RequestConsentRetracted", path, "", error);
		}
		return success("RequestConsentRetracted", ValidationType.TYPE_FORMAT, "RequestConsentRetracted", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConsentRetracted", ValidationType.TYPE_FORMAT, "RequestConsentRetracted", path, "", res.getError());
				}
				return success("RequestConsentRetracted", ValidationType.TYPE_FORMAT, "RequestConsentRetracted", path, "");
			})
			.collect(toList());
