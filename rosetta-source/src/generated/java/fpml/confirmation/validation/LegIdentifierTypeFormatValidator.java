...
public class LegIdentifierTypeFormatValidator implements Validator<LegIdentifier> {
	...
	public ValidationResult<LegIdentifier> validate(RosettaPath path, LegIdentifier o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("LegIdentifier", ValidationType.TYPE_FORMAT, "LegIdentifier", path, "", error);
		}
		return success("LegIdentifier", ValidationType.TYPE_FORMAT, "LegIdentifier", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegIdentifier", ValidationType.TYPE_FORMAT, "LegIdentifier", path, "", res.getError());
				}
				return success("LegIdentifier", ValidationType.TYPE_FORMAT, "LegIdentifier", path, "");
			})
			.collect(toList());
