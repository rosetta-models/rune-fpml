...
public class CommodityBasketUnderlyingBaseTypeFormatValidator implements Validator<CommodityBasketUnderlyingBase> {
	...
	public ValidationResult<CommodityBasketUnderlyingBase> validate(RosettaPath path, CommodityBasketUnderlyingBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityBasketUnderlyingBase", ValidationType.TYPE_FORMAT, "CommodityBasketUnderlyingBase", path, "", error);
		}
		return success("CommodityBasketUnderlyingBase", ValidationType.TYPE_FORMAT, "CommodityBasketUnderlyingBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBasketUnderlyingBase", ValidationType.TYPE_FORMAT, "CommodityBasketUnderlyingBase", path, "", res.getError());
				}
				return success("CommodityBasketUnderlyingBase", ValidationType.TYPE_FORMAT, "CommodityBasketUnderlyingBase", path, "");
			})
			.collect(toList());
