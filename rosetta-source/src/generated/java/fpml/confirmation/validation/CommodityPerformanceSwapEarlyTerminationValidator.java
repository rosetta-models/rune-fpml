...
public class CommodityPerformanceSwapEarlyTerminationValidator implements Validator<CommodityPerformanceSwapEarlyTermination> {
	...
	public ValidationResult<CommodityPerformanceSwapEarlyTermination> validate(RosettaPath path, CommodityPerformanceSwapEarlyTermination o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CommodityPerformanceSwapEarlyTermination", ValidationType.CARDINALITY, "CommodityPerformanceSwapEarlyTermination", path, "", error);
		}
		return success("CommodityPerformanceSwapEarlyTermination", ValidationType.CARDINALITY, "CommodityPerformanceSwapEarlyTermination", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPerformanceSwapEarlyTermination", ValidationType.CARDINALITY, "CommodityPerformanceSwapEarlyTermination", path, "", res.getError());
				}
				return success("CommodityPerformanceSwapEarlyTermination", ValidationType.CARDINALITY, "CommodityPerformanceSwapEarlyTermination", path, "");
			})
			.collect(toList());
