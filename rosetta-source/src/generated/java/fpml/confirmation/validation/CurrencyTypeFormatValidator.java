...
public class CurrencyTypeFormatValidator implements Validator<Currency> {
	...
	public ValidationResult<Currency> validate(RosettaPath path, Currency o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Currency", ValidationType.TYPE_FORMAT, "Currency", path, "", error);
		}
		return success("Currency", ValidationType.TYPE_FORMAT, "Currency", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Currency", ValidationType.TYPE_FORMAT, "Currency", path, "", res.getError());
				}
				return success("Currency", ValidationType.TYPE_FORMAT, "Currency", path, "");
			})
			.collect(toList());
