...
public class AllocationReportingStatusValidator implements Validator<AllocationReportingStatus> {
	...
	public ValidationResult<AllocationReportingStatus> validate(RosettaPath path, AllocationReportingStatus o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AllocationReportingStatus", ValidationType.CARDINALITY, "AllocationReportingStatus", path, "", error);
		}
		return success("AllocationReportingStatus", ValidationType.CARDINALITY, "AllocationReportingStatus", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationReportingStatus", ValidationType.CARDINALITY, "AllocationReportingStatus", path, "", res.getError());
				}
				return success("AllocationReportingStatus", ValidationType.CARDINALITY, "AllocationReportingStatus", path, "");
			})
			.collect(toList());
