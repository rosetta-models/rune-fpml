...
public class FacilityPositionValidator implements Validator<FacilityPosition> {
	...
	public ValidationResult<FacilityPosition> validate(RosettaPath path, FacilityPosition o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FacilityPosition", ValidationType.CARDINALITY, "FacilityPosition", path, "", error);
		}
		return success("FacilityPosition", ValidationType.CARDINALITY, "FacilityPosition", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityPosition", ValidationType.CARDINALITY, "FacilityPosition", path, "", res.getError());
				}
				return success("FacilityPosition", ValidationType.CARDINALITY, "FacilityPosition", path, "");
			})
			.collect(toList());
