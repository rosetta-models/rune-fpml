...
public class TriggerChoiceValidator implements Validator<TriggerChoice> {
	...
	public ValidationResult<TriggerChoice> validate(RosettaPath path, TriggerChoice o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TriggerChoice", ValidationType.CARDINALITY, "TriggerChoice", path, "", error);
		}
		return success("TriggerChoice", ValidationType.CARDINALITY, "TriggerChoice", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TriggerChoice", ValidationType.CARDINALITY, "TriggerChoice", path, "", res.getError());
				}
				return success("TriggerChoice", ValidationType.CARDINALITY, "TriggerChoice", path, "");
			})
			.collect(toList());
