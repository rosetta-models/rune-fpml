...
public class ValuationDatesReferenceTypeFormatValidator implements Validator<ValuationDatesReference> {
	...
	public ValidationResult<ValuationDatesReference> validate(RosettaPath path, ValuationDatesReference o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ValuationDatesReference", ValidationType.TYPE_FORMAT, "ValuationDatesReference", path, "", error);
		}
		return success("ValuationDatesReference", ValidationType.TYPE_FORMAT, "ValuationDatesReference", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationDatesReference", ValidationType.TYPE_FORMAT, "ValuationDatesReference", path, "", res.getError());
				}
				return success("ValuationDatesReference", ValidationType.TYPE_FORMAT, "ValuationDatesReference", path, "");
			})
			.collect(toList());
