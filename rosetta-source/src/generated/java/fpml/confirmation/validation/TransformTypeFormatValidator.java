...
public class TransformTypeFormatValidator implements Validator<Transform> {
	...
	public ValidationResult<Transform> validate(RosettaPath path, Transform o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Transform", ValidationType.TYPE_FORMAT, "Transform", path, "", error);
		}
		return success("Transform", ValidationType.TYPE_FORMAT, "Transform", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Transform", ValidationType.TYPE_FORMAT, "Transform", path, "", res.getError());
				}
				return success("Transform", ValidationType.TYPE_FORMAT, "Transform", path, "");
			})
			.collect(toList());
