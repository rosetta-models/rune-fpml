...
public class AllocationTypeFormatValidator implements Validator<Allocation> {
	...
	public ValidationResult<Allocation> validate(RosettaPath path, Allocation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Allocation", ValidationType.TYPE_FORMAT, "Allocation", path, "", error);
		}
		return success("Allocation", ValidationType.TYPE_FORMAT, "Allocation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Allocation", ValidationType.TYPE_FORMAT, "Allocation", path, "", res.getError());
				}
				return success("Allocation", ValidationType.TYPE_FORMAT, "Allocation", path, "");
			})
			.collect(toList());
